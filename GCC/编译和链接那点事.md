...
不过因为**数学函数库libm.a没有默认链接**，所以我们**使用了数学函数的代码**在**编译**时需要在命令行指定 `-lm` 链接(`-l`是**指定链接库**，`m`是**去掉lib之后的库名**)，不过现在**好多gcc**都**默认链接libm.c库**了，比如我机子上的gcc 4.6.3会默认链接的. 
...
```
	...
	movl	-4(%rbp), %edx    ; 得到参数
	addl	%edx, %eax        ; a + b
	movl	%eax, c(%rip)     ; 将结果赋值给c, 表明是内存单元(rip + c)
```
