...
**目标文件的格式**已经无限度**接近可执行文件**了，Unix/Linux下的**目标文件的格式**叫做**ELF**（`Executable and Linkable Format，可执行连接格式`）。详细的讨论可执行文件的格式超出了本文范围，我们只需要知道可执行文件中代码，数据，符号等内容分别存储在不同的段中就可以了，这也和保护模式下的内存分段是有一定关系的，但是这个又会扯远就不详谈了……
