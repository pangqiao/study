...
Perf是内置于Linux源代码树中的性能剖析(profiling)工具。它基于事件采样原理，以**性能事件**为基础，支持针对处理器相关性能指标与操作系统相关性能指标的性能剖析。可用于性能瓶颈的查找与热点代码的定位。
...
......
![2020-07-19-21-06-09.png](./images/2020-07-19-21-06-09.png)
......
...
上图显示的是`[pi]`程序中`do_pi()`函数的**注解**。从图上可以看到perf对`do_pi()`中的C代码给出了汇编语言的注解。并且给出了各条指令的采样率。从上图可以看到**耗时较多的指令**（如访存指令）比较容易被perf采到。
