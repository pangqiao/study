...
* `perf kvm record <command>`: 记录性能数据并保存到文件. 
    * 默认情况下, `perf.data.guest`
    * 如果 `--host` 和 `--guest` 均在命令中使用，输出将被储存在 `perf.data.kvm` 中。
    * 如果只有 `--host` 被使用，文件将被命名为 `perf.data.host`。
    * 同样地，如果仅有 `--guest` 被使用，文件将被命名为 `perf.data.guest`。
...
```
./perf kvm stat record -a -p 178264 sleep
```