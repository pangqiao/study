...
- [8. Perf 性能事件](#8-perf-性能事件)
  ...
  - [8.2. perf list查看所有性能事件](#82-perf-list查看所有性能事件)
    - [8.2.1. 硬件性能事件(PMCs)](#821-硬件性能事件pmcs)
    - [8.2.2. 软件性能事件](#822-软件性能事件)
    - [8.2.3. 内核态Tracepoints](#823-内核态tracepoints)
    - [8.2.4. User-Level Statically Defined Tracing (USDT): 用户态tracepoint事件](#824-user-level-statically-defined-tracing-usdt-用户态tracepoint事件)
    - [8.2.5. 动态追踪](#825-动态追踪)
    - [8.2.6.](#826)
...
- [例子](#例子)
  - [CPU Statistics: CPU统计](#cpu-statistics-cpu统计)
  - [Static Kernel Tracing: 静态内核追踪](#static-kernel-tracing-静态内核追踪)
...
### 8.2.4. User-Level Statically Defined Tracing (USDT): 用户态tracepoint事件

和kernel tracepoints类似, USDT是在用户态应用中硬编码的, 对外表现就是稳定的API(事件名字和参数).

目前, 很多应用程序已经开始包含了tracepoints, 用来支持DTrace.
