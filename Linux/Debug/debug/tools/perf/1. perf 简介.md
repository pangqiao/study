...
Perf是一个基于内核的子系统，它提供一个**性能分析框架**，比如**硬件**（`CPU`、`PMU`）功能和**软件**(`软件计数器`、`tracepoint`)功能, 通过perf，应用程序可以利用这几个功能(**PMU**、**tracepoint**和**内核中的计数器**`<软件计数器>`)来进行**性能统计**。由`perf_events`**接口**暴露，并提供 **perf** 这一**用户空间工具**.
