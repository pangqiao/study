...
- [6. Perf性能事件](#6-perf性能事件)
  ...
  - [6.2. 性能事件的属性](#62-性能事件的属性)
    ...
    - [6.2.3. 其他属性](#623-其他属性)
  - [6.3. 没有预定义字符描述的硬件性能事件](#63-没有预定义字符描述的硬件性能事件)
...
Perf是一个基于内核的子系统，它提供一个**性能分析框架**，它以**性能事件**为基础，基于**对这些事件采样**进行**性能统计**原理，可用于性能瓶颈的查找与热点代码的定位.

而Linux有多少性能事件呢?? 见下面部分。
...
Perf比如**硬件**（`CPU`、`PMU`）功能和**软件**(`软件计数器`、`tracepoint`)功能, 通过perf，应用程序可以利用这几个功能(**PMU**、**tracepoint**和**内核中的计数器**`<软件计数器>`)来进行**性能统计**。由`perf_events`**接口**暴露，并提供 **perf** 这一**用户空间工具**.

Perf基于**事件采样**原理，以**性能事件**为基础，支持针对处理器相关性能指标与操作系统相关性能指标的性能剖析。可用于性能瓶颈的查找与热点代码的定位。
...
- Software Event是**内核产生的事件**，分布在**各个功能模块**中，统计和**操作系统相关性能事件**。比如**进程切换**，等。
