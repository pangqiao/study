...
- [5. ftrace 的相关文件](#5-ftrace-的相关文件)
  ...
  - [set_ftrace_filter 和 set_ftrace_notrace](#set_ftrace_filter-和-set_ftrace_notrace)
  - [set_ftrace_pid](#set_ftrace_pid)
  - [set_ftrace_notrace_pid](#set_ftrace_notrace_pid)
  - [set_event_pid](#set_event_pid)
  - [set_event_notrace_pid](#set_event_notrace_pid)
...
- [task 是否被 trace 跳过](#task-是否被-trace-跳过)
...
- [wakeup跟踪器](#wakeup跟踪器)
- [wakeup_rt跟踪器](#wakeup_rt跟踪器)
- [Latency tracing and events](#latency-tracing-and-events)
- [hwlat跟踪器](#hwlat跟踪器)
- [14. max stack tracer 的使用](#14-max-stack-tracer-的使用)
- [15. 动态ftrace](#15-动态ftrace)
  - [15.1. 指定模块](#151-指定模块)
  - [15.2. 跟踪模块初始化](#152-跟踪模块初始化)
- [16. trace选项](#16-trace选项)
  - [16.1. 追踪器特定的 option](#161-追踪器特定的-option)
    - [16.1.1. function tracer](#1611-function-tracer)
    - [16.1.2. function_graph tracer](#1612-function_graph-tracer)
    - [16.1.3. blk tracer](#1613-blk-tracer)
...
tracing目录（`/sys/kernel/debug/tracing`）中的文件控制着跟踪的能力. 根据你在内核配置时的选项的不同, 这里列的文件可能稍有差异. 有些是**各种跟踪器共享使用**的, 有些是**特定于某个跟踪器使用**的. 
...
>注意: `set_ftrace_filter` 和 `set_ftrace_notrace` 还是会起作用, 会决定哪些函数被追踪, 哪些不被追踪.
...
### 16.1.1. function tracer

* func_stack_trace

### 16.1.2. function_graph tracer
