...
- [8. Perf 性能事件](#8-perf-性能事件)
  - [8.1. 六种类型事件](#81-六种类型事件)
    - [8.1.1. perf list查看所有性能事件](#811-perf-list查看所有性能事件)
  - [8.2. Hardware Events (PMCs): 硬件性能事件(PMCs)](#82-hardware-events-pmcs-硬件性能事件pmcs)
  - [8.3. Software Events: 软件性能事件](#83-software-events-软件性能事件)
  - [8.4. Kernel Tracepoints: 内核态Tracepoints](#84-kernel-tracepoints-内核态tracepoints)
  - [8.5. User-Level Statically Defined Tracing (USDT): 用户态tracepoint](#85-user-level-statically-defined-tracing-usdt-用户态tracepoint)
...
- [9. 例子](#9-例子)
  - [9.1. CPU Statistics: CPU统计](#91-cpu-statistics-cpu统计)
    ...
    - [9.1.2. Detailed Mode](#912-detailed-mode)
    - [9.1.3. Specific Counters](#913-specific-counters)
    - [9.1.4. Raw Counters](#914-raw-counters)
    - [9.1.5. Other Options](#915-other-options)
...
* `sample`: 采样. 对**某几个事件**进行**采样**, 收集**性能数据**(比如**IP**和**堆栈**), 这就意味着每隔一段时间就会收集一次数据, 所以开销变大
...
* `counting`(**计数**): 在**内核上下文**中对事件进行**计数**，其中perf打印计数摘要。 此模式**不会生成perf.data文件**。
* `sampling`(**采样**): 采样事件，将**事件数据**写入**内核缓冲区**，perf命令以柔和的异步速率读取事件数据，以写入`perf.data`文件。 然后，perf report或perf script命令将**读取此文件**。
* `bpf`: 关于事件的**bpf程序**，这是Linux 4.4+内核中的一项新功能，可以在**内核空间**中执行**自定义的用户定义程序**，从而可以执行高效的数据过滤和汇总。 例如，有效测量的时间延迟直方图。
...
3. 要真正**减少开销**并生成更多**高级摘要**，需要编写由perf执行的**BPF程序**。 请参阅[eBPF](http://www.brendangregg.com/perf.html#eBPF)部分。

## 7.7. 使用示例

选择这些例子是为了说明perf的**不同使用方式**, 从收集到报告.

1. **gzip命令**的**性能计数**摘要，包括IPC等：
...
2. 统计(count)5秒的**所有调度程序进程事件**，按**tracepoint**计数：
...
3. 跟踪(trace)5秒的**所有调度程序进程事件**，按**tracepoint**和**进程名称**计数：
...
* **硬件事件**(`Hardware Events`): CPU性能监控计数(CPU performance monitoring counters).
* **软件事件**(`Software Events`): 这些是基于内核计数器的低级事件. 比如, CPU迁移、次要故障和主要故障等等.
* **内核tracepoint事件**(`Kernel Tracepoint Events`): 这些是内核中的静态检测点, 是在内核合适位置硬编码进去的.
* **用户静态定义追踪**(`User Statically-Defined Tracing`, USDT): 这些是用户态程序中的静态tracepoint.
* **动态追踪**(`Dynamic Tracing`): 可以在软件的任何位置创建事件, 从而可以被动态检测. 
  * 对于**内核**来说, 利用了`kprobes`框架;
  * 对于**用户态**程序, 使用了`uprobes`.
* **定时分析**(`Timed Profiling`): 使用`perf record -F Hz`, 能够以任意频率收集快照(当时的系统信息). 这通常用于**CPU使用情况**分析, 并且通过**创建自定义定时中断事件**来工作.

### 8.1.1. perf list查看所有性能事件
...
## 8.2. Hardware Events (PMCs): 硬件性能事件(PMCs)
...
## 8.4. Kernel Tracepoints: 内核态Tracepoints
...
## 8.5. User-Level Statically Defined Tracing (USDT): 用户态tracepoint
...
虽然动态跟踪可以看到**所有东西**，但它也是一个不稳定的接口，因为它**检测原始代码**。这意味着在**内核补丁**或**更新**之后，您开发的任何**动态跟踪工具**都**可能崩溃**。**首先**尝试使用**静态跟踪点**，因为它们的接口应该更加稳定。它们也更容易使用和理解，因为在设计时考虑了跟踪最终用户。

动态跟踪的一个好处是可以在活动的系统上启用它，而不需要重新启动任何东西。您可以使用一个已经运行的内核或应用程序，然后开始动态插装，这将(安全地)对内存中的指令进行补丁，以添加插装。这意味着在您开始使用该特性之前，它没有任何开销或税收。前一刻二进制文件还在全速运行，而下一刻，它又在运行一些您动态添加的额外插装指令。一旦您完成了动态跟踪会话的使用，这些指令最终应该被删除。

使用动态跟踪和执行额外指令时的开销相对于检测事件的频率乘以在每个检测上完成的工作。


使用动态追踪的例子, 见`Dynamic Tracing`
...
`perf stat`有一个Detailed Mode, 通过`-d`选项:
...
这个额外包含了L1 data cache(L1 数据缓存)和LLC cache(最后一级缓存)事件.

### 9.1.3. Specific Counters
