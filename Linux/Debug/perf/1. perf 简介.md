...
- [9. 示例(性能分析分类)](#9-示例性能分析分类)
  ...
  - [9.6. Dynamic Tracing: 动态追踪](#96-dynamic-tracing-动态追踪)
    - [Kernel: tcp_sending(): 某个内核函数](#kernel-tcp_sending-某个内核函数)
    ...
    - [User: malloc(): 用户态函数](#user-malloc-用户态函数)
    - [User: malloc() with size:](#user-malloc-with-size)
  - [9.7. Scheduler Analysis: 调度器分析](#97-scheduler-analysis-调度器分析)
    - [perf sched record: 数据采样](#perf-sched-record-数据采样)
    - [perf script --header: 查看数据文件中的事件](#perf-script-header-查看数据文件中的事件)
    - [对数据文件进行分析](#对数据文件进行分析)
      - [perf sched latency: 调度延迟分析](#perf-sched-latency-调度延迟分析)
      - [perf sched map:](#perf-sched-map)
  - [9.8. eBPF](#98-ebpf)
    - [9.8.1. Prerequisites: 前提条件](#981-prerequisites-前提条件)
    - [kmem_cache_alloc from Example](#kmem_cache_alloc-from-example)
...
# 指定命令的CPU上的功能, 以99hz的频率
perf record -F 99 command

# Sample on-CPU functions for the specified PID, at 99 Hertz, until Ctrl-C:
# 指定进程的CPU上的功能, 以99hz的频率
...
* `sampling`(**采样**): 采样事件，将**事件数据**写入**内核缓冲区**，perf命令以柔和的异步速率读取事件数据，以写入`perf.data`文件。 然后，`perf report`或`perf script`命令将**读取此文件**。`sampling-with-post-processing tracer`采样然后处理追踪器.
* `bpf`(**动态插入自定义程序**): 事件的**bpf程序**，这是Linux 4.4+内核中的一项新功能，可以在**内核空间**中执行**自定义的用户定义程序**，从而可以执行高效的**数据过滤**和**汇总**。 例如，有效测量的时间延迟直方图。`programmable tracer`可编程追踪器.
...
![2020-12-27-19-11-28.png](./images/2020-12-27-19-11-28.png)
...
eBPF当前受到perf的限制，很难使用。 一直在变好。 通过bcc Python界面是访问eBPF的一种不同且当前更简单的方法，该界面在我的eBPF工具页面上进行了描述。 在此页面上，我将讨论性能。
...
该程序仅在其调用函数与在内核上下文中过滤的指定范围匹配时才跟踪内核`kmem_cache_alloc()`函数。 您可以想象这样做是为了提高效率：不必跟踪所有分配（这会非常频繁并且会增加大量开销），而是仅对一系列感兴趣的内核调用函数（例如内核模块）进行过滤。 我将松散地匹配tcp函数作为示例，这些函数在内存中的以下地址处：
...
我假设这些**函数是连续的**，因此通过跟踪范围`0xffffffff817c1bb0`到`0xffffffff8187bd89`，可以匹配很多tcp。
