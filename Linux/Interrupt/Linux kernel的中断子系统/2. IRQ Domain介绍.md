...
**map和unmap**是**操作相反**的函数，我们描述其中之一就OK了。**调用map**函数的时机是在**创建（或者更新）HW interrupt ID（hw参数）和IRQ number（virq参数）关系的时候**。其实，从**发生一个中断**到**调用该中断的handler**仅仅调用一个**request\_threaded\_irq**是不够的，还需要针对该irq number设定：
