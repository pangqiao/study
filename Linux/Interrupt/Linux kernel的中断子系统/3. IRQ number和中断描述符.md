...
（2）控制interrupt controller，关闭**某个IRQ number**对应的中断。更准确的术语是mask或者unmask一个IRQ。
...
（1）**handle\_irq**就是**highlevel irq\-events handler**，何谓high level？站在高处自然看不到细节。我认为high level是和specific相对，specific handler处理具体的事务，例如处理一个按键中断、处理一个磁盘中断。而**high level**则是对处理各种**中断交互过程的一个抽象**，根据下列硬件的不同：
