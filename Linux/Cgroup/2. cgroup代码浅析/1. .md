...
- [cgroup概念](#cgroup概念)
  ...
  - [进程分组: css_set](#进程分组-css_set)
  - [进程分组css_set和节点cgroup的关系: 多对多](#进程分组css_set和节点cgroup的关系-多对多)
- [task_struct](#task_struct)
- [css_set](#css_set)
- [cgroup_subsys_state 和 cgroup_subsys](#cgroup_subsys_state-和-cgroup_subsys)
- [cgroup](#cgroup)
- [全局变量](#全局变量)
- [参考](#参考)
...
我们把**每种资源**叫做**子系统**，比如**CPU子系统**，内存子系统。为什么叫做子系统呢，因为它是从**整个操作系统的资源衍生出来**的。

然后我们创建一种**虚拟的节点**，叫做**cgroup**，然后这个虚拟节点可以扩展，以树形的结构，有**root节点**和**子节点**。这个**父节点**和**各个子节点**就形成了**层级**（`hierarchy`）。

**每个层级**都可以附带继承**一个或者多个子系统**，就意味着，我们把**资源**按照**分割**到**多个层级系统**中，层级系统中的**每个节点**对**这个资源**的**占比各有不同**。

* 子系统: **每个代表了使用的某类系统资源信息**
...
**进程分组css_set**、**不同层级**中的**节点cgroup**也都有了。那么，就要把**节点cgroup**和**进程分组css_set**进行关联，和数据库中关系表一样。这个是一个**多对多的关系**。为什么呢？
...
`css_set`是直接和task关联的结构，`css_set`用来关联一组`cgroup_subsys_state`对象，同时通过`cg_cgroup_link`和**所有相关的cgroup**建立关联。

```cpp
struct css_set {
    // 包含一系列的css, 这个就代表了css_set和子系统的多对多的其中一面
    // css_set关联的所以css(cgroup_subsys_state). css就是子系统状态 
...
subsys应当包括所有子系统的css. 如果此css_set没有指定某个subsystem的css或者subsystem没有mount，则默认初始化为根css.
...
这里说一下idr。这个是linux的**整数id管理机制**。你可以把它看成**一个map**，这个map是把**id**和**指定指针**关联在一起的机制。它的原理是使用**基数树**。**一个结构**存储了一个**idr**，就能很方便**根据id找出这个id对应的结构的地址**了。http://blog.csdn.net/dlutbrucezhang/article/details/10103371

# cgroup

`cgroup_root`和子节点cgroup是使用两个不同结构表示的.
