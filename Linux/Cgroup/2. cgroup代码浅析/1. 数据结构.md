...
- [3. 相关数据结构](#3-相关数据结构)
  ...
  - [3.2. css_set](#32-css_set)
    - [3.2.1. rcu 锁](#321-rcu-锁)
  - [3.3. cgroup_subsys_state](#33-cgroup_subsys_state)
  - [3.4. cgroup_subsys](#34-cgroup_subsys)
  - [3.5. cgroup](#35-cgroup)
- [4. css_set 和 cgroup](#4-css_set-和-cgroup)
  - [4.1. cgrp_cset_link](#41-cgrp_cset_link)
  - [查找 task 的 cgroup 节点](#查找-task-的-cgroup-节点)
- [5. css 和 cgroup](#5-css-和-cgroup)
  - [5.3. cgroup_root](#53-cgroup_root)
  - [5.4. 全局变量](#54-全局变量)
  - [5.5. 小结](#55-小结)
- [2. 整体架构](#2-整体架构)
  - [2.1. 约束](#21-约束)
- [示例](#示例)
  - [5.1. cgroup v1](#51-cgroup-v1)
  - [5.2. cgroup v2](#52-cgroup-v2)
- [7. 参考](#7-参考)
...
`css_set` 是**各类子系统状态(资源限制等)实例！！！的一个集合**. 每个 css_set 都有一个 `cgroup_subsys_state subsys[]` 数组, 代表**关联到这个 css_set 的进程组**对应的**每种类型资源的状态**(即**每种子系统的资源限制**), `css_set->subsys[]->cgroup`就是**这类资源限额信息**的**节点**, 这个cgroup节点本质上**不是task所在的cgroup！！！**, 只是多数情况下相等. 

> 比如一种情况, 父进程的 `pids css >cgroup` 是 `/sys/fs/cgroup/pids/A`, 自然也是父进程的 cgroup 节点; 后面子进程fork的, 所以初始也是那样, 后来有了子目录, echo进去了, 会不会出现`pids css >cgroup` 是 `/sys/fs/cgroup/pids/A`, 但是子进程 cgroup 节点是 `/sys/fs/cgroup/pids/B`
...
`css_set` 直接和**task**关联(`tasks`); 同时通过`cgrp_cset_links`, css_set 和**所有相关的cgroup**建立关联, 这样多个cgroup节点和一组task就关联起来了; 从而达到了**限制资源**的目的(**一组进程共享指定数额的资源**). 至于 cgroup 和 css_set 关联的方式, 下面会讲到.
...
* `task_struct->css_set->cgroup_subsys_state[]->cgroup`, 进程组所关联的特定子系统的cgroup节点
...
1. 一个**资源限制集合**(`css_set`)在**一种subsystem**(cpu，devices)的**限制条件只能有一个！！！**，通过 `css_set->subsys[]` 体现**各种类型资源的限制**, `css_set->subsys[]->cgroup` 是**某种类型资源限制**所对应的**cgroup节点**;

2. **多个** `task` 同**一个** `css_set` 关联, 通过 `task->cgroups` 和  `css_set->tasks`; 
...
`task_struct` 是进程描述符的结构体，这里的`ccs_set *cgroups`成员是**task**和**cgroup**的桥梁, `tasks`是一个链表的头指针，该链表包含了所有同一css_set下的task进程。**一个task**对应了**唯一的ccs_set**，但是**一个ccs_set**可以对应**多个task**.
