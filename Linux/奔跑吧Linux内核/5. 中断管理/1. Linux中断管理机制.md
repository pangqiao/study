...
回到\_\_handle\_domain\_irq()函数中，位置2, **irq\_find\_mapping**()函数通过**硬件中断号hwirq**查找**IRQ中断号**，该**中断号在注册中断**时**已经映射过**。
...
while循环**遍历中断描述符**中的**action链表**，依次执行**每个action元素**中的**primary handler**回调函数action\->handler。(一般的，**设备产生中断时，会标记自己的状态寄存器**，action通过检查设备的**状态寄存器**来查找产生中断的设备)
...
最后wake\_up\_process()唤醒该action对应的中断线程。
