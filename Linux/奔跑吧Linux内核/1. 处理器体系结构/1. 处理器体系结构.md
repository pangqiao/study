...
现代处理器在设计上都采用了**超标量体系结构**（Superscalar Architecture）和**乱序执行**（out-of-order）技术。超标量技术能够**在一个时钟周期内执行多个指令**，实现指令级的并行，有效提高了 ILP（Instruction Level Parallelism）指令级的并行效率，同时也增加了整个 cache 和 memory 层次结构的实现难度。
