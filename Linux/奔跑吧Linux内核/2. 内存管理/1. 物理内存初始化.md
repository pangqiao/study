...
| 层次 | 描述 |
|:---:|:----|
| **用户空间层** |可以理解为 `Linux` 内核内存管理**为用户空间暴露的系统调用接口**. 例如 `brk( )`, `mmap( )` 等系统调用. 通常 `libc` 库会将系统调用封装成大家常见的 `C` 库函数, 比如 `malloc( )`, `mmap( )` 等. |
...
| **硬件层** | 包含处理器的 `MMU`, `TLB` 和 `cache` 部件, 以及板载的**物理内存**, 例如 `LPDDR` 或者 `DDR` |
...
```cpp
//[start_kemel() ->setup_arch() ->paging_init() -> prepare_page_table()]
...
其中 `zone` 结构的定义用 `struct zone` 。struct zone数据结构的主要成员如下：
