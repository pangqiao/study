...
**每个MCS锁**有一个**optimistic\_spin\_queue(！！！**)数据结构，该数据结构只有一个成员**tail，初始化为0**。

struct optimistic\_spin\_node数据结构表示**本地CPU上的节点(！！！**)，它可以组织成一个**双向链表(！！！**)，包含**next和prev指针**，lock成员用于表示**加锁状态**，cpu成员用于**重新编码CPU编号**，表示该node是在**哪个CPU上**。
