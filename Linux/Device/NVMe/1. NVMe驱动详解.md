...
3. 遍历**静态 id**(**NVMe 驱动**中的 `id_table` 表), 如果匹配返回 `pci_device_id`(如果设备设置了 `dev->override`, 且**注册的驱动名字**和设备需要的名字匹配, 就算没找到也会也返回一个 `pci_device_id_any`).

> 这里的 id 表会有 Spec 定义的 NVMe 设备的 Class code, 0x010802h
