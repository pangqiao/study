...
在 NVMe 驱动加载中, 如果总线支持自动 probe 设备(`/sys/bus/pci/drivers_autoprobe`, PCI总线是支持的), 则会调用 `driver_attach()` 来 probe 设备, 将设备绑定到该驱动上.

> `driver_attach()` 是一个全局方法, 也就是其他都可以调用, 用于将设备绑定到驱动上
