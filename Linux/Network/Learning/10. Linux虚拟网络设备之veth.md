...
从上面的介绍中可以看出，从**veth0设备**出去的**数据包**，会**转发到veth1**上，如果**目的地址**是**veth1的IP**的话，就能被**协议栈处理**，否则连ARP那关都过不了，IP forward啥的都用不上，所以**不借助其它虚拟设备!!!**的话，这样的数据包只能在本地协议栈里面打转转，没法走到eth0上去，即没法发送到外面的网络中去。
