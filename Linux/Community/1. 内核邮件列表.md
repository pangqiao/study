...
- [3. 归档的邮件列表](#3-归档的邮件列表)
  ...
  - [3.4. mail-archive](#34-mail-archive)
  - [3.5. lkml](#35-lkml)
- [4. 使用方法](#4-使用方法)
  - [4.1. 使用](#41-使用)
    - [4.1.1. mailist更新](#411-mailist更新)
    - [4.1.2. 搜索功能](#412-搜索功能)
  - [4.2. 示例](#42-示例)
...
b. 首页上同一patch set的所有patch都会显示, 有点冗余
...
> 这个比较丑陋, 不是很建议使用, 但是搜索功能很好

* 整体: https://lore.kernel.org/patchwork/project/lkml/list/
...
再使用 `lore.kernel.org/patchwork`, 使用这个**搜索功能**就能得到以前的很多数据.

既想有模块, 又能看以前的数据, 则用 `spinics`

如果想要更多的讨论(非patchset式), 可以在 `lore.kernel.org/lists.html` 相应模块中进行搜索

## 4.1. 使用

一般使用场景: 要么看某个模块的mail list的更新, 要么就想看某个历史patchset(搜索功能)

### 4.1.1. mailist更新

查看**某个模块的mail list的更新**: `patchwork.kernel`

查看所有更新: [lore.kernel/patchwork](https://lore.kernel.org/patchwork/project/lkml/list/?series=&submitter=&state=*&q=&archive=both&delegate=)
...
没有搜索到相关patch数据: [搜索结果](https://patchwork.kernel.org/project/linux-fsdevel/list/?series=&submitter=&state=*&q=Lazily+remove+mounts+on+unlinked+files+and+directories&archive=both&delegate=)
