...
- [2. 怎么订阅](#2-怎么订阅)
- [3. 归档的邮件列表](#3-归档的邮件列表)
  - [patchwork.kernel](#patchworkkernel)
  - [spinics](#spinics)
  - [lore.kernel](#lorekernel)
- [使用方法](#使用方法)
  - [使用](#使用)
  - [示例](#示例)
...
优先使用 `patchwork.kernel`, 但是可能存在以前的数据没有

再使用 `lore.kernel.org/patchwork`, 使用这个搜索就能得到以前的很多数据.
...
一般要么看某个模块的mail list的更新, 要么就想看某个历史patchset(搜索功能)

查看某个模块的mail list的更新: `patchwork.kernel`

历史数据(搜索功能): 优先使用 `patchwork.kernel`, 实在不行用 `lore.kernel.org/patchwork`
