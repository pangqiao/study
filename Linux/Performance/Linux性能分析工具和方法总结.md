...
- [分析方法](#分析方法)
- [CPU使用有问题?](#cpu使用有问题)
  - [内核空间占用了大多数CPU](#内核空间占用了大多数cpu)
  - [内核处理了很多中断吗？](#内核处理了很多中断吗)
  - [内核的时间花在哪了？](#内核的时间花在哪了)
  - [用户空间占了大多是CPU？](#用户空间占了大多是cpu)
  - [哪个进程占用了大多数CPU？](#哪个进程占用了大多数cpu)
  - [进程在内核还是用户空间花费了时间？](#进程在内核还是用户空间花费了时间)
  - [进程在哪些系统调用上花费了较多时间？](#进程在哪些系统调用上花费了较多时间)
  - [进程在哪些函数上花费了时间？](#进程在哪些函数上花费了时间)
- [内存使用有问题？](#内存使用有问题)
  - [内核使用内存在增加？](#内核使用内存在增加)
  - [内核使用的内存类型是什么？](#内核使用的内存类型是什么)
  - [进程使用内存在增加？](#进程使用内存在增加)
    - [进程使用的内存类型是什么？](#进程使用的内存类型是什么)
    - [哪些函数使用大量的栈空间？](#哪些函数使用大量的栈空间)
