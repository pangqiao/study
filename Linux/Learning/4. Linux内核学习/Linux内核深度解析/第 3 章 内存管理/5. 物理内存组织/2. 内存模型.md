...
- [3. DISCONTIG MEM](#3-discontig-mem)
  ...
  - [3.4. pfn与page相互转换](#34-pfn与page相互转换)
...
上述该宏会调用 `virt_to_page()`. 1. 首先要将传入的**物理页帧号 pfn** 转换成**虚拟地址**, 注意**此时的页帧号**是一个**全局的概念**即从 `PAGE_OFFSET` **开始**的**第几页**, 即基于**全局** `mem_map`, 转成成物理地址即 PAGE_OFFSET 的 offset: `(unsigned long)(pfn) << PAGE_SHIFT)` 偏移即可:

```cpp
(unsigned long)(pfn) << PAGE_SHIFT)
```

`__va` 宏为将**物理地址**转换成**虚拟地址**, 加上 `PAGE_OFFSET` **基址**即可:
