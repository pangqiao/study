...
> VMEMMAP_START: 定义了vmemmap区域的起始地址(是线性地址), 大小2TB. sparsemem内存模型中用来存放所有`struct page`的**虚拟地址空间**. 这个地址开始存放 struct page 数组.
>
> 存放的不是 page 内容, 而是 struct page 结构体 metadata
...
// 
struct page *p = virt_to_page(kaddr);
struct page tmp_p;
