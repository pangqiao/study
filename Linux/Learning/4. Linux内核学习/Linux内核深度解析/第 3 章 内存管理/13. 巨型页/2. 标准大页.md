...
- [7. 实现原理](#7-实现原理)
  ...
  - [7.5. 创建内存映射](#75-创建内存映射)
    - [7.5.1. 申请预留大页](#751-申请预留大页)
  - [7.6. 分配和映射到大页](#76-分配和映射到大页)
  - [7.7. 写时复制](#77-写时复制)
...
通过写文件 “`/proc/sys/vm/nr_overcommit_hugepages`” 指定**大页池**中**临时大页的数量**.
...
1. 会先解析 hugepagesz, 再解析 hugepages, 没有 sz, pages 就是默认 sz 的
2. 不同 size 的都会调用
...
第29行和第30行代码, 如果前面有内核参数 “`hugepagesz=`” 指定大页长度, 并且**大页长度**超过**页分配器**支持的**最大阶数**, 那么需要从**引导内存分配器**分配大页. 如果**大页长度小于页分配器支持的最大阶数**, **大页子系统初始化**的时候从**页分配器**分配大页.

> 这里并**没有**针对大页长度order小于页分配器order(11, 2M)这种情况分配页面
...
```cpp
hugetlb_init()
 ├─ hugetlb_add_hstate(HUGETLB_PAGE_ORDER); // 添加 hstate(确保HPAGE_SIZE 的 hstate 存在), hugepagesz_setup 也会调用
...
* 第10～13行代码, 如果有**临时大页**, 那么**把临时大页转换为永久大页**.

* 第15～32行代码, 如果**永久大页**的**数量不够**, 那么**分配大页**.

第 34 行和第 45 行代码, 如果减小永久大页的数量, 处理如下.

* 第34行代码, `min_count` 等于(**大页总数** − (**空闲大页数量** − **预留大页数量**) ), 即**扣除没有预留的空闲大页**.

> 注意**预留大页数量**包含在空闲大页数量里面, 进程创建内存映射的时候已经申请预留大页.

* 第35行代码, `min_count` **不能小于** `count`.

* 第36行代码, 如果支持高端内存区域, 优先把从低端内存区域分配的没有预留的空闲大页归还给页分配器.

* 第37～41行代码, 如果永久大页的数量超过 `min_count`, 那么把没有预留的空闲大页归还给页分配器.

* 第42～45行代码, 如果永久大页的数量超过指定的最大数量, 那么把**永久大页**转换为**临时大页**.
...
如图3.66所示, 超级块的成员 `s_fs_info` 指向 hugetblfs 文件系统的**私有信息**；成员 `s_blocksize` 是**块长度**, 被设置为**大页的长度**.
...
3) 如果是**私有映射**, 那么**创建预留图**, 虚拟内存区域的成员 `vm_private_data` 指向**预留图**, 并且设置标志 `HPAGE_RESV_OWNER` 指明**该虚拟内存区域拥有这个预留**, 如图3.68所示. 计算**需要预留的大页的数量** N =(文件的结束偏移 − 起始偏移), 偏移的单位是**大页长度**.
