...
对于**硬盘**, BIOS将尝试**寻找引导扇区**. 如果在硬盘上存在一个**MBR分区**, 那么**引导扇**区储存在**第一个扇区(512字节)的头446字节**, 引导扇区的最后必须是**0x55和0xaa**, 这2个字节称为魔术字节(Magic Bytes), 如果 BIOS 看到这2个字节, 就知道这个设备是一个可引导设备. 举个例子: 
