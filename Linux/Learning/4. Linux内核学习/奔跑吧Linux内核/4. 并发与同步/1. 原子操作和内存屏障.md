...
如下, 假设线程A和线程B都尝试进行操作, 请问线程A和B函数执行完后, i的值是多少？
...
例 2: Linux内核里面的**睡眠和唤醒API**也运用了**内存屏障指令**, 通常一个**进程**因为**等待某些事件**需要**睡眠**, 例如调用`wait_even()`. **睡眠者代码片段(！！！**)如下: 
...
其中, `set_current_state()`在**修改进程的状态**时隐含插入了**内存屏障函数smp\_mb**(). 
...
唤醒者通常会调用`wake_up()`, 在**修改task状态之前**也隐含地插入内存屏障函数smp\_wmb(). 
