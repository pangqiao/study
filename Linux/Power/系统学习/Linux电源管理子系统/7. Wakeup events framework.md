...
我们知道, 系统处于 suspend 状态, 可通过 wakeup events 唤醒. 具体的 wakeup events 可以是**按键按下**, 可以是**充电器插入**, 等等. 但是, 如果在 suspend 的过程中, 产生了 wakeup events, 怎么办? 答案很肯定, "wakeup"系统. 由于此时系统没有真正 suspend, 所以这的 "wakeup" 是个假动作, 实际上只是终止suspend.
