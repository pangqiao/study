...
第三. 如果写入的值和 cnt 不同(说明读、写的过程中产生 events), 或者 inpr 不为零(说明有 events 正在被处理), 返回 false(说明此时**不宜 suspend**).
...
注2: wakeup后, 会清除 events_check_enabled 标记.

"Wakeup events framework" 已经介绍过 `pm_wakeup_pending` 了, 让我们再看一遍
...
wakeup count 使用了 `events_check_enabled`, `suspend_work` 是 wakeup count 的一个例子
