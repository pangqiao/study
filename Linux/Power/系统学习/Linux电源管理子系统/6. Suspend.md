...
native linux kernel 在 suspend 时候会 save cfg(`pci_pm_suspend_noirq` -> `pci_save_state`), resume 时候会重新 load cfg(`pci_pm_resume` -> `pci_restore_standard_config` -> `pci_restore_state` -> `pci_restore_config_space`), resume是先写 bar 后写 command
