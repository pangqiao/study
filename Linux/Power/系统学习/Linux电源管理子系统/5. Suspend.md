...
理解信号异步机制的关键是**信号的响应时机**, 我们对一个进程**发送一个信号**后, 其实并**没有硬中断发生**, 只是简单把**信号**挂载到目标进程的**信号 pending 队列**上去, 信号真正得到执行的时机是**进程执行完异常/中断返回到用户态**的时刻;
...
内核线程和 workqueue 默认是不能被冻结的, 少数内核线程和 workqueue 在创建的时指定了 freezable 标志。这些任务需要对freeze状态进行判断, 当系统进入 freezing 时, 可以通过调用 freezing 来判断 freezing 状态, 并主动调用, `try_to_freeze` 进入冻结;
