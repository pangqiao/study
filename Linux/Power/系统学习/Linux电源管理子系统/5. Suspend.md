...
**内核线程**和 **workqueue** **默认**是**不能被冻结**的, 少数内核线程和 workqueue 在创建的时指定了 **freezable 标志**。这些任务需要对 freeze 状态进行判断, 当系统进入 freezing 时, 可以通过调用 freezing 来判断 freezing 状态, 并主动调用 `try_to_freeze` 进入冻结;
