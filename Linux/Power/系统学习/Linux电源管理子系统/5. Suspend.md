...
对 suspend 分的几个阶段都是按照 pm test 的 5 种模式来划分的(按顺序进行)：freezer、devices、platform、processors、core。以 `suspend_test(TEST_FREEZER/TEST_DEVICES/TEST_PLATFORM/TEST_CPUS/TEST_CORE)` 为标志.
...
state_store() // 入口
 ...
 └─ pm_suspend() // 进入流程
     ...
     |   ├─ if (suspend_test(TEST_FREEZER)) // 第一个阶段: freezer
