...
static int suspend_enter(suspend_state_t state, bool *wakeup)
{
    ...
    // 上面的内容都是针对 pm_test 是 platform 的, 下面的就不包含了, 所以直接跳转到 Platform_wake 进行唤醒
...
对 suspend 分的几个阶段都是按照 pm test 的 5 种模式来划分的：freezer、devices、platform、processors、core。
...
广播一个通知给通知链 `pm_chain_head`，该通知链上都是用函数 `register_pm_notifier()` 注册的pm通知项(也可以用宏pm_notifier定义和注册)，这里按照注册时候的定义的优先级来调用该通知链上注册的回调函数。
...
那谁会收到这类消息呢?  只有通过 `register_pm_notifier` 进行注册的 pm 通知项的**设备**, 子系统会在这个时候**处理自己的事情**.
