...
内核启动时候在调用每一个init函数前有一句打印，结束后再有一句打印并且输出了该Init函数运行的时间，通过这个信息可以用来定位启动过程中哪个 init函数运行失败以及哪些 init 函数运行时间较长。
...
进程怎么阻止休眠：无论通过wake_lock接口, __pm_stay_awake等, 最终调用的都是 `wakeup_source_activate`, 都是增加inpr表示有事件在处理不准休眠;
