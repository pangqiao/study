...
state_store() // 入口
 ...
 └─ pm_suspend() // 进入流程
     ...
     |   ├─ if valid_state() // 检查平台是否支持, 会调用平台相关的 valid 函数, platform_suspend_ops->valid()
     |   ├─ mutex_trylock(); // 拿锁
     ...
     |   |   ├─ suspend_freeze_processes(); // 冻结 user 进程以及内核线程
     |   |   |   ├─ freeze_processes(); 
     |   |   |       ├─ 
     ...
     |   |   ├─ platform_suspend_begin(state); // 调用 suspend_ops->begin()
     |   |   ├─ suspend_console(); // 挂起控制台
     |   |   ├─ dpm_suspend_start(PMSG_SUSPEND); // 设备 suspend
     |   |   |   ├─ starttime = ktime_get(); // 获取时间
     |   |   |   ├─ dpm_prepare(state); // 执行所有设备的 prepare 回调函数. 执行顺序是 pm_domain-type-class-bus-driver
     |   |   |   ├─ dpm_suspend(); // 执行所有设备的 suspend 回调函数]
     |   |   |   └─ dpm_show_time(starttime, state, error, "start"); // 
     |   |   ├─ suspend_test_finish("suspend devices");
     |   |   ├─ suspend_enter(state, &wakeup); // 系统进入 suspend, 下面的全部就都是 resume 了
     |   |   |   ├─ platform_suspend_prepare(); // 调用平台相关 prepare 回调函数
     |   |   |   ├─ dpm_suspend_late(); // 调用设备的 suspend_late 回调, 并加入一个链表
     |   |   |   ├─ platform_suspend_prepare_late(); // freeze 则调用 freeze_ops 中的 prepare 回调
     |   |   |   ├─ dpm_suspend_noirq(PMSG_SUSPEND); // disable 各个设备的 irq, 
     |   |   |   ├─ platform_suspend_prepare_noirq(state); // 回调平台相关的 preplate_late 函数
     ...
     |   |   ├─ platform_resume_end(state); // 平台 resume
