...
Linux 内核提供了三种 Suspend: Freeze、Standby 和 STR(Suspend to RAM)，在用户空间通过 cat /sys/power/state 查看支持的, 同时向 ”/sys/power/state” 文件分别写入”freeze”、”standby”和”mem”，即可触发它们。
...
通过 `echo mem > /sys/power/state` 就可以达到睡眠, 所以可以根据此节点的 sys 代码分析 suspend 的流程.
...
确保 guest kernel 有 CONFIG_PM_SLEEP_DEBUG 这个选项, 它加上 `/sys/power/pm_debug_messages` 或者 kernel parameter `pm_debug_messages` 从而 debug 函数 pm_pr_dbg() 才能生效
...
# 6. suspend_devices_and_enter 函数分析
