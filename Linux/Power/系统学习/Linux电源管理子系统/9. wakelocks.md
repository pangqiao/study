...
e)如果指定 timeout 值, 以wakelock的wakeup source指针为参数, 调用 `__pm_wakeup_event` 接口, 上报一个具有时限的wakeup events; 否则, 调用__pm_stay_awake, 上报一个没有时限的wakeup event. 有关这两个接口的详细说明, 可参考文章 "Wakeup events framework".
