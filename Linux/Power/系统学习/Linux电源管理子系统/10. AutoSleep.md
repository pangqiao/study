...
第一. 注册了一个 **wakeup source**, 名字叫 "**autosleep**". 因为是虚拟的, 所以关联的 dev 是 NULL. 代码实现见 "Wakeup events framework", 因为没有关联真实设备, 所以 sysfs 节点是 `/sys/devices/wakeup0`

在 `/sys/kernel/debug/wakeup_sources` 中可以看到该信息
