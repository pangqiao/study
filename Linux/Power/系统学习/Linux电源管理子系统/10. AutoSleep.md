...
1) 注册了一个名称为 "`autosleep`" 的 **wakeup source**(`autosleep_ws`), 在 autosleep 执行关键操作时, **阻止系统休眠**(我们可以从中理解 wakeup source 的应用场景和使用方法).

2) 分配了一个名称为 "`autosleep`" 的**有序 workqueue**, 用于**触发实际的休眠动作**(休眠应由**进程**或者**线程**触发). 这里我们要提出 2 个问题: 什么是有序workqueue? 为什么要使用有序workqueue? 后面分析.
...
第一. 注册了一个 **wakeup source**, 名字叫 "**autosleep**". 因为是虚拟的, 所以关联的 dev 是 NULL. 代码实现见 "Wakeup events framework", sysfs 节点是 `/sys/devices/wakeup0`

在 `/sys/kernel/debug/wakeup_sources` 中可以看到
