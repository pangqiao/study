...
第一. 调用 `dpm_prepare` 函数, 执行**所有设备** (`dpm_list` 链表中) 的 `prepare` 回调函数.
...
由于设备模型有 bus, driver, device 等多个层级,而 prepare 接口可能由任意一个层级实现. 所以优先执行顺序是 `dev->pm_domain->ops, dev->type->pm, dev->class->pm, dev->bus->pm, dev->driver->pm`. 如果执行成功, 将该设备添加到 `dpm_prepared_list` 中; 如果**失败**设置 `failed_prepare` 的**引用计数值**.
