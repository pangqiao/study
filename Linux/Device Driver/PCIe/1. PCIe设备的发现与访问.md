...
Host 在**枚举设备的同时**也会**对设备进行配置**, **每个 PCIe 设备**都会指定**一段 CPU memory 访问空间**, 从上面的图中我们可以看到这个设备仅仅支持一段访问空间, 一段的大小是16K byte, **操作系统**会指定**它的基地址**. 基地址设置完成以后, Host 就可以通过地址来对 PCIe memory 空间进行访问了.
