...
该函数的for循环主要是用于检测max\_low\_pfn直接映射空间后面的物理内存是否存在系统启动引导时创建的页表, 如果存在, 则使用pte\_clear()将其清除. 
...
通过max\_zone\_pfns获取各个管理区的最大页面数, 并作为参数调用free\_area\_init\_nodes(),其中free\_area\_init\_nodes()函数实现:
