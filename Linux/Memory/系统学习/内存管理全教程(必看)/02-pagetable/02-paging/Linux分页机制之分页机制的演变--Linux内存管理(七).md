...
- 页表项大小扩展到64位。**64GB的RAM(36位物理总线**）被分成了**2\^24个页框（一个页4MB**）, **页表项**的**物理地址字段从20位扩展到了24位(由于页框有2\^24个！！**）.因为PAE**页表项**必须包含**12个标志位**和**24个物理地址位**,总数之和为**36**,**页表项**大小从32位扩展到了**64位**,结果,一个4KB的页表项包含512个表项而不是1024个表项。

- 引入一个页目录指针表(Page Directory Pointer Table，**PDPT**)的**页表新级别**,它由**4个64位表项**组成.
