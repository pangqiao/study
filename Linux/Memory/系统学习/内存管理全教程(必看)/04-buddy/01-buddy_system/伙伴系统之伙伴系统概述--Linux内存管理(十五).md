...
- [2 伙伴系统的结构](#2-伙伴系统的结构)
  ...
  - [2.2 最大阶MAX\_ORDER与FORCE\_MAX\_ZONEORDER配置选项](#22-最大阶max_order与force_max_zoneorder配置选项)
...
- [3	避免碎片](#3避免碎片)
  ...
  - [3.3 避免碎片数据结构](#33-避免碎片数据结构)
    ...
    - [3.3.3 全局pageblock\_order变量](#333-全局pageblock_order变量)
    - [3.3.4 gfpflags\_to\_migratetype函数转换分配标识到迁移类型](#334-gfpflags_to_migratetype函数转换分配标识到迁移类型)
    - [3.3.5 pageblock\_flags变量与其函数接口](#335-pageblock_flags变量与其函数接口)
  - [3.4 初始化基于可移动性的分组](#34-初始化基于可移动性的分组)
- [4 分配器API](#4-分配器api)
  - [4.1 分配内存的接口](#41-分配内存的接口)
  - [4.2 释放函数](#42-释放函数)
  - [4.3 分配掩码(gfp\_mask标志)](#43-分配掩码gfp_mask标志)
    - [4.3.1 分配掩码](#431-分配掩码)
    - [4.3.2 掩码分类](#432-掩码分类)
  ...
  - [4.4 分配页](#44-分配页)
    - [4.4.1 内存分配统一到alloc\_pages接口](#441-内存分配统一到alloc_pages接口)
    - [4.4.2 alloc\_pages函数分配页](#442-alloc_pages函数分配页)
    - [4.4.3 伙伴系统的心脏\_\_alloc\_pages\_nodemask](#443-伙伴系统的心脏__alloc_pages_nodemask)
  - [4.5 \_\_free\_pages](#45-__free_pages)
...
由于内存域修饰符的解释方式不是那么直观, 表3-7给出了该函数结果的一个例子, 其中DMA和DMA32内存域相同. 假定在下文中没有设置\_\_GFP\_MOVABLE修饰符.
