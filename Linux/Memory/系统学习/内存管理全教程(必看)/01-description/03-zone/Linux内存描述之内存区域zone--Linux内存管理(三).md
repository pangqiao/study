...
- [前景回顾](#前景回顾)
  ...
  - [用pd_data_t描述内存节点node](#用pd_data_t描述内存节点node)
  - [今日内容(内存管理域zone)](#今日内容内存管理域zone)
- [为什么要将内存node分成不同的区域zone](#为什么要将内存node分成不同的区域zone)
- [内存管理区类型zone_type](#内存管理区类型zone_type)
  - [内存区域类型zone_type](#内存区域类型zone_type)
  - [不同的内存区域的作用](#不同的内存区域的作用)
...
我们会发现在**UMA结构**下, 数组大小**MAX\_ZONELISTS = 1**,因为只有一个内存结点,zonelist中只会存储一个`ZONELIST_FALLBACK`类型的结构,但是NUMA下需要多余的ZONELIST\_NOFALLBACK用以表示**当前结点**的信息
