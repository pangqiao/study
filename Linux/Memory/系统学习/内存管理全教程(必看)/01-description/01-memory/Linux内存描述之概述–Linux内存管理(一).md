...
- 首先, **内存被划分为结点**. 每个节点关联到系统中的**一个处理器**, 内核中表示为`pg_data_t`的实例. 系统中每个节点被链接到一个以NULL结尾的`pgdat_list`**链表**中<而其中的每个节点利用`pg_data_tnode_next`字段链接到下一节．而对于UMA结构,只使用了一个成为contig\_page\_data的静态pg\_data\_t结构.
