...
可以看到其异于通常的伙伴管理算法，内存页面是由最高阶开始进行查找的，而查找的迁移类型是根据fallbacks备选类型中进行遍历获得并止于MIGRATE_RESERVE类型。由此获得的阶号和迁移类型查找`zone->free_area[]->free_list[]`空闲页面管理链表，如果查找到的话，则将其摘除，否则进入下一类型查找，最后所有类型都查找不到的时候，才会降阶查找。
