...
- [7. 实现原理](#7-实现原理)
  ...
  - [7.2. 预先分配永久巨型页](#72-预先分配永久巨型页)
    ...
    - [HugePage 分配器初始化](#hugepage-分配器初始化)
    - [hugetlbfs 文件系统](#hugetlbfs-文件系统)
    - [虚拟内存与物理内存映射](#虚拟内存与物理内存映射)
    - [7.2.2. 写 nr_hugepages 文件](#722-写-nr_hugepages-文件)
  - [7.3. 挂载hugetlbfs文件](#73-挂载hugetlbfs文件)
  - [7.4. 创建文件](#74-创建文件)
...
创建内存映射. 内存映射, 简而言之就是将**内核空间**的**一段内存区域**映射到**用户空间**. 映射成功后, 用户对**这段内存区域的修改**可以直接反映到**内核空间**, 相反, 内核空间对这段区域的修改也直接反映用户空间. 那么对于内核空间与用户空间两者之间需要大量数据传输等操作的话效率是非常高的. 当然, 也可以将内核空间的一段内存区域同时映射到多个进程, 这样还可以实现**进程间的共享内存通信**.
...
```cpp
hugetlb_init()
 ├─ hugetlb_add_hstate(HUGETLB_PAGE_ORDER); // 添加 hstate
...
; // page->lru 初始化
 ...
 │  │     └─ put_page(page); // 会调用上面的 destructor
 │  │        └─ __put_page(page); 
 ...
 │  └─ hugetlb_vmemmap_init(h);
 ├─ gather_bootmem_prealloc(); // 处理 bootmem 的大页
...
为系统准备好空闲的大内存页后, 现在来了解下怎样分配大内存页. 要申请大内存页, 必须使用 mmap 系统调用把虚拟内存映射到 hugetlbfs 文件系统中的文件中.
...
从上面的代码可以发现, hugetlbfs 文件的 mmap 方法被设置为 `hugetlbfs_file_mmap` 函数.

而 `hugetlbfs_file_mmap` 函数最主要的工作就是给**虚拟内存分区**对象的 `vm_flags` 字段添加 `VM_HUGETLB` 标志位, 如下代码:
...
> 注: 这里仅仅处理了从 bootmem 分配的 huge page, 即 huge page size 的 order 大于页分配器最大 order

而如果巨型页长度小于或等于页分配器支持的**最大阶数**, 那么在**巨型页子系统初始化的时候**从**页分配器**预先分配永久巨型页, 其代码如下:
...
针对**每个巨型页池**, 如果巨型页长度**小于或等于**页分配器支持的**最大阶数**, 那么从**页分配器**分配永久巨型页, 添加到巨型页池中.

### 7.2.2. 写 nr_hugepages 文件
