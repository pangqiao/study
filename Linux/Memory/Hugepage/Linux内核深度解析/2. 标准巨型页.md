...
- [4. 总的 HugePages](#4-总的-hugepages)
  ...
  - [HugePages超发](#hugepages超发)
  - [参数之间的关系](#参数之间的关系)
  - [在 /sys/kernel/mm/hugepages/ 的接口](#在-syskernelmmhugepages-的接口)
- [NUMA node 的 HugePages](#numa-node-的-hugepages)
  - [单个NUMA node的HugePages的/sys接口](#单个numa-node的hugepages的sys接口)
  - [单个NUMA node的HugePages的查看](#单个numa-node的hugepages的查看)
  - [单个NUMA node的HugePages的分配](#单个numa-node的hugepages的分配)
- [代码示例](#代码示例)
- [5. 使用方法](#5-使用方法)
  - [5.1. 创建内存映射](#51-创建内存映射)
    - [5.1.1. 匿名映射](#511-匿名映射)
    - [5.1.2. 基于文件的映射](#512-基于文件的映射)
      - [5.1.2.1. 挂载 hugetlbfs](#5121-挂载-hugetlbfs)
      - [5.1.2.2. 创建文件和创建内存映射](#5122-创建文件和创建内存映射)
  - [5.2. hugetlbfs 库](#52-hugetlbfs-库)
- [6. 实现原理](#6-实现原理)
  - [6.1. 巨型页池 struct hstate](#61-巨型页池-struct-hstate)
  - [6.2. 预先分配永久巨型页](#62-预先分配永久巨型页)
    - [6.2.1. 内核参数 hugepages](#621-内核参数-hugepages)
    - [6.2.2. 写 nr_hugepages 文件](#622-写-nr_hugepages-文件)
  - [6.3. 挂载hugetlbfs文件](#63-挂载hugetlbfs文件)
  - [6.4. 创建文件](#64-创建文件)
  - [6.5. 创建内存映射](#65-创建内存映射)
    - [6.5.1. 申请预留巨型页](#651-申请预留巨型页)
  - [6.6. 分配和映射到巨型页](#66-分配和映射到巨型页)
  - [6.7. 写时复制](#67-写时复制)
