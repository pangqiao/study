...
- [总的 HugePages](#总的-hugepages)
  ...
  - [总的HugePages的分配](#总的hugepages的分配)
  - [HugePages 分配策略](#hugepages-分配策略)
- [1. 使用方法](#1-使用方法)
...
* 默认情况下系统所有NUMA node平均分配所有HugePages。
除非NUMA node本身没有足够的内存来生成HugePages，那么将由另外一个生成。
指定NUMA node分配HugePages，需要配合numactl -m一起使用。
