...
- [4. 总的 HugePages](#4-总的-hugepages)
  - [4.1. 总的 HugePages 的查看](#41-总的-hugepages-的查看)
  - [4.2. /proc/sys/vm 的接口](#42-procsysvm-的接口)
    - [4.2.1. 设置总的 HugePages](#421-设置总的-hugepages)
    - [4.2.2. HugePages 分配策略](#422-hugepages-分配策略)
    - [4.2.3. 设置超发的 HugePages](#423-设置超发的-hugepages)
  - [4.3. 参数之间的关系](#43-参数之间的关系)
  - [4.4. 在 /sys/kernel/mm/hugepages/ 的接口](#44-在-syskernelmmhugepages-的接口)
- [5. NUMA node 的 HugePages](#5-numa-node-的-hugepages)
  - [5.1. 单个NUMA node的HugePages的/sys接口](#51-单个numa-node的hugepages的sys接口)
  - [5.2. 单个NUMA node的HugePages的查看](#52-单个numa-node的hugepages的查看)
  - [5.3. 单个NUMA node的HugePages的分配](#53-单个numa-node的hugepages的分配)
- [6. 使用方法](#6-使用方法)
  - [6.1. 创建内存映射](#61-创建内存映射)
    - [6.1.1. 匿名映射](#611-匿名映射)
    - [6.1.2. 基于文件的映射](#612-基于文件的映射)
      - [6.1.2.1. 挂载 hugetlbfs](#6121-挂载-hugetlbfs)
      - [6.1.2.2. 创建文件和创建内存映射](#6122-创建文件和创建内存映射)
  - [6.2. hugetlbfs 库](#62-hugetlbfs-库)
- [7. 代码示例](#7-代码示例)
- [8. 实现原理](#8-实现原理)
...
## 4.4. 在 /sys/kernel/mm/hugepages/ 的接口

如果系统支持**多种大小**的 HugePages, 就会有多个 `/sys/kernel/mm/hugepages/hugepages-${huge_page_size}` 目录.
...
单个 NUMA node 的 HugePages 的 `/sys` 接口, 可以提供更加准确的配置和查看功能.
...
## 5.1. 单个NUMA node的HugePages的/sys接口
...
## 5.3. 单个NUMA node的HugePages的分配

`/sys/devices/system/node/node${numa_node_id}/hugepages/hugepages-${huge_page_size}/nr_hugepages` 用于设置**单个NUMA node**的 HugePages 内存信息. 可读可写.

```
# ll /sys/devices/system/node/node0/hugepages/hugepages-2048kB/
-r--r--r-- 1 root root 4096 Feb 27 19:47 free_hugepages
-rw-r--r-- 1 root root 4096 Feb 27 19:47 nr_hugepages
-r--r--r-- 1 root root 4096 Feb 27 19:47 surplus_hugepages
```
