...
- [7. 实现原理](#7-实现原理)
  - [7.1. 巨型页池 struct hstate](#71-巨型页池-struct-hstate)
  - [7.2. 预先分配永久巨型页](#72-预先分配永久巨型页)
    - [7.2.1. 内核参数 hugepages](#721-内核参数-hugepages)
    - [7.2.2. HugePage 分配器初始化](#722-hugepage-分配器初始化)
    - [7.2.5. 写 nr_hugepages 文件](#725-写-nr_hugepages-文件)
  - [7.3. 挂载hugetlbfs文件系统](#73-挂载hugetlbfs文件系统)
  - [7.4. 创建文件](#74-创建文件)
  - [7.5. 创建内存映射](#75-创建内存映射)
    - [7.5.1. 申请预留巨型页](#751-申请预留巨型页)
  - [7.6. 分配和映射到巨型页](#76-分配和映射到巨型页)
  - [7.7. 写时复制](#77-写时复制)
...
如果 hugepages 是**第一个参数**或者**唯一参数**, 它隐式地(implicitly)指定要分配的**默认大小巨型页**的**数量**. 如果**默认大小巨型页的数量**是隐式指定的(上面的**第一个参数**方法), 那就**不能**通过 `hugepagesz,hugepages` 参数**覆写**.

> hugepages=256 hugepagesz=2M hugepages=512

这里隐式指定了默认大小巨型页是 256 个, 后面两个参数对默认大小页的设置会被忽略.
...
“`default_hugepagesz=<size>[kKmMgG]`” 用于选择**默认的巨型页长度**. 这个参数仅能出现一次.
...
在支持**多种 size 大页**的系统中, “`/proc/sys/vm/nr_hugepages`” 表示**当前系统中默认长度的永久巨型页的数量**.
...
创建**匿名的巨型页映射**, 代码参见 `tools/testing/selftests/vm/map_hugetlb.c`
...
参见文件 `hugepages.c` 或者 `tools/testing/selftests/vm/hugepage-shm.c`
...
创建**基于文件的巨型页映射**, 代码参见 `tools/testing/selftests/vm/hugepage-mmap.c`
...
## 7.1. 巨型页池 struct hstate

内核使用**巨型页池**管理巨型页. 巨型页池的**数据结构**是结构体 `hstate`.
...
1. 会先解析 hugepagesz, 再解析 hugepages
2. 不同 size 的都会调用
...
第6～10行代码, 如果**前一个内核参数** “`hugepagesz=`” 指定的**巨型页长度**是**非法**的, 直接返回.

第16行和第17行代码, 如果前面**没有内核参数** “`hugepagesz=`”, 那么内核参数 “`hugepages=`” 指定的是**默认巨型页池**的永久巨型页的**数量**.

第18行和第19行代码, 如果前面有内核参数 “`hugepagesz=`” 指定**巨型页长度**, 那么内核参数 “`hugepages=`” 指定**该大小巨型页**对应的巨型页池的永久巨型页的**数量**.

第26行代码, 解析并保存内核参数 “`hugepagesz=`” 的值.

第29行和第30行代码, 如果前面有内核参数 “`hugepagesz=`” 指定巨型页长度, 并且**巨型页长度**超过**页分配器**支持的**最大阶数**, 那么需要从**引导内存分配器**分配巨型页. 如果**巨型页长度小于页分配器支持的最大阶数**, **巨型页子系统初始化**的时候从**页分配器**分配巨型页.

> 这里并**没有**针对巨型页长度order小于页分配器order(11, 2M)这种情况分配页面
...
```cpp
hugetlb_init()
 ├─ hugetlb_add_hstate(HUGETLB_PAGE_ORDER); // 添加 hstate, hugepagesz_setup 也会调用
 ...
 │  ├─ hugetlb_hstate_alloc_pages(h); // 分配大页
...
* 初始化**空闲大内存页链表** `hugepage_freelists`, 这个链表保存了系统中能够使用的大内存.
* 为系统**申请空闲的大内存**页, 并且保存到 `hugepage_freelists` 链表中.

我们再来分析下 `alloc_fresh_huge_page` 函数是怎么**申请大内存页**的, 其实现如下:
...
* 调用 `put_page` 函数**释放大内存页**, 其将会调用 `free_huge_page` 函数进行相关操作.
...
`free_huge_page` 函数主要调用 `enqueue_huge_page` 函数把**大内存页**添加到**空闲大内存页链表**中, 其实现如下:
...
第 34 行和第 45 行代码, 如果减小永久巨型页的数量, 处理如下.
...
> 关联: super_block -> hugetlbfs_sb_info -> hugepage_subpool | hstate
...
3) 虚拟内存区域的成员 `vm_ops` 指向巨型页特有的**虚拟内存操作集合** `hugetlb_vm_ops`.

4) 检查文件的偏移是不是巨型页长度的整数倍.

5) 调用函数 `hugetlb_reserve_pages()`, 向巨型页池**申请预留巨型页**.(并**不是真正申请**, **只是预留**)
...
1) 如果设置标志位 `VM_NORESERVE` 指定**不需要预留巨型页**, 直接返回.
...
`hugetlb_fault` 函数主要**对进程的页表进行填充**, 所以我们先来回顾一下 HugePages 对应的页表结构, 如下图:

![2022-03-08-10-25-17.png](./images/2022-03-08-10-25-17.png)

从上图可以看出, 使用 HugePages 后, **页中间目录项** 直接指向物理内存页. 所以, `hugetlb_fault` 函数主要就是对 **页中间目录项** 进行填充. 实现如下:
...
* 调用 `alloc_huge_page` 函数从**空闲大内存页链表** `hugepage_freelists` 中**申请一个大内存页**. 如果是**共享映射**, 那么把巨型页加入文件的**页缓存**, 以便和其他进程共享页.
* 通过**大内存页**的**物理地址**生成**页中间目录项的值**.
* 设置页中间目录项的值为上面生成的值.
* 如果第一步在页缓存中找到页, 映射是**私有的**, 并且执行写操作, 那么**执行写时复制**.
...
假设进程1创建了**私有**的**巨型页映射**, 然后进程1分叉生成进程2和进程3. 其中一个进程试图写巨型页的时候, 触发页错误异常, 巨型页的页错误处理函数 `hugetlb_fault` 调用函数 `hugetlb_cow` 以执行写时复制.

函数 `hugetlb_cow` 的执行过程如下.

1) 如果**只有一个虚拟页**映射到该**物理页**, 并且是匿名映射, 那么不需要复制, 直接修改页表项设置可写.
