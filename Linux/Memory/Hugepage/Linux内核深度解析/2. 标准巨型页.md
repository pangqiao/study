...
hugetlb_init()
 ...
 │  │     │  ├─ alloc_buddy_huge_page(); // 本质是调用__alloc_pages
 │  │     │  └─ prep_new_huge_page(h, page, page_to_nide(page)); // 所有大页都会调用, 进行初始化
 │  │     │     ├─ INIT_LIST_HEAD(&page->lru);
; // page->lru 初始化
 │  │     │     ├─ set_compound_page_dtor(page, HUGETLB_PAGE_DTOR); // 设置page[1].compound_dtor = HUGETLB_PAGE_DTOR
