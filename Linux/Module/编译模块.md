...
开发包里面放的是**头文件**和**Makefile文件**, 用RPM命令来查看下: 
...
原因是配置文件默认没有选择KVM, 所以需要手动make menuconfig, 选上KVM.
...
将 `arch/x86/kvm` 目录下生成kvm.ko和kvm-intel.ko生成的两个模块文件, 拷贝到 `/lib/modules/$(uname -r)/kernel/arch/x86/kvm` 目录下
