...
学会使用image文件后, 接下来就是, 如何可以生成image文件? 如果要推广自己的软件, 必须要制作自己的image文件.

这就需要用到Dockerfile文件. 它是一个文本文件, 用来配置image. Docker根据该文件通过docker build命令生成二进制的image文件.
