...
>注: 除非您修改了 Docker 守护进程的 `--registry-mirror` 参数 (见[官方链接](https://www.docker-cn.com/registry-mirror)), 否则您将需要完整地指定官方镜像的名称。例如，library/ubuntu、library/redis、library/nginx。
...
上面代码中，**docker image pull**是抓取 image 文件的命令。**library/hello-world**是 image 文件在仓库里面的位置，其中**library**是 image 文件所在的组，hello-world是 image 文件的名字。
