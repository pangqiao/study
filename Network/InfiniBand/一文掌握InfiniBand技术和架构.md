...
**HCA**是**Host Channel Adapter**的缩写，它是**连接内存控制器和TCA的桥梁**；**TCA**是**Target Channel Adapter**的缩写，它将**I/O设备（例如网卡、SCSI控制器**）的**数字信号**打包发送给**HCA**；**IB Link**包含了**连接HCA和TCA的光纤**以及**光纤交换机**、**路由器等整套设备**。在现阶段**一根光纤的传输速率是2.5Git/s**，支持**全双工模式**，并且可以把**多条光纤捆绑到一起工作**，目前的模式有x4、x12两种。
