...
时间是一年半左右。InfiniBand这一名称本身即说明了其无限的带宽发展前景。InfiniBand路线图设计的目的就是要保证**单个链路的带宽**能够保持在**大于PCIExpress(PCIe)总线数据速率(这是服务器内部总线标准！！！**)的水平。这样，系统就能够以其可产生的最快速度，在网络间移动数据，并且不会因出现因网络限制而导致的备份。这样，就可让 InfiniBand具备无限带宽。
