...
**InfiniBand**对**数据在传送中的处理**几乎照搬了**以太网协议**的全部思想。和TCP/IP协议集极其相似，InfiniBand就是分了**6个协议层**，从Transport层向下各层数据的封装都是由TCA和HCA自己完成的，不需要占用系统CPU的资源，交换机和路由器工作在 Network层。如果超越了Transport层，HCA会把任务交给系统处理，这一部分的工作需要操作系统的支持，但是对于TCA来说，它是不需要考虑这些的，只要把物理层的信号打包生成网络层可以识别的packet，它的工作就算完成了。
