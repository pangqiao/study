...
- [1. 类型大小](#1-类型大小)
  - [1.1. 可确定大小类型和动态大小类型](#11-可确定大小类型和动态大小类型)
  - [1.2. 零大小类型](#12-零大小类型)
  - [1.3. 底类型](#13-底类型)
- [2. 类型推导](#2-类型推导)
  - [2.1. Turbofish 操作符](#21-turbofish-操作符)
  - [2.2. 类型推导的不足](#22-类型推导的不足)
...
**底类型**(`Bottom Type`)是源自**类型理论**的术语, 它其实是第2章介绍过的 **never 类型**. 它的特点是:

* 没有值
* 是其他任意类型的子类型

如果说 **ZST 类型**表示“**空**”的话, 那么**底类型**就表示“**无**”. 底类型**无值**, 而且它可以等价于**任意类型**, 有点无中生有之意.
...
上面两种方式为泛型函数标注类型. `3_14` 这种标注类型 (`::<>`) 的形式叫作 **turbofish 操作符**.
