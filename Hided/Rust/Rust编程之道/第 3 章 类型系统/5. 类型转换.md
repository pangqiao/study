...
* match x.as_ref（），String类型提供了as_ref方法来返回一个&str类似，该方法定义于AsRef trait中。· match x.borrow（），方法borrow定义于Borrow trait中，行为和AsRef类型一样。需要use std：：borrow：：Borrow。· match&*x，使用“解引用”操作符，将String转换为str，然后再用“引用”操作符转为&str。· match&x[..]，这是因为String类型的index操作可以返回&str类型。
