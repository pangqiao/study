...
**as** 转换还可以用于**类型和子类型之间的转换**. Rust中**没有标准定义中的子类型**, 比如**结构体继承之类**, 但是**生命周期标记**可看作**子类型**. 比如 `&'static str` 类型是 `&'a str` **类型**的**子类型**, 因为二者的**生命周期标记不同**, `'a` 和`'static` 都是**生命周期标记**, 其中 `'a` 是**泛型标记**, 是 `&str` 的**通用形式**, 而 `'static` 则是**特指静态生命周期**的 `&str` 字符串. 所以, 通过as操作符转换可以将&'static str类型转为&'a str类型, 如代码清单3-67所示.
