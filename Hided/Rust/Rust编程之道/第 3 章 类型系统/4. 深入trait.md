...
impl Add for u32 {
	type Output = u32;
	fn add3_23(self, other: u32) -> u32 { self + other }
}
```
...
**Add trait** 和 **u32** 都**不是**在**当前 crate** 中定义的, 而是定义于标准库中的.
...
假如此时需要多加一个功能, 为不影响之前的代码, 可以使用 trait 继承扩展功能:
...
**为泛型 T 实现**了 trait Paginate. 意思是, **为所有拥有 Page 和 PerPage 行为的类型实现 trait Paginate**.
...
> 注意, 对泛型约束的时候，`Add＜T，Output=T＞` 通过类型参数确定了**关联类型Output**也是 T，也可以省略类型参数 T，直接写为 `Add＜Output=T＞`
