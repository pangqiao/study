...
TraitObject 包括两个指针: **data 指针**和 **vtable 指针**. 以 `implMyTrait for T` 为例, data 指针指向 trait 对象保存的类型数据 T, vtable 指针指向包含为 T 实现的 MyTrait 的 `Vtable(Virtual Table)`, 该名称来源于C++, 所以可以称之为虚表. 虚表的本质是一个结构体, 包含了析构函数、大小、对齐和方法等信息.

TraitObject 的结构如图:

![2021-10-27-22-23-26.png](./images/2021-10-27-22-23-26.png)

在**编译期**, 编译器只知道 TraitObject 包含指针的信息, 并且指针的大小也是确定的, 并不知道要调用哪个方法. 在运行期, 当有 `trait_object.method()` 方法被调用时, **TraitObject 会根据虚表指针从虚表中查出正确的指针, 然后再进行动态调用**. 这也是将 trait 对象称为动态分发的原因.
