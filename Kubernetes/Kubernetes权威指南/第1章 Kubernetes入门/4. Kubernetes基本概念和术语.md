...
- [概述](#概述)
  - [apiVersion属性和核心](#apiversion属性和核心)
  - [资源对象新属性的支持方法](#资源对象新属性的支持方法)
- [整体架构](#整体架构)
  - [Master](#master)
  - [Node](#node)
- [Pod](#pod)
...
- [9 Service](#9-service)
  - [9.1 概述](#91-概述)
    - [9.1.1 Pod、RC与Service的关系](#911-pod-rc与service的关系)
    - [9.1.2 Kubernetes提供的微服务网络架构](#912-kubernetes提供的微服务网络架构)
    - [9.1.3 多Pod副本访问](#913-多pod副本访问)
    - [9.1.4 Kubernetes的负载均衡器: kube\-proxy(Node上), 转发对Service的请求到某个Pod](#914-kubernetes的负载均衡器-kube-proxynode上-转发对service的请求到某个pod)
    - [9.1.5 Cluster IP](#915-cluster-ip)
    - [9.1.6 定义举例](#916-定义举例)
    - [9.1.7 Service的多端口问题](#917-service的多端口问题)
  - [9.2 Kubernetes的服务发现机制](#92-kubernetes的服务发现机制)
    - [9.2.1 Linux环境变量](#921-linux环境变量)
    - [9.2.2 DNS系统](#922-dns系统)
  - [9.3 外部系统访问Service的问题](#93-外部系统访问service的问题)
    - [9.3.1 3种IP地址](#931-3种ip地址)
    - [9.3.2 外部应用如何访问Service](#932-外部应用如何访问service)
      - [9.3.2.1 NodePort: 外部访问NodeIP \+ NodePort](#9321-nodeport-外部访问nodeip-nodeport)
      - [9.3.2.2 负载均衡问题: 外部访问负载均衡器的IP(由负载均衡器转发到Node)](#9322-负载均衡问题-外部访问负载均衡器的ip由负载均衡器转发到node)
- [10 Job: 批处理任务](#10-job-批处理任务)
- [11 Volume: Pod中多个容器共享](#11-volume-pod中多个容器共享)
  - [11.1 Volume类型](#111-volume类型)
    - [11.1.1 emptyDir: 临时空间](#1111-emptydir-临时空间)
    - [11.1.2 hostPath: 宿主机文件或目录](#1112-hostpath-宿主机文件或目录)
    - [11.1.3 gcePersistentDisk: Google公有云PD](#1113-gcepersistentdisk-google公有云pd)
    - [11.1.4 awsElasticBlockStore: AWS的EBS Volume](#1114-awselasticblockstore-aws的ebs-volume)
    - [11.1.5 NFS: NFS网络文件系统](#1115-nfs-nfs网络文件系统)
    - [11.1.6 其他类型的Volume](#1116-其他类型的volume)
- [12 Persistent Volume(PV): 独立于Pod之外](#12-persistent-volumepv-独立于pod之外)
- [13 Namespace: 多租户隔离](#13-namespace-多租户隔离)
...
尽管Pod中可以包含多个容器，但是作为**最小调度单元**，它应该尽可能地保持“小”，即**通常**只应该包含一个主容器，以及必要的辅助型容器（sidecar）
...
### 9.1.4 Kubernetes的负载均衡器: kube\-proxy(Node上), 转发对Service的请求到某个Pod
...
### 9.3.2 外部应用如何访问Service

基本明白：Service的**Cluster IP**属于Kubernetes**集群内部的地址！！！**，无法在集群外部直接使用这个地址。那么矛盾来了：实际上在我们开发的业务系统中肯定多少有一部分**服务**是要提供给Kubernetes集群**外部的应用或者用户**来使用的，典型的例子就是Web端的服务模块，比如上面的tomcat\-service，那么用户怎么访问它？

#### 9.3.2.1 NodePort: 外部访问NodeIP \+ NodePort
...
#### 9.3.2.2 负载均衡问题: 外部访问负载均衡器的IP(由负载均衡器转发到Node)
...
### 11.1.3 gcePersistentDisk: Google公有云PD
...
### 11.1.4 awsElasticBlockStore: AWS的EBS Volume
...
# 12 Persistent Volume(PV): 独立于Pod之外
