...
如上图所示，目标代码的**offset值**扩展为**64位**，在Call\-gate的高8字节Bit 44到Bit 40位**共5位**必须**设置为0值**，代表一个**无效的descriptor类型**。在long mode下段描述符（Code和Data）仍然是8字节宽度。由于Call-gate存放在GDT或LDT中，占据了2个segment descriptor的空间（16字节）。
...
在x64体系就变得安全得多，利用TSS进行任务切换的机制已经被废除，包括Taskgate机制。
...
在long-mode下TR不能使用隐式的加载（不能使用任务切换机制），因此必须使用显式的加载。
...
这个tss\_sel不能为Null selector，选择子的TI必须为0（指示在GDT内）。ltr指令只能在0级权限里执行，这个tss_sel.RPL被忽略不起作用。
...
处理器会比较栈内的CS.selector.RPL和SS.selector.RPL是否相等，如果不相等会产生#GP异常，当所有检查都通过后，处理器会依次POP出EIP、CS、ESP与SS值，转入执行新代码。

③ 当CPL>CS.Selector.RPL时，意味着目标代码是高权限代码，将产生#GP异常，不能使用retf指令切换到高权限上。

![config](./images/52.png)

只能通过gate形式进入以及使用TSS发起任务切换到高权限代码。
...
② SS selector是否属于03H（即：3级权限下的Null selector），如果为3则产生#GP异常（详见前面关于Null selector所述）。如果切换到相应的0级、1级或2级，那么0、1或2将是允许的Null selector。
...
在前面的10.5.4.4节5中我们看到了伪造一个任务嵌套环境使用iret指令进行任务切换，iret指令使用TSS selector从TSS段中加载CS和SS寄存器。在这一节里我们使用iret指令进行正常的中断返回调用。使用iret指令返回实际上也属于伪造一个中断handler返回环境。
...
在前面10.5.4.4节10下的使用retf指令从0级代码返回到3级权限的例子里，也可以改用iret指令来代替。区别是多压入一个EFLAGS寄存器。
...
sysenter指令可以执行于任何权限中，但是不要企图在非3级（CPL！=3）权限下使用sysenter指令，因为sysexit指令会强制返回到3级权限里（当然：在不使用sysexit指令返回时可以这么做！），这里会造成严重错误。
...
比较有意思的是，在IA32\_SYSENTER\_CS寄存器里只要不是Null selector，其他值都可以。
...
胡乱给IA32\_SYSENTER\_CS设一个值，要小心处理。在以后的代码执行流里如果出现了stack切换时，一个混乱的值会引发#GP异常的产生。
...
sysexit只能执行在0级权限的代码里，处理器同样对CS和SS进行强制的设置。
...
当SS或其他数据段寄存器内的属性标志E=0时，它属于Expand-up段。对于一个Expand-up类型的Stack或Data段来说，它的段内有效区域并不依赖于B标志位。
...
② Stack段只能是Expand-up类型，Expand-down类型是无效的。
...
这个调整的操作是RSP&FFFFFFFF\_FFFFFFF0h，结果是16字节边界对齐。对于一般的push和call调用，处理器并不进行对齐调整。
...
D/B标志位使用于Stack段时，被作为B标志位，如前面的Stack结构中所述。当使用于其他的数据段时，D/B标志位也同样被用于Expand\-down类型的段才有意义，它与G标志位结合起来对Expand\-down段的limit值进行定义，如前面对Expand-down类型的段所述。
...
这个标志位对于DS、ES、FS和GS寄存器来说并无影响，被忽略。当被加载到SS寄存器时，处理器会检查W标志位是否为1（可写的），否则会产生#GP异常。可是在64位模式下的非3级权限下可以加载Null selector到SS寄存器里，这时所有的段描述符属性都被忽略。
...
FS段和GS段完整的64位Base地址需要在相应的MSR里设置，FS的基地址寄存器是IA32\_FS\_BASE，地址在C0000100H。GS的基地址寄存器是IA32\_GS\_BASE，地址在C0000101H上，这些值必须在0级权限下使用wrmsr指令进行设置。
...
在上图里使用一个Call-gate从3级权限切换到0级权限里，处理器从当前的TSS段里（由TR的Base域获取了TSS段位置），读取属于0级权限的SS和ESP值（在TSS段里的SS0和ESP0域），分别加载到SS寄存器和ESP寄存器里，SS和ESP的原值被临时保存起来。

现在处理器加载新的SS和ESP值后，已经切换到0级的stack里，在当前的Stack（0级的Stack）里依次压入调用者的SS、ESP、CS，以及EIP值，然后转去执行0级代码。
...
3级的栈指针值（SS和ESP）放在两个地方：要么存放在当前的SS寄存器和ESP寄存器里，要么在切换到高权限的stack时被压入在高权限的stack里。
...
使用32位的call\-gate指针（16：32形式），从3级的compatibility模式进入到0级的64位模式Call\-gate服务例程。当前的stack将从compatibiltiy切换到64位模式，如下图所示。
