...
- [配置定时器模式: 写LVTT](#配置定时器模式-写lvtt)
- [4. 设置定时器: 写TMICT或TSC-Deadline](#4-设置定时器-写tmict或tsc-deadline)
...
- [5. timer到期](#5-timer到期)
  - [5.1. preemption timer到期](#51-preemption-timer到期)
    - [5.1.1. 整体流程](#511-整体流程)
    - [5.1.2. handle_preemption_timer(): 到期vmexit处理函数](#512-handle_preemption_timer-到期vmexit处理函数)
    - [5.1.3. preemption timer处理](#513-preemption-timer处理)
  - [5.2. hrtimer到期的外部中断](#52-hrtimer到期的外部中断)
    - [5.2.1. 整体流程](#521-整体流程)
    - [5.2.2. apic_timer_fn(): hrtimer到期的响应函数](#522-apic_timer_fn-hrtimer到期的响应函数)
...
1. 虚拟机设置APIC timer, 主要包括两个, 写TMICT寄存器(用于one-shot/period模式); 写TSCDEADLINE MSR寄存器(用于tscdeadline模式). 从而发生vm-exit, 进入KVM中(第一步肯定是设置LVTT寄存器, 见architecture的apic timer部分)
...
### 5.1.2. handle_preemption_timer(): 到期vmexit处理函数
...
### 5.2.2. apic_timer_fn(): hrtimer到期的响应函数
