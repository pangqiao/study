...
- [1.2. trace](#12-trace)
  - [1.2.1. 使用ftrace](#121-使用ftrace)
    - [tscdeadline](#tscdeadline)
...
2. 在KVM中, 模拟虚拟机的APIC设置, 先设置这几个寄存器, 然后启用lapic timer定时器(优先使用hv timer`<preemption timer>`, 其次使用hrtimer)作为中断源, 最后进入Guest模式
3. timer到期会产生中断, 发生vm-exit, 进入KVM中
* preemption timer导致的vm-exit
* hrtimer到期会产生外部中断, 导致虚拟机vm-exit
...
注: 也可以 `echo kvm_set_lapic_tscdeadline_msr > set_graph_function`用来单独追踪
...
```
# taskset -c 65 ./x86-run x86/vmexit.flat
/usr/local/bin/qemu-system-x86_64 -cpu host,+x2apic --no-reboot -nodefaults -device pc-testdev -device isa-debug-exit,iobase=0xf4,iosize=0x4 -vnc none -serial stdio -device pci-testdev -machine accel=kvm -kernel x86/vmexit.flat # -initrd /tmp/tmp.fXRBt9HyKB
enabling apic
paging enabled
cr0 = 80010011
cr3 = 61e000
...
```cpp
 ...
 65)   0.127 us    |        kvm_can_use_hv_timer [kvm]();
 65)               |        vmx_set_hv_timer [kvm_intel]() {
 65)   0.126 us    |          kvm_read_l1_tsc [kvm]();
 65)   0.376 us    |        }
 65)               |        apic_timer_expired [kvm]() {
 65)   0.123 us    |          kvm_get_running_vcpu [kvm]();
 ...
 65)   0.124 us    |          vmx_cancel_hv_timer [kvm_intel]();
 65)   0.367 us    |        }
 65)   2.926 us    |      }
 65)   3.161 us    |    }
 65)   3.412 us    |  }
