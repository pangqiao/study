...
**一条总线**对应一个 `kvm_io_bus` 结构体**, 如**ISA总线**、**PCI总线**.
...
`struct vcpu_vmx`代表**vcpu的一个运行环境**, 这个和vcpu是一对一的.

`loaded_vmcs` 指向当前 vcpu 使用的 vmcs. 对于非嵌套(即L1)虚拟机, `loaded_vmcs` 一直指向`vmcs01`, 对于嵌套(L2), 指向另外的vmcs.
