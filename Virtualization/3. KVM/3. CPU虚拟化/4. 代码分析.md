...
在 x86体系架构中, **KVM结构体**的**初始化**任务在`kvm_arch_init_vm`函数(`arch/x86/kvm/x86.c`)中进行, 进行了**分配内存**、**初始化设备列表**、设置**中断管理**和**初始化 tsc 的spin\_lock的功能**. 

在**完成之后**, 将执行**硬件初始化**工作, 该部分硬件初始化工作通过调用**on\_each\_cpu宏**, 将在**每个物理CPU**上执行**同样的操作**. 该操作主要是尝试将**所有的CPU**切换入**vitualize模式**(使用`vmxon`指令), 并且设置好**时钟**等信息, 这个过程通过 `kvm_arch_hardware_enable` 函数完成. 
