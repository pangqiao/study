...
VPID 用来为**每个虚拟处理器**提供-一个 ID 值。在第4章开头我们已经了解过“虚拟处理器”的概念，每次VM-entry与VM-exit都相当于进行- - 次虚拟处理器的切换。

一个 VM (虚拟机)就代表着-一个虚拟处理器。在每次VM切换时(在一个逻辑处理器下可能管理多个VM实例)，VPID用来标识该VM (一个VPID对应一个VM)。从另一个角度看，VPID用来标识VM在处理器中的cache域。

当 secondary procesor-based VM-execution control 字段的“enable VPID”为 1 时, VMM 需要在 virtual-processor identifier 字段提供一个 16 位的非 0 值作为 VPID(参见 3.5.18 节与 4.4.1.3 节)。VMM应该为每个 VM 准备独立的 VPID 值，用来隔离每个 VM 的 cache 域。也就是每个VM应该有自己的 VPID 值，用来标识自己的 cache 空间。
