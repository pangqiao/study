...
DMA 全称 Direct Memory Access, CPU访问外设内存很慢, 如果由CPU给外设大量搬运数据, CPU会大量空转等待搬运数据完成, 所以发明出DMA engine, 把**搬运数据**的任务由 **DMA engine** 来完成, **CPU只要**告诉 DMA engine 从**什么地方开始搬运多大数据**就行了, 然后就可能干其它有意义的工作, DMA engine搬运完数据就打断CPU说搬运完了, 接着搬运哪的数据手动多大.
