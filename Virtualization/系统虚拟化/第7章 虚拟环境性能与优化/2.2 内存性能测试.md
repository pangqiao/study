...
这里实验的结果数据，由于**限制了CPU和内存等资源**，并**不表示机器环境的绝对性能**，而是注重虚拟化环境与原生环境在同等资源条件下的性能对比。

从图10\-5可以看出，KVM虚拟化中内存的访问带宽与原生系统相比都是比较接近的。Copy操作甚至客户机表现还略微（可以忽略不计）好于原生系统，Scale操作为原生系统的95.93%，Add操作为94.72%，Triad操作为96.52%。这主要归功于**硬件EPT支持**。
