...
- 当使用UDP功能时，Iperf允许用户自定义数据包大小，并最终提供一个数据包吞吐量值和丢包值。

另外，有一个项目叫Iperf3（项目主页为 http://code.google.com/p/iperf ），它完全重新实现了Iperf，其目的是使用更小、更简单的源代码来实现相同的功能，同时也开发了可用于其他程序的一个函数库。
