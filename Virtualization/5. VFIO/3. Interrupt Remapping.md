...
- [MSI](#msi)
- [no interrupt remapping](#no-interrupt-remapping)
...
在 Intel 架构上, 是通过 DMA 写物理地址0x0FEE_XXXX来产生 MSI 中断, PCI config space 中有 MSI Address 和 Data 寄存器, 驱动配置这两个寄存器, Address 寄存器中有 Destination ID, 表示 Local APIC ID, Address 寄存器所有字段组合起来就是 x0FEE_XXXX, Data 寄存器有 vector 号, 表示中断号.
