...
VFIO代码(QEMU部分)主要有三个结构体: VFIODevice, VFIOGroup,VFIOContianer. 其中 VFIODevice 表示VFIO设备, VFIOGroup表示Group, 一个 Group 可以包含多个设备, VFIOContainer 表示整个地址空间.

QEMU分别通过三个结构体的文件描述符fd访问内核中的信息.通过访问设备的文件描述符device->fd可以获取到访问到设备的资源(配置空间,BAR空间,中断).通过访问Group的文件描述符/dev/vfio/<groupid>可以获取group状态或对group进行设置.通过访问Container的文件描述符/dev/vfio/vfio将group与container关联,同时与iommu_driver联系出来,这样通过container可以进行IOMMU相关的操作.
...
* Container 文件描述符
    * 打开 `/dev/vfio/vfio` 字符设备可得
* IOMMU group 文件描述符
    * 打开 `/dev/vfio/groupid` 文件可得(详见后文)
* Device 文件描述符
