...
* 接口层将服务抽象为 IOCTL 命令, 规化操作流程, 定义通用数据结构, 与用户态交互. 实现层完成承诺的服务. 据此, 可在用户态实现支持 DMA 操作的高性能驱动. 在虚拟化场景中, 亦可借此完全在用户态实现 device passthrough.

* 实现层又分为**设备实现层**与 **IOMMU 实现层**. 当前 VFIO 仅支持 PCI 设备. IOMMU 实现层则有 x86 与 PowerPC 其它种类硬件及 IOMMU 的支持.
