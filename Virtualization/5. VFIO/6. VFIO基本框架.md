...
* VFIO Interface 作为接口层, 用于向应用层导出接口,QEMU 等用户程序可以通过相应的ioctl与VFIO进行交互;

* vfio_iommu 是对底层 iommu driver 的封装, 用来向上提供 IOMMU 的功能, iommu driver 接口, 做DMA重映射和中断重映射;

* iommu driver 是物理硬件IOMMU的驱动实现，如 Intel 和 ADM 的 IOMMU
