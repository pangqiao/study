...
再来看 vfio group, **每个 vfio group** 都是和一个 **IOMMU Group** 相**对应**的:
...
vfio group 的 **dev** 会指向 `/dev/vfio/$GROUP` 对应的 **Device**, 和 `vfio_add_group_dev` 传入的 Device **无关**. 由于 vfio group 和 IOMMU Group 是一一对应关系, 一个 Group 下可以有多个 VFIO Device, vfio group 通过 `device_list` 链表引用这些 VFIO Device. VFIO Device 的定义如下:
