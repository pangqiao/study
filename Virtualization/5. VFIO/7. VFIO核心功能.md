...
VFIO 分为 **VFIO 核心模块**和 **VFIO 驱动模块**, **VFIO Group** 是由 **VFIO 驱动模块创建**的, 最常用的是 `vfio-pci` 驱动. VFIO 驱动是以**设备驱动**的形式实现, 它们会注册一个 Driver, 并在其 probe 函数中调用 `vfio_add_group_dev`, 并最终会调用 `device_create` 为 VFIO Group 创建一个 Device(从而也创建了 `/dev/vfio/$GROUP` 文件):
