...
回顾 VFIO Container的定义, 除了IOMMU Driver 以外, 还有一个 **iommu_data**:
...
在 VFIO_SET_IOMMU 的实现 `vfio_ioctl_set_iommu` 中, 通过调用 **IOMMU Driver** 的 **open** 方法获得了 **IOMMU Data**:
...
这一步完成后, 接着会对 Container 上已经挂载的 VFIO Group 调用 IOMMU Driver的 attach_group 方法:
...
其中 `group_list` 又是 struct vfio_group 构成的链表(此 vfio group 非彼vfio group, 前者定义在 `drivers/vfio/vfio_iommu_type1.c`, 后者定义在 `drivers/vfio/vfio.c`):
...
这里, 一个 struct vfio_group 和一个 vfio group 相对应, 同时也对应于一个IOMMU Group. 不同的 IOMMU Group 可以共享同一张 IOMMU 页表, 我们说这些 IOMMU Group 属于同一个 IOMMU Domain, 在这里 struct vfio_domain 就对应着 IOMMU Domain. 最后, 一个 container 中可以容纳若干 IOMMU Domain, 即可以同时管理多个 IOMMU 页表. `external_domain` 是由 VFIO 驱动管理的外部 IOMMU Domain, 可以暂时忽略, 分析 vfio-mdev 时会详细解释.
