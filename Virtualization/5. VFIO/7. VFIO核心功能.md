...
vfio container上的其余操作都会代理给其IOMMU Driver执行, 包括read、write、mmap和上述三个 ioctl 以外的ioctl.
...
上一节也说过(上面结构体也说明), container 会与 iommu_driver 联系起来, 这样通过 container 可以进行 IOMMU 相关的操作.
