...
* 对于由 **PCIe switch** 扩展出的 **PCI 桥及桥下设备**，在发送 DMA 请求时，Source Identifier 是 PCIe switch 的，这样的话该 PCI 桥及桥下所有设备都会使用 PCIe switch 的 Source Identifier 去定位Context Entry，找到的**页表也是同一个**，如果将这个PCI桥下的不同设备分给不同虚机，由于会使用同一份页表，这样会产生问题，针对这种情况，**当前PCI 桥及桥下的所有设备必须！！！分配给同一个虚机**，这就是 VFIO 中 group 的概念，下面会再讲到。
