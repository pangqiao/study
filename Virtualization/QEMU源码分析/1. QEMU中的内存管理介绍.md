...
“**memory\-backend**(HostMemoryBackend对象中的内存)”表示的host上真实内存资源由RAMBlock调用exec.c文件中的qemu_ram_alloc()函数，最终通过mmap映射到host上的一块虚拟内存。RAMBlock结构体中的uint8_t *host指向host上虚拟内存的起始值，ram_addr_t offset表示当前RAMBlock相对于RAMList（描述host虚拟内存的全局链表）的偏移量。也就是说ram_addr_t offset位于一个全局命名空间中，可以通过此offset偏移量定位某个RAMBlock。
