...
**RAMBlock结构体**中的**uint8\_t \*host**指向**host**上**虚拟内存的起始值**，ram_addr_t offset表示当前RAMBlock相对于RAMList（描述host虚拟内存的全局链表）的偏移量。也就是说ram_addr_t offset位于一个全局命名空间中，可以通过此offset偏移量定位某个RAMBlock。
