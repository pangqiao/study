...
* 将**新 client** 的(peer_id, 所有向量的 eventfd) 发给**新 client 自己**
...
`watch_vector_notifier()`. 如果 kvm 没有 irqfd 支持, 那就使用 ioeventfd. Linux 的 eventfd 机制使用了 `POLLIN` 通知方案, 所以仅仅设置了 read 函数(`ivshmem_vector_notify`).

基于 QEMU 的事件循环机制, 当该 ioeventfd 在 kvm 侧有 `eventfd_signal`, 那 qemu 就调用该 read 函数(`ivshmem_vector_notify`).
...
当 guest 写 config space 时候, 会调用 qemu 侧的 `ivshmem_write_config`
...
static void ivshmem_enable_irqfd(IVShmemState *s)
{
    PCIDevice *pdev = PCI_DEVICE(s);
    int i;
    // 当前设备的所有向量对应的 eventfd
    for (i = 0; i < s->peers[s->vm_id].nb_eventfds; i++) {
        Error *err = NULL;

        ivshmem_add_kvm_msi_virq(s, i, &err);
    }
    // 将设备的所有msix_vector对应的use_noifier,release_notifier, poll_notifier回调函数都进行记录
...
之后在虚拟机内通过触发 ivshmem 虚拟 PCI 设备的 DOORBELL 寄存器的写入, 虚拟机的 QEMU 进程便会通过 **DOORBELL 寄存器**中的 `peer_id` 和**中断向量号**来找到**相应的 eventfd**, 从而通知到对端的 QEMU 进程来**产生相应的 PCI 中断**.
