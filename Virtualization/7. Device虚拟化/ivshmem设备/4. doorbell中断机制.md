...
ivshmem_server_handle_new_conn(IvshmemServer *server)
{
    ...
    // 1. 0(协议版本号)
    // 2. peer id
    // 3. -1 并且 message 中带server->shm_fd(server的共享内存体fd)
...
第二. 设置了后续对 ivshmem server 的回调函数.
...
后续通过 `ivshmem_read` 接收 server 端发来的 posn 和 efd 信息, 再通过 `create_eventfd_chr_device` 创建 eventfd 字符设备.
