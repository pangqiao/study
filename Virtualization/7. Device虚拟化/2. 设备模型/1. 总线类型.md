...
其设置了 Object 基类的 unparent函数，这个回调函数在子类解除父类引用时被调用，另一个是 BusClass 的get_fw_dev_path，这是默认函数，如果子类没有重写这个函数，那么当查找固件路径的时候就会用这个函数
...
`device_set_realized` 中具现化总线的代码如下所示。
...
这样就会调用到**总线对象的具现化回调函数** `bus_set_realized`，
从而实现总线的具现化。