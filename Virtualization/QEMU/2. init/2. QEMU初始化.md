...
int main()
 ...
 |   |   |   └─ memory_region_do_init(mr, owner, name, size);           // 初始化, 就是一些赋值操作
 ...
 |   |   |   |   |   ├─ render_memory_region(&view, mr, int128_zero(), addre) // 根据 mr 生成 view
 ...
 |           |   |   |   |               |─ new_block->host = phys_mem_alloc(new_block->max_length, &new_block->mr->align, shared); // 在host上分配虚拟内存
