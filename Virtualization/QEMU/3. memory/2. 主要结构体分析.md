...
全局的AddressSpace, address_space_memory和address_space_io，一个是管理系统地址空间，一个是IO地址空间
...
所有的AddressSpace是通过一个**全局链表**`address_spaces`链接起来, 每个 AddressSpace 的`address_spaces_link`作为链表节点, 表头保存在全局的 AddressSpace 结构中.

### 2.1.1. AddressSpace与MemoryRegion

所有AddressSpace下面**root**及其**子树**形成了**一个虚拟机**的**物理地址！！！**, 也就是说一个AddressSpace对应多个MemoryRegion

见下面图

## 2.2. MemoryRegion: 虚拟机物理地址GPA到主机虚拟地址HVA
...
这个树就是由MemoryRegion构成, MemoryRegion负责管理host的内存，理论上是树状结构，但是实际上根据代码来看仅仅有两层，也就是说AddressSpace下面是MR root, 再往下就是子MR链表, 头是subregions, 子MR通过`subregions_link`链接.
...
对于系统地址空间的ram，会把刚才得到的subregion注册到系统中，父MR是**全局MR** `system_memory`, `subregions_link`是**链表节点**。
...
所有的RAMBlock通过**next字段**连接起来，表头保存在一个**全局的RAMList结构**中，但是根据代码来看，**原始MR分配**内存时分配的是**一整块block**，之所以这样做也许是为了扩展用吧！！RAMList中有个字段**mru_block指针**，指向**最近使用的block**，这样需要**遍历所有的block**时，先判断指定的block是否是`mru_block`，如果不是再进行遍历从而提高效率。
...
这个就用FlatView表示，**一个AddressSpace**对应**一个FlatView**。FlatView管理MR展开后得到的所有 FlatRange .
