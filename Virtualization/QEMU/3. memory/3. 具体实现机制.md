...
- [实际内存的分配GPA/HVA](#实际内存的分配gpahva)
  ...
  - [将整体mr(ram)划分为两个子MR(subregion)](#将整体mrram划分为两个子mrsubregion)
- [](#)
- [参考](#参考)
...
## 将整体mr(ram)划分为两个子MR(subregion)

此为**全局的MR**，根据`ram_below_4g`和`ram_above_4g`，**全局的MR**被划分为两部分，形成**两个子MR**。
