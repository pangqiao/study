...
- [实际内存的分配GPA/HVA](#实际内存的分配gpahva)
  - [分配全局RAM](#分配全局ram)
    - [memory_region_init_ram: 创建ramblock并从host上分配虚拟内存](#memory_region_init_ram-创建ramblock并从host上分配虚拟内存)
    - [vmstate_register_ram_global(): ramblock与mr关联](#vmstate_register_ram_global-ramblock与mr关联)
  - [划分为两个子MR](#划分为两个子mr)
- [](#)
- [参考](#参考)
...
### memory_region_init_ram: 创建ramblock并从host上分配虚拟内存
...
### vmstate_register_ram_global(): ramblock与mr关联
...
`vmstate_register_ram_global`这个函数则是负责将前面提到的ramlist中的**ramblock**和**memory region**的初始地址对应一下，将mr->name填充到ramblock的idstr里面，就是让二者有确定的对应关系，如此mr就有了物理内存使用。
