...
- [4. 实际内存的分配GPA/HVA](#4-实际内存的分配gpahva)
  ...
  - [4.2. 将整体mr(pc.ram)划分为两个子MR(subregion): 不再申请内存](#42-将整体mrpcram划分为两个子mrsubregion-不再申请内存)
- [5.](#5)
- [6. 参考](#6-参考)
...
## 4.2. 将整体mr(pc.ram)划分为两个子MR(subregion): 不再申请内存

在创建好了ram并且分配好了空间之后，创建了**两个mr alias**，**ram\_below\_4g**以及**ram\_above\_4g**，这两个mr分别指向**ram的低4g**以及**高4g空间**，这两个alias是挂在**根system\_memory mr下面**的, 即高低端内存（也不一定是32bit机器）
...
对于 `x86_64`, 需要给system io留0.5G内存, 检查虚拟机内存是否大于4G(ram_size是否大于3.5G), 
...
lowmem = machine->ram_size >= 0xe0000000 ? 0xc0000000:0xe0000000;
pcms->below_4g_mem_size = lowmem;
pcms->above_4g_mem_size = machine->ram_size - lowmem;
```

其中 0xc0000000 为 3G, 0xe0000000 为 3.5G 
