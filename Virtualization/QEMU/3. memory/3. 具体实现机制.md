...
前面注册listener也好，或是初始化addressspace也好，实际上均**没有对应的物理内存**。顺着main函数往下走，会调用到`machine—>init`，实际上对应于`pc_init1`函数，在该函数中有`pc_memory_init()`函数对实际的内存做了分配。我们直接从`pc_memory_init()`函数开始
