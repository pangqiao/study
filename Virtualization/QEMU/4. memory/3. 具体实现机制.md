...
- [5. MEMORY_LISTENER_UPDATE_REGION(): 新FlatRange的更新](#5-memory_listener_update_region-新flatrange的更新)
  - [5.1. kvm_set_phys_mem(): 修改内核内存](#51-kvm_set_phys_mem-修改内核内存)
    - [基本检查](#基本检查)
    - [得到section的起始地址和大小](#得到section的起始地址和大小)
- [6. 总结](#6-总结)
- [7. 参考](#7-参考)
...
该函数倒是不长，主体是一个while循环，循环条件是`old_view->nr`和`new_view->nr`,表示**新旧view**的**可用FlatRange数目**。这里依次对**FR数组**的**对应FR** 做对比，主要由下面几种情况：**frold**和**frnew**均存在、frold存在但frnew不存在，frold不存在但frnew存在。
...
其中`address_space_to_flatview`会返回as的`current_map`
