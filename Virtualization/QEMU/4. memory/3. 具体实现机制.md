...
- [4. 实际内存的分配GPA/HVA](#4-实际内存的分配gpahva)
  ...
  - [4.2. 将整体mr(pc.ram)划分为两个子MR(subregion): 不再申请内存, 即不关联host虚拟内存(HVA)](#42-将整体mrpcram划分为两个子mrsubregion-不再申请内存-即不关联host虚拟内存hva)
    ...
    - [4.2.2. 从pc.ram中划分并加入system_memory的subregion](#422-从pcram中划分并加入system_memory的subregion)
      ...
      - [3.1.1. address_space_update_topology(): 设置拓扑flatview, 会与KVM交互](#311-address_space_update_topology-设置拓扑flatview-会与kvm交互)
        - [3.1.1.1. generate_memory_topology(mr): 根据给定的mr生成新的Flatview](#3111-generate_memory_topologymr-根据给定的mr生成新的flatview)
...
#### 3.1.1. address_space_update_topology(): 设置拓扑flatview, 会与KVM交互
...
##### 3.1.1.1. generate_memory_topology(mr): 根据给定的mr生成新的Flatview
...
- clip 表示**父空间的地址范围**, 子空间的地址范围是不允许落在父空间的范围外的, 如
