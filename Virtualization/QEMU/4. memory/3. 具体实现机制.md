...
- [3. 全局的系统AS和MR的初始化: 不分配GPA/HVA](#3-全局的系统as和mr的初始化-不分配gpahva)
  - [address_space_init(): 整个AS初始化](#address_space_init-整个as初始化)
    - [address_space_update_topology(): 设置拓扑flatview](#address_space_update_topology-设置拓扑flatview)
      - [generate_memory_topology(): 生成Flatview](#generate_memory_topology-生成flatview)
      - [address_space_set_flatview(): 设置FlatView, 会调用KVM](#address_space_set_flatview-设置flatview-会调用kvm)
- [4. 实际内存的分配GPA/HVA](#4-实际内存的分配gpahva)
  - [4.1. 分配整体内存全局MR(ram)](#41-分配整体内存全局mrram)
  ...
  - [4.2. 将整体mr(pc.ram)划分为两个子MR(subregion): 不再申请内存, 即不关联host虚拟内存(HVA)](#42-将整体mrpcram划分为两个子mrsubregion-不再申请内存-即不关联host虚拟内存hva)
    - [计算above_4g_mem_size和below_4g_mem_size](#计算above_4g_mem_size和below_4g_mem_size)
    - [从pc.ram中划分并加入system_memory的subregion](#从pcram中划分并加入system_memory的subregion)
      - [memory_region_init_alias(): 初始化alias的mr, 不申请内存](#memory_region_init_alias-初始化alias的mr-不申请内存)
      - [memory_region_add_subregion(): subregion注册到全局的system_memory](#memory_region_add_subregion-subregion注册到全局的system_memory)
- [5. FlatRange](#5-flatrange)
- [6. 参考](#6-参考)
...
### address_space_update_topology(): 设置拓扑flatview
```cpp
...
#### generate_memory_topology(): 生成Flatview
...
#### address_space_set_flatview(): 设置FlatView, 会调用KVM
...
### 计算above_4g_mem_size和below_4g_mem_size
...
### 从pc.ram中划分并加入system_memory的subregion
...
#### memory_region_init_alias(): 初始化alias的mr, 不申请内存
...
#### memory_region_add_subregion(): subregion注册到全局的system_memory
...
可以看到，这里listener的作用就凸显出来了。对于**每个address_space**，调用address_space_update_topology()执行更新。里面涉及两个重要的函数generate_memory_topology和address_space_update_topology_pass。前者对于一个给定的MR，生成其对应的FlatView，而后者则根据oldview和newview对当前视图进行更新。我们还是看下address_space_update_topology函数代码
