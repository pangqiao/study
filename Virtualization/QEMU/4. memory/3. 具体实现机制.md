...
函数主要做了以下几个工作，设置**addressSpaceh和MR的关联**，并通过`address_space_update_topology(as)`**初始化对应的FlatView**, 设置**其名称**。最后把`address_space_memory`加入到**全局的address_spaces链表**中，最后调用`MEMORY_LISTENER_UPDATE_REGION()`提交本次修改，这个后面做论述。
