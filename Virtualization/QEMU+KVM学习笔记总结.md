...
一个**事件驱动的架构**是以一个派发事件到处理函数的循环为核心的。
...
- 处理到期的**定时器(timer**)。定时器的管理在**qemu\-timer.c**文件中。
...
不应该执行可阻断系统调用或是**长运行计算**（long\-running computations）。由于事件循环在继续其他事件时会等待当前回调函数返回，所以如果违反这条规定会导致guest暂停并且使管理器无响应。
