...
**Parallel architecture(平行架构**)把那些**可以同时执行的工作**分成**多个进程或是线程**。我叫他线程化的架构（threaded architecture）。
...
目前，QEMU使用一种**混合架构**，把事件驱动和线程组合在一起。这种做法之所以有效是因为只在单个线程上执行的事件循环不能有效利用底层多核心的硬件。再则，有时候使用一个专用线程来减少特定工作的负担要比把它整合在一个事件驱动的架构中更简单有效。虽然如此，QEMU的核心还是事件驱动的，大多数代码都是在这样一个环境中执行的。
