...
<caption>DRHD结构</caption>
  ...
  <tr>
    ...
    <li>如果<b>置位</b>, 此硬件单元包含其作用域内具有指定分段 Segment 中的所有 PCI 兼容设备, 但在相同 Segment 的其他硬件单元范围内报告的设备除外. 如果为某个段报告了带有 INCLUDE_PCI_ALL 标志集的 DRHD 结构, 则必须由 BIOS 在同一个段的所有其他 DRHD 结构之后枚举该结构. 具有 INCLUDE_PCI_ALL 标志集的 DRHD 结构可以使用 “Device Scope” 字段枚举其范围内的 I/OxAPIC 和 HPET 设备. </li>
    <b>Bits 1-7: 保留位.</b>
    </td>
  </tr>
  <tr>
