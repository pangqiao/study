...
<caption>DRHD结构</caption>
  ...
  <tr>
    ...
    <li>如果<b>清位</b>, 此硬件单元仅仅包含其作用域下的设备(通过 “Device Scope” 字段显式标识<b>指定分段Segment</b>)。设备可以是 Device Scope Struct 中 “Type” 字段所述的任何类型的设备，包括（但不限于） I/OxAPIC 和 HPET。</li>
    <li>如果<b>置位</b>, 此硬件单元在其作用域内具有指定分段中的所有 PCI 兼容设备，但在同一分段的其他重新映射硬件单元范围内报告的设备除外。如果为某个段报告了带有INCLUDE_PCI_ALL标志集的 DRHD 结构，则必须由 BIOS 在同一个段 1 的所有其他 DRHD 结构之后枚举该结构。具有INCLUDE_PCI_ALL标志集的 DRHD 结构可以使用“设备范围”字段枚举其范围内的 I/OxAPIC 和 HPET 设备。</li>
    <b>Bits 1-7: 保留位.</b>
    </td>
  </tr>
  <tr>
