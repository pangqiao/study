...
在 **64 位模式**下运行时，第一阶段转换支持与处理器相同的分页结构。表 3 给出了第一阶段翻译结构的不同名称，这些名称是根据它们在翻译过程中的使用情况给出的。第 9.7 节详细描述了每个分页结构的格式。对于支持第一阶段转换的 5 级分页的实现，根据相应可扩展模式 PASID 表条目中第一阶段分页模式 （FSPM） 字段的编程选择 4 级分页与 5 级分页（请参阅第 9.5 节和第 9.6 节）
