...
上下文条目和 scalable-mode PASID-Table entry 可以配置为支持第二阶段转换。对于上下文条目，第二阶段转换仅适用于没有 PASID 的请求。 使用可扩展模式 PASID-Table 条目，第二阶段转换可以应用于所有请求（带或不带 PASID），并且可以嵌套应用所有请求的第一阶段转换（带或不带 PASID）。本节介绍如何使用不嵌套的第二阶段转换。 第 3.8 节描述了第二阶段翻译的嵌套使用。

每个上下文条目都包含一个指向第二阶段翻译结构基础的指针。第 9.3 节描述了上下文条目的确切格式。可扩展模式上下文条目引用可缩放模式 PASID 结构。每个可扩展模式 PASID 表条目都包含一个指向第二阶段转换结构基础的指针。第二阶段转换将输入地址限制为通过功能寄存器中的“最大访客地址宽度 （MGAW）”字段报告的特定于实现的地址宽度。输入地址将受到 MGAW 地址检查，任何违规行为都将被视为转换错误。第7章详细介绍了故障情况及其向软件报告。
