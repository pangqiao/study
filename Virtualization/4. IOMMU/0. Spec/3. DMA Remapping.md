...
Root Table 在内存中的地址通过 Root Table Address Register （RTADDR_REG）描述.

root table 作为顶层结构将设备映射到它们各自的域。根表在系统内存中的位置通过第 11.4.5 节中描述的根表地址寄存器进行编程。当 RTADDR_REG 寄存器中的转换表模式字段被编程为传统模式（RTADDR_REG.TTM 为 00b）时，根表地址寄存器 (RTADDR_REG) 指向根表。根表的大小为 4 KB，包含 256 个根条目以覆盖 PCI 总线编号空间 (0-255)。在请求的源 ID 字段中编码的总线编号（高 8 位）用于索引根条目结构。根表条目包含引用所有设备的上下文表的上下文表指针在根条目标识的总线上。
