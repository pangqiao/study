...
* 当 **5 级页表**用于**第一阶段转换**时，**PML4 表**位于 **PML5E** 中地址 （ADDR） 字段中指定的**物理地址**（请参阅表 30）, 该地址是 4 KB 自然对齐的 。如果 **4 级页表**用于**第一阶段转换**，则 4 KB 自然对齐的 **PML4 表**位于 `PASID-table entry` 的 **FSPTPTR** 字段中指定的物理地址（请参阅第 9.5 节）。PML4 表包含 512 个 64 位条目 （PML4E）。使用定义如下的物理地址选择 PML4E：
