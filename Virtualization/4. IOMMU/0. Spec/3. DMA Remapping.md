...
层次结构中的**每个分页结构**的大小均为 **4 KB**，有 **512** 个 **8** 字节条目。**重映射硬件**使用**输入地址的上半部分**来**标识**一系列**分页结构条目**。 这些条目中的最后一个标识输入地址转换为的区域的物理地址（称为页面框架）。输入地址的下半部分（称为页面偏移量）标识输入地址转换为的区域内的特定偏移量。每个分页结构条目都包含一个物理地址，该地址可以是另一个分页结构的地址，也可以是页面框架的地址。第一阶段翻译支持 4 级结构或 5 级结构。 第二阶段转换支持 N 级结构，其中 N 的值取决于能力寄存器中枚举的实现支持的来宾地址宽度 （GAW）。
