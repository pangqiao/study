...
在 **64 位模式**下运行时，第一阶段转换支持与处理器相同的分页结构。表 3 给出了第一阶段翻译结构的不同名称，这些名称是根据它们在翻译过程中的使用情况给出的。第 9.7 节详细描述了**每个分页结构的格式**。对于支持 5 级分页的实现，根据相应 scalable-mode PASID Table entry 中**第一阶段分页模式**（`First Stage Paging Mode`, FSPM） 字段的编程选择 4 级分页或 5 级分页（请参阅第 9.5 节和第 9.6 节）
