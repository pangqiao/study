...
**第一阶段转换**支持 **4 级结构**或 **5 级结构**。**第二阶段转换**支持 **N 级结构**，其中 N 的值取决于**功能寄存器**(Capability Register)中的支持的 guest 地址宽度（GAW）。
...
**一级转换**将**输入地址**限制为 **canonical 地址**（即地址 `bits 63:N` 与 地址 `bits [N-1]`具有相同的值，其中 4 级分页下 N 是 48，而 5 级分页下 N 是 57）。重映射硬件对第一层转换的请求将进行经典的地址检查，作为进行第一层转换的前提条件，将违反视为转化错误。

第一阶段转换将输入地址限制为规范地址（即，地址位 63：N 与地址位 [N-1] 具有相同的值，其中 N 是 4 级分页的 48 位和 5 级分页的 57 位）。通过重新映射硬件进行第一阶段转换的请求需要经过规范地址检查，这是第一阶段转换的先决条件，违规将被视为翻译错误。第7章详细介绍了转换故障情况以及如何将其报告给软件。
