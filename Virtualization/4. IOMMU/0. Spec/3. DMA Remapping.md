...
例如，一种实现可以 expose 一个重映射硬件, 从而支持 root bus 上的一个或多个集成设备，以及其他的重映射硬件 一个或一组 PCI Express 根端口后面的设备的其他重新映射硬件单元。平台固件 （BIOS） 将平台中的每个重新映射硬件单元报告给软件。 第8章描述了通过ACPI结构的报告结构。
