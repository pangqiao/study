...
对于仅第一阶段转换和仅第二阶段转换，请求中的 **DMA 地址**用作输入地址。

对于嵌套转换，**第一阶段**转换**生成**的**任何地址**（**访问第一阶段转换结构的地址**和第一阶段转换的**输出地址**）都用作第二阶段转换嵌套的输入地址。第 3.6 节、第 3.7 节和第 3.8 节分别提供了有关第一阶段、第二阶段和嵌套转换的更多详细信息。
