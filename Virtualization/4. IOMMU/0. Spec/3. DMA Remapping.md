...
一级转换用的 IO 虚拟地址（IOVA） 的软件必须限制为小于 **MGAW** 和 FSPM 隐含的规范地址宽度下限（即，当 FSPM 为 **4 级**时为 **47** 位，当 FSPM 为 **5 级**时为 **56** 位）的最小值。

在 **64 位模式**下运行时，第一阶段转换支持与处理器相同的分页结构。表 3 给出了第一阶段翻译结构的不同名称，这些名称是根据它们在翻译过程中的使用情况给出的。它还为每个结构提供用于定位它的物理地址的来源，用于从结构中选择条目的输入地址中的位，以及此类条目是否以及如何映射页面的详细信息。第 9.7 节详细描述了每个分页结构的格式。对于支持第一阶段转换的 5 级分页的实现，根据相应可扩展模式 PASID 表条目中第一阶段分页模式 （FSPM） 字段的编程选择 4 级分页与 5 级分页（请参阅第 9.5 节和第 9.6 节）
