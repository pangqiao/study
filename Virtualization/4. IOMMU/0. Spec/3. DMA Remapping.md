...
在此模式下，**硬件**将**中止所有 DMA 操作**，而无需设置 root-table 将**每个条目**标记为**不存在**。此模式在启用基于 DMA 重映射的内存保护的过程中最有用。在没有 abort-dma 模式的情况下，系统软件必须在内存中找到一个不受 DMA 访问保护的区域来设置转换表，然后启用 DMA 重新映射硬件。 abort-dma 模式消除了对 DMA 保护区的需求，以便启用 DMA 重新映射。
