...
* 当 **5 级页表**用于第一阶段转换时，4-K Byte 自然对齐的 PML5 表位于 `scalable-mode PASID-table entry` 中 **FSPTPTR** 字段中指定的**物理地址**（参见第 9.5 节和第 9.6 节）。一个 PML5 表包含 512 个 64 位条目 (PML5E)。使用定义如下的物理地址选择 PML5E：

    * bits 2:0 是全 0.

    * bits 11:3 是输入地址的第 bits 56:48 