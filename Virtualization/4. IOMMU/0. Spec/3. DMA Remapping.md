...
当 `RTADDR_REG` 寄存器中的**转换表模式字段**(`Translation Table Mode`)被设置为**传统模式**（`RTADDR_REG.TTM` 为 `00b`）时，`RTADDR_REG` 指向 root table。
...
对于**支持可扩展模式转换**（Extended Capability Register 中的 SMTS=1）的实现，当RTADDR_REG寄存器中的转换表模式字段编程为可扩展模式 （RTADDR_REG） 时，根表地址寄存器 （RTADDR_REG） 指向可扩展模式根表。TTM 是 01b）。可扩展模式根表类似于根表（大小为 4KB，包含 256 个可扩展模式根条目以覆盖 0-255 PCI 总线编号空间），但引用可扩展模式上下文条目的格式不同。每个可扩展模式根条目都引用一个较低的可扩展模式上下文表和一个较高的可扩展模式上下文表。
