...
# 7. Second-Level Translation

> 二级转换

Context entry 和 scalable-mode PASID-Table entry 可以配置为支持**二级转换**。

* 对于 Context entry，二级转换仅适用于 `requests-without-PASID`。 

* 对于 scalable-mode PASID-Table entry，**二级转换**可以应用于**所有请求**（with or without PASID），并且可以为所有请求嵌套应用第一阶段转换（with or without PASID）。

本节介绍如何使用不嵌套的二级转换。

每个上下文条目都包含一个指向二级转换结构基础的指针。第 9.3 节描述了上下文条目的确切格式。可扩展模式上下文条目引用可缩放模式 PASID 结构。每个可扩展模式 PASID 表条目都包含一个指向二级转换结构基础的指针。二级转换将输入地址限制为通过功能寄存器中的“最大访客地址宽度 （MGAW）”字段报告的特定于实现的地址宽度。输入地址将受到 MGAW 地址检查，任何违规行为都将被视为转换错误。第7章详细介绍了故障情况及其向软件报告。
