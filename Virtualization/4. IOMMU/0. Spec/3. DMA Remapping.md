...
上下文条目和 scalable-mode PASID-Table entry 可以配置为支持**二级转换**。对于上下文条目，第二阶段转换仅适用于没有 PASID 的请求。 使用可扩展模式 PASID-Table 条目，第二阶段转换可以应用于所有请求（带或不带 PASID），并且可以嵌套应用所有请求的第一阶段转换（带或不带 PASID）。本节介绍如何使用不嵌套的第二阶段转换。 第 3.8 节描述了第二阶段翻译的嵌套使用。
