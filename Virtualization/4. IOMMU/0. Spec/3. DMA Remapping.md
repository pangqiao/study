...
* 当 **5 级页表**用于第一阶段转换时，**PML5 表**位于 `scalable-mode PASID-table entry` 中 **FSPTPTR** 字段中指定的**物理地址**（参见第 9.5 节和第 9.6 节）, 这个地址是 4-K Byte 自然对齐的。一个 PML5 表包含 512 个 **64 位条目**(**PML5E**)。使用**如下构成的物理地址**选择 **PML5E**：
...
由于 PML5E 使用输入地址的 `63：48` 位进行标识，因此它控制对输入地址空间的 256 TB 区域的访问。
