...
地址转换硬件上出现的每个入站请求都需要标识发起请求的设备。标识 I/O 事务发起方的属性在本文档中称为“源 ID”。重新映射硬件可以通过特定于实现的方式确定事务的源 ID。例如，某些 I/O 总线协议可能会在每个 I/O 事务中提供源设备标识。在其他情况下（例如，对于根复合体集成设备），源 id 可以基于根复合体内部实现派生。
