...
真正的 vq 内存在 struct vring 中的 desc, 
...
最后设备按照 Virtio 标准的约定, 调用函数 `vring_init` 分别计算并设置了**描述符区域**、**可用描述符区域**、**已用描述符区域**的地址. `vring_init` 与我们前面讨论的**驱动侧的代码一致**, 不再赘述. 至此, 驱动侧和设备侧协商好了传递数据的基础设施 Virtqueue.
