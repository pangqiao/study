...
**数组 ring** 为**可用描述符的集合**, **每个可用描述符**占用 **2 字节** `(__u16)`, 当 Virtqueue 为**空**时, 最多有 num 个可用描述符, 加上变量 flags 和 idx, 所以**可用描述符区域**需要的**内存**为 `num + 2` 个 `__u16`, 即 `sizeof(__u16) × (2 + num)` 是可用描述符区域需要的内存.
...
**结构体 vring** 的字段 desc 指向字符**描述符表**, 这里参数 p 指向的就是前面分配的**内存区的起始位置**. **可用描述符区域**位于 num 个描述符之后的位置, 所以我们可以看到 avail 指向的是从 p 开始预留了 num 个描述符的位置. 在可用描述符区域之后是**已用描述符区域**, 所以 used 指向 avail 中数组 ring 的最后一个元素之后, 而且是按照页面对齐的位置.
