...
函数 `do_req` 需要将 **request** 中的这些数据, 组织到**一个描述符链**的**多个描述符**中.

**request**(`struct request`, `include/linux/blkdev.h`) 包含一个 **bio 链表**, 每个 bio 中可能还包含多个 `bio_vec`, 内核中借用了一个数据结构 scatterlist, 来将这个立体的数据结构转换为一个平面的数据结构, scatterlist 中的**每一项**对应一个 `bio_vec`, 具体的转换函数为**通用 I/O 层**的 `blk_rq_map_sg`.
