...
kvmtool 首先从寄存器 `CONFIG_ADDRESS` 中**取出**目标 PCI 设备的**设备号**, 然后以设备号为索引, 在**数组** `pci_devices` 中确认是否存在这个 PCI 设备. PCI 标准规定, 对于不存在的设备, 寄存器 `CONFIG_DATA` 的**所有位**都置为 "1", 表示**无效设备**, 见第 36 行代码.
