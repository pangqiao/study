...
这些位在 `IA32_VMX_CRO_FIXEDO` MSR 寄存器里属于 `Fixed to 1`（**固定为 1**) 位（参见第 `2.5.10` 节）, 否则执行 VMXON 指令将会产生 `#GP` 异常。由于 `CRO.PG` 与 `CRO.PE` 位必须为 1, 因此 VMX operation **不能**从**实模式**和**非分页**的**保护模式**里进入。如果需要在 **SMX 模式**里进入 VMX 模式，那么 `CRO.SMXE` 位（`bit 14`) 也需要为 **1** 值。
...
上面所述的那些 CRO 与 CR4 寄存器**固定位**从**第 1 代支持 VMX 的处理器**开始就被确定了，后面我们将会看到软件需要使用 `IA32_VMX_CRO_FIXEDO MSR` 与 `IA32_VMX_CRO_FIXED1 MSR` 对 **CRO 寄存器**的**固定位**进行检测，使用 `IA32_VMX_CR4_FIXEDO MSR` 与 `IA32_VMX_CR4_FIXEDO MSR` 对 **CR4** 寄存器**固定位**进行检测。
