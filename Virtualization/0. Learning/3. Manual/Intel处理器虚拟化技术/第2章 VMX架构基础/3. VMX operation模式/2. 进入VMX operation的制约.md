...
- [IA32_FEATURE_CONTROL MSR寄存器](#ia32_feature_control-msr寄存器)
  - [小结](#小结)
- [CR0 与 CR4 固定位](#cr0-与-cr4-固定位)
  - [小结](#小结-1)
- [A20M 模式](#a20m-模式)
...
`IA32_FEATURE_CONTROL MSR` 寄存器(地址`3AH`)也**影响着 VMXON 指令的执行**. 当**物理处理器**被**reset**, 这个MSR会全部被**清零**.
...
**系统BIOS！！！** 可以**使用此bit**为BIOS**提供设置选项**，以**禁用对VMX的支持**。 为了启用 VMX 支持, BIOS必须设置 `bit 1` 或 `bit 2` 或**两者均设置**, 当然bit 0必须设置.
...
**进入 VMX operation 模式前**，**CRO** 与 **CR4** 寄存器也**必须**要满足 VMX operation 模式运行的**条件**：

(1) **CRO** 寄存器需要满足 **PG**(`bit 31`)、**NE**(`bit 5`），以及 **PE**(`bit 0`) 位为 1。也就是需要开启**分页机制**的**保护模式**(包括了 `IA-32e` 模式)，x87 FPU 数字异常的**处理模式**需要使用 **native 模式**.

(2) **CR4** 寄存器需要开启 **VMXE 位**（`bit 13`)。

这些位在 `IA32_VMX_CR0_FIXED0` MSR 寄存器里属于 `Fixed to 1`（**固定为 1**) 位（参见第 `2.5.10` 节）, 否则执行 VMXON 指令将会产生 `#GP` 异常。由于 `CRO.PG` 与 `CRO.PE` 位必须为 1, 因此 VMX operation **不能**从**实模式**和**非分页**的**保护模式**里进入。如果需要在 **SMX 模式**里进入 VMX 模式，那么 `CRO.SMXE` 位（`bit 14`) 也需要为 **1** 值。
...
在 **VMCS 区城的字段**设置里，也有很多**类似**这样的**制约条件**存在，某些位必须为 1 值，某些位必须为 0 值，某些位可以为 0 值也可以为 1 值。例如，上面的 CRO 与 CR4 寄存器列举的位必须为 1 值。因此，软件需要检测制约条件而进行相应的设置，这些设置也可能随着**处理器架构的发展**而**有所改变**。

上面所述的那些 CRO 与 CR4 寄存器**固定位**从**第 1 代支持 VMX 的处理器**开始就被确定了，后面我们将会看到软件需要使用 `IA32_VMX_CR0_FIXED0 MSR` 与 `IA32_VMX_CR0_FIXED1 MSR` 对 **CRO 寄存器**的**固定位**进行检测，使用 `IA32_VMX_CR4_FIXED0 MSR` 与 `IA32_VMX_CR4_FIXED1 MSR` 对 **CR4** 寄存器**固定位**进行检测。
...
下面一段代码来自 `libl\Vmx\VmxInit.asm` 里的 `initialize_vmxon_region` 函数，执行对 **CRO** 与 **CR4** 寄存器**固定位**的设置。
...
(1) 检查 **CRO** 寄存器的 **PE** 与 **PG** 位是否满足条件（在**开启分页的保护模式**下），通过与 Crofixedmask 值进行比较，如果 PG 和 PE 位与 Crofixedmask 中的 PG 和 PE 值不同，则失败返回。此错误不能在当前函数修复。

(2) 如果 CRO 满足上面的基本条件，则直接设置 CRO 的 Fixed to 1（固定 1) 位与  Fixed to 0（固定 0) 位。这里的 **Cr0fixed0** 与 **Cr0fixed1** 的值来自于 `IA32_VMX_CRO_FIXEDO` 与 `IA32_VMX_CRO_FIXED1` 寄存器。

(3) CR4 可以直接设置，通过“或”上 Cr4FixedMask 值，然后“与”上 Cr4Fixed1 值，得到最终的 CR4 的值。

CRO 与 CR4 寄存器设置的算法是一样的，通过 “**或**” **固定为 1 值**，然后 “**与**” **固定为 0 值**，得出最后需要的值。

根据 CRO 寄存器的限制，处理器在 VMX operation 模式（包括 `VMX root-operation` 与 `VMX non-root operation` 模式）必须要开启**分页保护模式**，那么 guest 软件必须运行在分页保护模式，在进入 guest 端时，处理器会检查是否符合这个要求。当不符合这个制约条件时，将产生 VM-entry 失败。如果需要 guest 软件执行实模式代码，那么可以选择进  virtual-8086 模式。
...
在这种情况下，进入 guest 时，处理器将不会检查CRO.PE与CRO.PG 控制位（参见第 4.5.1 节），而忽略 CRO 的 Fixed to 1（固定为 1) 中的相应位。但是，如果 CRO.PG为 1,则 CRO.PE 必须为 1。
...
例子的 boot 阶段就调用了 FAST A20 ENABLE 宏来开启 A20 地址线（即 A20 线  unmask)，这个宏实现在 inclu. Inc 文件里
