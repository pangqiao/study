...
- [1. IA32_FEATURE_CONTROL 寄存器](#1-ia32_feature_control-寄存器)
...
vmx_init()                               // 初始化入口
 ...
 |   ├─ cpuhp_setup_state_nocalls(CPUHP_AP_KVM_STARTING, "kvm/cpu:starting", kvm_starting_cpu, kvm_dying_cpu)      // 注册cpu状态变化(热插拔)的回调函数
```

> 另: 注册了cpu热插时候调用 `kvm_starting_cpu`, 其中也会调用`hardware_enable`开启虚拟化功能
...
如果满足下面这几种情况, 则表明**vmx被禁用**, 当lock 位为1时:
* in = 0 打开tboot
* out = 0, in =1, 打开tboot
* out = 0, 关闭tboot
...
update_caps:
        ...
        // tboot打开并且inside不为0 或者
        // tboot不打开并且outside不为0
