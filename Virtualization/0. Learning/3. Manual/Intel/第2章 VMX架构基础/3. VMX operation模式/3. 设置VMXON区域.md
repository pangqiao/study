...
软件应为每个 cpu 使用单独的区域，并且不应在 cpu 执行 VMXON 指令和 VMXOFF 指令之间访问或修改该逻辑处理器的 VMXON 区域. 否则可能会导致不可预知的行为.
...
在执行 VMXON 之前，软件应将 VMCS 修订版标识符（请参见第 23.2 节）写入 VMXON 区域. （具体来说，它应该将 31 位 VMCS 修订版标识符写入 VMXON 区域前 4 个字节的 30：0 位;第 31 位应清除为 0. 它不需要以任何其他方式初始化 VMXON 区域. 软件应为每个逻辑处理器使用单独的区域，并且不应在逻辑处理器上执行 VMXON 和 VMXOFF 之间访问或修改该逻辑处理器的 VMXON 区域. 否则可能会导致不可预知的行为（包括第 23.11.1 节中确定的行为）
...
具体来说, 它应该将 31 位 `VMCS revision ID` 写入 VMXON 区域前 4 个字节的 `bit 30:0` 位; 第 31 位应清除为 0.
