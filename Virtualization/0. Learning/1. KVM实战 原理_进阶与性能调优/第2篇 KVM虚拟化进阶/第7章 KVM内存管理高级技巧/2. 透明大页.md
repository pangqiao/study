...
- [透明大页原理](#透明大页原理)
- [透明大页的使用](#透明大页的使用)
  - [内核编译选项和启动参数](#内核编译选项和启动参数)
  - [宿主机中查看并配置THP的使用方式](#宿主机中查看并配置thp的使用方式)
    - [enable接口](#enable接口)
    - [defrag接口](#defrag接口)
    - [khugepaged/defrag接口](#khugepageddefrag接口)
    - [客户机和宿主机同时使用](#客户机和宿主机同时使用)
  - [透明大页的效果(系统和进程)](#透明大页的效果系统和进程)
    - [启动虚拟机前透明大页](#启动虚拟机前透明大页)
    - [启动虚拟机](#启动虚拟机)
    - [查看系统透明大页](#查看系统透明大页)
    - [查看QEMU进程透明大页](#查看qemu进程透明大页)
- [4 THP的提出](#4-thp的提出)
...
- tmpfs文件系统通过mount \-o参数huge=always/never/within\_size/advise指定是否打开以及使用策略.
