...
### 1.2.1. AddressSpace: 虚拟机物理地址空间 GPA
...
#### 1.2.1.2. AddressSpace 与 MemoryRegion
...
### 1.2.2. MemoryRegion: 虚拟机物理地址 GPA 到主机虚拟内存 HVA(RAMBlock)的桥梁
...
#### 1.2.2.3. MemoryRegion 与 RAMBlock
...
### 1.2.3. RAMBlock: 虚拟机物理内存 GPA 与主机一段虚拟内存 HVA
...
### 1.2.4. Address, MemoryRegion, RAMBlock 关系
...
### 1.2.5. FlatView: 虚拟机物理地址空间

#### 1.2.5.1. AddressSpace 与 FlatView
...
#### 1.2.5.3. FlatView 与 MemoryRegion

FlatView 是根据 MemoryRegion 通过 `generate_memory_topology()`生成的.

### 1.2.6. MemoryRegionSection: 虚拟机物理地址空间 GPA
...
### 1.2.8. kvm_userspace_memory_region: GPA->HVA
...
### 1.2.9. AddressSpace、MemoryRegion、FlatView、MemoryRegionSection 与 KVMslot 的关系
...
#### 1.2.10.1. 全局链表和 AddressSpace 自己的 MemoryListener
...
### 1.2.12. memory_listeners、AddressSpace、AddressSpaceDispatch 等之间的联系
...
## kvm_mem_slot 结构体: 一个代表一段空间, GPA -> HVA
