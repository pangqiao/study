...
现在假定`KVM_CREATE_VM`所代表的虚拟机创建成功，下面通过检查`kvm_check_extension`结果填充KVMState，**kvm\_arch\_init初始化KVMState**，其中有IDENTITY\_MAP\_ADDR，TSS\_ADDR，NR\_MMU\_PAGES等，**cpu\_register\_phys\_memory\_client**注册qemu对**内存管理的函数集**
