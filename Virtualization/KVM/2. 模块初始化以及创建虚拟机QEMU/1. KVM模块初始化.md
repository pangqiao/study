...
- [2. kvm_init: 初始化kvm框架](#2-kvm_init-初始化kvm框架)
  - [2.1. 架构初始化: kvm_arch_init](#21-架构初始化-kvm_arch_init)
    ...
    - [2.1.4. mmu模块相关处理](#214-mmu模块相关处理)
    - [2.1.5. 设置全局的kvm_x86_ops](#215-设置全局的kvm_x86_ops)
    - [2.1.6. 设置MMU的shadow PTE masks](#216-设置mmu的shadow-pte-masks)
    - [2.1.7. timer初始化](#217-timer初始化)
    - [2.1.8. lapic初始化](#218-lapic初始化)
  - [2.2. workqueue?](#22-workqueue)
  - [2.3. 架构相关的硬件设置](#23-架构相关的硬件设置)
    - [2.3.1. vmx/svm的硬件配置hardware_setup()](#231-vmxsvm的硬件配置hardware_setup)
        - [2.3.1.1. 设置全局vmcs_config和vmx_capability](#2311-设置全局vmcs_config和vmx_capability)
        - [2.3.1.2. 一堆检查](#2312-一堆检查)
        - [2.3.1.3. posted中断的wakeup处理函数设置](#2313-posted中断的wakeup处理函数设置)
...
vmx_init()                               // 初始化入口
 ...
 |   ├─ kvm_cache_create_usercopy()      // 创建kvm_vcpu 的 kmem cache
...
### 2.3.1. vmx/svm的硬件配置hardware_setup()
...
##### 2.3.1.1. 设置全局vmcs_config和vmx_capability
...
后面会给创建vcpu时候使用, 该cache的arch部分可被拷贝到用户态使用.
