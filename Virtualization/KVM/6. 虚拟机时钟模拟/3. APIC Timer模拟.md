...
- [4. 启动定时器](#4-启动定时器)
  ...
  - [4.5. restart_apic_timer(): 重启apic timer定时器](#45-restart_apic_timer-重启apic-timer定时器)
    ...
    - [4.5.3. start_sw_period(): 周期性或一次性模式](#453-start_sw_period-周期性或一次性模式)
    - [4.5.4. start_sw_tscdeadline(): tscdeadline模式](#454-start_sw_tscdeadline-tscdeadline模式)
    - [4.5.5. apic_timer_fn(): hrtimer到期的响应函数](#455-apic_timer_fn-hrtimer到期的响应函数)
    - [4.5.6. apic_timer_expired(): apic timer已经到期处理](#456-apic_timer_expired-apic-timer已经到期处理)
...
kvm_lapic_reg_write(struct kvm_lapic *apic, u32 reg, u32 val)
...
### 4.5.3. start_sw_period(): 周期性或一次性模式
...
3. 如果是**一次性的话**, 那就此**结束**, 直接返回
4. 否则(**周期性**)调用`advance_periodic_target_expiration()`设置**周期性目标过期时间**
5. 启动hrtimer, 超时时间是`apic->lapic_timer.target_expiration`(`ktime_get() + deadline`), 在
6. 如果没有过期, 则直接启动apic timer的hrtimer.

apic timer的hrtimer实在lapic创建时候初始化的, 该hrtimer的中断响应回调函数是`apic_timer_fn()`, 详细见`中断虚拟化`部分

总结: 对于周期性的, 无论如何都会**启用hrtimer**; 对于一次性的, **已经过期的直接注入一次**, 否则**启用hrtimer**
...
### 4.5.4. start_sw_tscdeadline(): tscdeadline模式
...
### 4.5.5. apic_timer_fn(): hrtimer到期的响应函数
...
### 4.5.6. apic_timer_expired(): apic timer已经到期处理
