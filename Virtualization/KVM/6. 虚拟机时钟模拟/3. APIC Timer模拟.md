...
4. 对于非周期性的, 已经到期了, 则调用`apic_timer_expired()`
...
1. 如果
1. 调用`apic_timer_expired()`进行中断注入?, 
2. 如果是一次性的话, 那就此结束, 直接返回
3. 否则(周期性)调用`advance_periodic_target_expiration()`设置周期性目标过期时间
4. 启动hrtimer, 超时时间是`apic->lapic_timer.target_expiration`(`ktime_get() + deadline`), 在

如果没有过期, 则直接启动apic timer的hrtimer.
