...
- [2. 相关结构体](#2-相关结构体)
  - [2.1. struct kvm_pit: pit芯片](#21-struct-kvm_pit-pit芯片)
- [3. 初始化虚拟PIT](#3-初始化虚拟pit)
  - [3.1. QEMU](#31-qemu)
  - [3.2. 整体流程](#32-整体流程)
  - [3.3. 入口代码](#33-入口代码)
  - [3.4. 初始化PIT的worker和work](#34-初始化pit的worker和work)
  - [3.5. 初始化PIT的定时器hrtimer](#35-初始化pit的定时器hrtimer)
  - [3.6. kvm_pit_reset(): 重置pit](#36-kvm_pit_reset-重置pit)
    - [3.6.1. pit_load_count(): 装载计数值](#361-pit_load_count-装载计数值)
    - [3.6.2. kvm_pit_reset_reinject(): 重置pit重注入信息](#362-kvm_pit_reset_reinject-重置pit重注入信息)
  - [3.7. kvm_pit_set_reinject(): 设置pit重注入信息](#37-kvm_pit_set_reinject-设置pit重注入信息)
  - [3.8. 注册pit设备和pio接口](#38-注册pit设备和pio接口)
- [4. pit的time tick和timer](#4-pit的time-tick和timer)
- [5. 创建定时器](#5-创建定时器)
  - [5.1. 整体流程](#51-整体流程)
  - [5.2. 入口代码](#52-入口代码)
  - [5.3. create_pit_timer(): 创建定时器](#53-create_pit_timer-创建定时器)
  - [5.4. pit_timer_fn(): 定时器中断函数](#54-pit_timer_fn-定时器中断函数)
  - [5.5. pit_do_work(): 时钟中断注入](#55-pit_do_work-时钟中断注入)
  - [5.6. 定时器的中断ack](#56-定时器的中断ack)
...
### 3.6.2. kvm_pit_reset_reinject(): 重置pit重注入信息
...
## 3.7. kvm_pit_set_reinject(): 设置pit重注入信息
