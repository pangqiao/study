...
kvm_arch_vcpu_ioctl_run()                	// vcpu运行
 ...
 |   |   ├─ vcpu_enter_guest()      // 物理CPU进入guest模式, 返回1表明不用退到userspace, 否则退到userspace
...
```cpp
// 在立即进入guest之前需要检查一些条件, 这个函数内容的检查，我们需要检查一些条件，然后再立即进入guest，我们跳过调用退出处理程序，并通过完整的运行循环，如果完成快速路径，但有东西阻止我们再次立即进入来宾
// Introduce kvm_vcpu_exit_request() helper, we need to check some conditions before enter guest again immediately, we skip invoking the exit handler and go through full run loop if complete fastpath but there is stuff preventing we enter guest again immediately.
...
2. rbx，rbp(第一步)，r12，r13，r14，r15(x32对应的是edi, esi)数据寄存器压栈保存, 因为这些都可以使用
3. 压栈保存虚拟机vcpu寄存器
