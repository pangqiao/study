...
kvm_vm_ioctl() // kvm ioctl vm指令入口
 ...
 |   |   ├─ vcpu->arch.mp_state = KVM_MP_STATE_UNINITIALIZED/KVM_MP_STATE_RUNNABLE; // 设置vcpu状态为可运行态
...
kvm_arch_vcpu_ioctl_run()                	// vcpu运行
 ...
 |	├─for(;;)                               // 循环受vcpu_enter_guest()返回值或系统信号控制, 只有异常或外部信号才退出循环
 |	|   ├─ vcpu_enter_guest()      // 物理CPU进入guest模式, 返回1表明不用退到userspace, 否则退到userspace
 |	|	|   ├─ kvm_request_pending(); // 进入Guest模式前先处理相关挂起的请求(vcpu->requests大于0), 每个有相应处理
