...
kvm_arch_vcpu_ioctl_run()                	// vcpu运行
 ...
 |	|	|   ├─ kvm_x86_ops->prepare_guest_switch(vcpu); // 准备陷入到guest, 保存vmexit时加载的host状态
...
2）将**vCPU的状态**切换为`KVM_MP_STATE_UNINITIALIZED`。
...
`vcpu_run`最终调用`vcpu_enter_guest`，该函数实现了进入Guest，并执行Guest OS具体指令的操作。

该函数较长，执行的内容也比较重要，重要的部分如下。

1）在5232～5240行中，将**虚拟APIC**和**vCPU**的状态**重置**。这个操作通过调用`kvm_lapic_reset`和`kvm_arch_vcpu_reset`函数实现。
...
在上面的执行流程中，最重要的执行在 **vcpu\_enter\_guest** 函数中，该函数实现了**进入客户机**并且**执行具体指令**的操作。
