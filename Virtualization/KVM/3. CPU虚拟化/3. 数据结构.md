...
- [struct vmcs_host_state: VMEXIT时加载的host状态](#struct-vmcs_host_state-vmexit时加载的host状态)
...
`struct vcpu_vmx`代表**vcpu的一个运行环境**, `loaded_vmcs`指向当前vcpu使用的vmcs. 对于非嵌套(即L1)虚拟机, `loaded_vmcs`一直指向`vmcs01`, 对于嵌套(L2), 指向另外的vmcs.
...
# struct vmcs_host_state: VMEXIT时加载的host状态
