...
该函数比较简单，就是**根据irq号**，获取**重定向表中的一项**，然后向**本地APIC传递**，即调用`ioapic_deliver`函数，当然前提是`kvm_ioapic_redirect_entry`**没有设置mask**，根据`kvm_ioapic_redirect_entry`，**构建**`kvm_lapic_irq`，这就类似于在**总线上的传递过程**。构建之后调用`kvm_irq_delivery_to_apic`，该函数会**把消息传递给相应的VCPU** ，
