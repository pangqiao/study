...
kvm_vm_ioctl()  // vm ioctl的入口
 ...
 |   |   |   |       └─ hrtimer_start(&ps->timer, ktime_add_ns(ktime_get(), interval), HRTIMER_MODE_ABS); // 启动定时器
 |   |   |   ├─ } // 循环3次
...
2. 创建了一个worker(内核线程)和work(工作), 用来向客户机注入时钟中断
...
初始化**hrtimer**，hrtimer中断处理函数为`pit_timer_fn`，hrtimer的启动在`kvm_pit_reset`里，每个1ms产生一次中断

所以内核虚拟PIT实际是利用**HOST**的**hrtimer实际时钟源**来提供**虚拟时钟**的。 
