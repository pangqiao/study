...
- [5. PIT(8254)模拟](#5-pit8254模拟)
  ...
  - [5.5. 创建定时器](#55-创建定时器)
    ...
    - [5.5.4. pit_timer_fn(): 定时器中断](#554-pit_timer_fn-定时器中断)
- [6. 参考](#6-参考)
...
将超时work添加到工作线程, 这里的work(工作)在初始化pit中设置好（`kthread_init_work(&pit->expired, pit_do_work)`）
