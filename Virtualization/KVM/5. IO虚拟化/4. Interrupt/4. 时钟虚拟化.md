...
- [5. PIT(8254)模拟](#5-pit8254模拟)
  ...
  - [5.2. 初始化虚拟PIT](#52-初始化虚拟pit)
    ...
    - [5.2.2. 整体流程](#522-整体流程)
    - [5.2.3. 入口代码](#523-入口代码)
    - [5.2.4. 初始化PIT的worker和work](#524-初始化pit的worker和work)
    - [5.2.5. 初始化PIT的定时器hrtimer](#525-初始化pit的定时器hrtimer)
    - [5.2.6. kvm_pit_reset(): 重置pit](#526-kvm_pit_reset-重置pit)
      - [5.2.6.1. pit_load_count(): 装载计数值](#5261-pit_load_count-装载计数值)
      - [5.2.6.2. kvm_pit_reset_reinject(): 重置pit重注入信息](#5262-kvm_pit_reset_reinject-重置pit重注入信息)
    - [5.2.7. kvm_pit_set_reinject(): 设置pit重注入信息](#527-kvm_pit_set_reinject-设置pit重注入信息)
    - [5.2.8. 注册pit设备和pio接口](#528-注册pit设备和pio接口)
  - [5.3. pit的time tick和timer](#53-pit的time-tick和timer)
  - [5.4. 创建定时器](#54-创建定时器)
    - [5.4.1. 整体流程](#541-整体流程)
    - [5.4.2. 入口代码](#542-入口代码)
    - [5.4.3. create_pit_timer(): 创建定时器](#543-create_pit_timer-创建定时器)
    - [5.4.4. pit_timer_fn(): 定时器中断函数](#544-pit_timer_fn-定时器中断函数)
    - [5.4.5. pit_do_work(): 时钟中断注入](#545-pit_do_work-时钟中断注入)
    - [5.4.6. 定时器的中断ack](#546-定时器的中断ack)
- [6. APIC timer中断](#6-apic-timer中断)
  ...
  - [6.3. 启动定时器](#63-启动定时器)
    - [6.3.1. 整体流程](#631-整体流程)
    - [6.3.2. 入口代码](#632-入口代码)
    - [6.3.3. set_target_expiration(): 设置目标过期时间](#633-set_target_expiration-设置目标过期时间)
    - [6.3.4. restart_apic_timer(): 重启apic timer定时器](#634-restart_apic_timer-重启apic-timer定时器)
...
`pit_load_count`用于装载计数值，`ps->channels[channel].count_load_time`是装载时tick的值(每ns增加一次); `ps->channels[channel].count`是8254的周期, 8254自己的计数值会按照1193kHZ的频率递减的。注意和`count_load_time`单位的不同，以及后续单位的转换。
...
#### 5.2.6.2. kvm_pit_reset_reinject(): 重置pit重注入信息
...
### 5.2.7. kvm_pit_set_reinject(): 设置pit重注入信息
...
前面pit初始化时候注册的操作地址是`0x40 ~ 0x43`, 属于io地址空间, 独立于内存地址空间, 所以gpa, 不需要地址转换
...
### 6.3.3. set_target_expiration(): 设置目标过期时间
...
### 6.3.4. restart_apic_timer(): 重启apic timer定时器
...
如果**不是周期性模式**时(即**一次性/tscdeadline模式**), 而apic timer有pending事件, 则不重启, 直接退出.
...
时间虚拟化的一种实现方式是通过**时钟中断计数**，进而**换算得到**，这种方式在虚拟机里存在问题，因为有时运行**vpcu的cpu被调度出来**使时钟中断**不能准时到达guest os**。

另外一种方式，如**模拟HPET**，guest os当**需要的时候**会去**读当前的时间**，这种方式会使得虚拟机**频繁的VM-exit**，影响性能。

为此kvm引入了基于**半虚拟化**的时钟kvmclock,这种方式需要在**guest**上实现一个**kvmclock驱动**， 建立guest os 到VMM的通道， 这样通过这个通道guest os 向vmm 查询时间。
