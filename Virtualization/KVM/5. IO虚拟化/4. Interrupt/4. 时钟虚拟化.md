...
- [3. 虚拟化时钟](#3-虚拟化时钟)
- [4. PIT(8254)模拟](#4-pit8254模拟)
  - [4.1. 相关结构体](#41-相关结构体)
    - [4.1.1. struct kvm_pit: pit芯片](#411-struct-kvm_pit-pit芯片)
    - [4.1.2. struct hrtimer](#412-struct-hrtimer)
  - [4.2. 初始化虚拟PIT](#42-初始化虚拟pit)
    - [4.2.1. QEMU](#421-qemu)
    - [4.2.2. 整体流程](#422-整体流程)
    - [4.2.3. 入口代码](#423-入口代码)
    - [4.2.4. kvm_create_pit(): 创建pit(8254)](#424-kvm_create_pit-创建pit8254)
      - [4.2.4.1. 初始化PIT的worker和work](#4241-初始化pit的worker和work)
      - [4.2.4.2. 初始化PIT的定时器hrtimer](#4242-初始化pit的定时器hrtimer)
      - [4.2.4.3. 重置pit](#4243-重置pit)
  - [4.3. pit的time tick和timer](#43-pit的time-tick和timer)
  - [4.4. 创建定时器](#44-创建定时器)
    - [4.4.1. 整体流程](#441-整体流程)
    - [4.4.2. 入口代码](#442-入口代码)
    - [4.4.3. create_pit_timer(): 创建定时器](#443-create_pit_timer-创建定时器)
    - [4.4.4. pit_timer_fn(): 中断流程](#444-pit_timer_fn-中断流程)
- [5. 参考](#5-参考)
...
kvm_vm_ioctl()  // vm ioctl的入口
 ...
 |   |   |   |       ├─ hrtimer_cancel(); // 取消一个正在排队定时器, 但是因为hrtimer没有start, 所以这个动作也就没作用
 |   |   |   |       └─ kthread_flush_work(); // 停止worker前, 保证所有work已完成, 因为pit的work没有执行, 所以不涉及
 |   |   |   ├─ } // 循环3次
...
### 4.2.4. kvm_create_pit(): 创建pit(8254)
...
当guest os通过寄存器操作启动一个时钟控制器时, 即**客户机读写了8254的IO**，写了PIT channel的IO，说我需要一个定时器, 从而启动一个hrtimer来模拟**时钟中断源**.
...
### 4.4.3. create_pit_timer(): 创建定时器

每个1ms产生一次中断, 这个是符合物理真实pit功能的
