...
start_apic_timer()  // vcpu创建中创建lapic
 ├─ __start_apic_timer() // lapic创建入口 
 |   ├─ set_target_expiration(); // 设置到期时间, 在当前流程中, 周期性/一次性模式情况下, 需要 tscdeadline不会调用
 |   |   ├─ ktime_t now = ktime_get(); // 获取当前ktime
 |   |   ├─ apic->lapic_timer.period = tmict_to_ns(apic, kvm_lapic_get_reg(apic, APIC_TMICT)); // 设置触发周期时间, 即IMICT初始计数寄存器值
