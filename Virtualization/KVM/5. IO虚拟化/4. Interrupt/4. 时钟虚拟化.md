...
- [6. APIC timer中断](#6-apic-timer中断)
  ...
  - [启动定时器](#启动定时器)
    - [整体流程](#整体流程)
    - [入口代码](#入口代码)
    - [set_target_expiration(): 设置目标过期时间](#set_target_expiration-设置目标过期时间)
    - [restart_apic_timer(): 重启apic timer定时器](#restart_apic_timer-重启apic-timer定时器)
...
start_apic_timer()  // 启动apic timer定时器
 ...
 |   |   ├─  !period(apic) && pending; return;// 如果apic timer当前不是周期模式而且有pending事件, 直接返回, 不重启
...
### set_target_expiration(): 设置目标过期时间
...
```
### restart_apic_timer(): 重启apic timer定时器
