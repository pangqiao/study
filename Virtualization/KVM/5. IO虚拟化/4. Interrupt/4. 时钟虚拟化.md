...
- [3. PIT(8254)模拟](#3-pit8254模拟)
  - [3.1. 相关结构体](#31-相关结构体)
    ...
    - [3.1.2. struct hrtimer](#312-struct-hrtimer)
  - [3.2. 初始化虚拟PIT](#32-初始化虚拟pit)
    - [3.2.1. QEMU](#321-qemu)
    - [3.2.2. 整体流程](#322-整体流程)
    - [3.2.3. 入口代码](#323-入口代码)
    - [3.2.4. kvm_create_pit(): 创建pit(8254)](#324-kvm_create_pit-创建pit8254)
      - [3.2.4.1. 初始化PIT的worker和work](#3241-初始化pit的worker和work)
      - [3.2.4.2. 初始化PIT的定时器hrtimer](#3242-初始化pit的定时器hrtimer)
      - [3.2.4.3. 重置pit](#3243-重置pit)
  - [3.3. 创建定时器](#33-创建定时器)
    - [整体流程](#整体流程)
    - [入口代码](#入口代码)
    - [create_pit_timer(): 创建定时器](#create_pit_timer-创建定时器)
    - [3.3.1. pit_timer_fn(): 中断流程](#331-pit_timer_fn-中断流程)
- [4. 参考](#4-参考)
...
### 3.2.4. kvm_create_pit(): 创建pit(8254)
...
可以看到, 前面创建PIT没有真正创建定时器

为什么会创建定时器，那是因为**客户机读写了8254的IO**，写了PIT channel的IO，说我需要一个定时器。那么我们在`pit_ioport_write`里面就需要分析这些IO，看用户是否需要创建定时器，如果需要就调用`create_pit_timer`函数创建一个PIT。
