...
- [5. PIT(8254)模拟](#5-pit8254模拟)
  ...
  - [5.5. 创建定时器](#55-创建定时器)
    ...
    - [5.5.4. pit_timer_fn(): 中断流程](#554-pit_timer_fn-中断流程)
- [6. 参考](#6-参考)
...
这里的pit定时器`&ps->timer`是在pit初始化时候创建的, `hrtimer_init(&pit_state->timer, CLOCK_MONOTONIC, HRTIMER_MODE_ABS);`, 这里通过`hrtimer_start();`启用, 第二个参数是设置超时参数
