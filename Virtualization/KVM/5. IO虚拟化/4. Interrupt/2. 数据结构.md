...
IO APIC可以一个系统里面有多个，但是在KVM实现中只有一个，由`kvm.arch.vioapic`指向`kvm_ioapic`结构体。

kvm_ioapic结构体就是**虚拟IOAPIC**。

irr成员就是IRR寄存器。

`redirtbl[IOAPIC_NUM_PINS]`是一个数组，构成**PRT表**，每个成员对一个IOAPIC的引脚，执行kvm_ioapic_redirect_entry结构体。
