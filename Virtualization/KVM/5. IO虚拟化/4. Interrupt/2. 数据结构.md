...
`irq_routing`是这个虚拟机所有**IRQ CHIP抽象接口**的**表**。
`irq_routing`是该**虚拟机的整个中断路由信息表**, 

`chip`每个成员**对应一个引脚**，内容记录了**该引脚的GSI号**，其实**该成员已经不用了**。

**map**是**以gsi为索引**的**结构体数组**，数组元素是**双链表**，同`struct kvm_kernel_irq_routing_entry`中**link关联**。
