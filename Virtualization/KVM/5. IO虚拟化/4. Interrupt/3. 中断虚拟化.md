...
- [2. Time Tick](#2-time-tick)
  ...
  - [2.3. 相关物理芯片](#23-相关物理芯片)
    - [2.3.1. PIC主要为8259A PIC芯片](#231-pic主要为8259a-pic芯片)
    - [2.3.2. APIC](#232-apic)
- [3. 相关代码](#3-相关代码)
- [4. 整体流程](#4-整体流程)
...
- [6. 中断注入的触发](#6-中断注入的触发)
  - [6.1. 整体流程](#61-整体流程)
  - [6.2. 入口函数: kvm_set_irq()](#62-入口函数-kvm_set_irq)
  - [6.3. PIC中断触发: kvm_set_pic_irq](#63-pic中断触发-kvm_set_pic_irq)
    - [6.3.1. 整体流程](#631-整体流程)
  - [6.4. IOAPIC中断触发: kvm_set_ioapic_irq](#64-ioapic中断触发-kvm_set_ioapic_irq)
    - [6.4.1. 整体流程](#641-整体流程)
  - [6.5. MSI中断触发函数: kvm_set_msi](#65-msi中断触发函数-kvm_set_msi)
    - [6.5.1. 整体流程](#651-整体流程)
  - [6.6. LAPIC接收中断](#66-lapic接收中断)
- [7. 中断具体注入过程](#7-中断具体注入过程)
- [8. 中断注入触发的源](#8-中断注入触发的源)
- [9. 参考](#9-参考)
...
### 2.3.1. PIC主要为8259A PIC芯片

参见`Architecture/x86/4. x86中断与异常.md`
...
## 6.4. IOAPIC中断触发: kvm_set_ioapic_irq
...
对于IOAPIC来说, 整个流程是先检查IOAPIC状态，如果符合注入条件，则**组建中断结构体**，发送到**指定VCPU**的**LAPIC**，**设置LAPIC的寄存器**，完成虚拟中断控制器设置。
...
具体需要调用`kvm_apic_set_irq`函数，继而调用`__apic_accept_irq`


## 6.5. MSI中断触发函数: kvm_set_msi
