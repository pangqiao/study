...
- [6. 中断注入](#6-中断注入)
  ...
  - [6.4. IOAPIC中断触发: kvm_set_ioapic_irq](#64-ioapic中断触发-kvm_set_ioapic_irq)
    - [6.4.1. 整体流程](#641-整体流程)
  - [6.5. LAPIC接收中断](#65-lapic接收中断)
  - [6.6. MSI中断触发函数: kvm_set_msi](#66-msi中断触发函数-kvm_set_msi)
...
整个流程是先检查IOAPIC状态，如果符合注入条件，则**组建中断结构体**，发送到**指定VCPU**的**LAPIC**，**设置LAPIC的寄存器**，完成虚拟中断控制器设置。
