...
- [2. Time Tick](#2-time-tick)
  ...
  - [2.2. 相关物理芯片](#22-相关物理芯片)
    - [2.2.1. PIT主要为Intel 8254 PIT芯片](#221-pit主要为intel-8254-pit芯片)
    - [2.2.2. PIC主要为8259A PIC芯片](#222-pic主要为8259a-pic芯片)
- [3. 相关代码](#3-相关代码)
- [4. 整体流程](#4-整体流程)
- [5. 初始化中断控制器](#5-初始化中断控制器)
  - [LAPIC的创建](#lapic的创建)
  - [5.1. PIC和IOAPIC的创建](#51-pic和ioapic的创建)
    - [5.1.1. QEMU](#511-qemu)
    - [5.1.2. 整体流程](#512-整体流程)
    - [5.1.3. 入口代码](#513-入口代码)
    - [5.1.4. kvm_pic_init(): 虚拟pic(8259A)的初始化](#514-kvm_pic_init-虚拟pic8259a的初始化)
      - [5.1.4.1. kvm_io_bus_register_dev(): 在相应的io_bus总线上注册设备](#5141-kvm_io_bus_register_dev-在相应的io_bus总线上注册设备)
      - [5.1.4.2. 三种设备在虚拟bus上的结构](#5142-三种设备在虚拟bus上的结构)
    - [5.1.5. kvm_ioapic_init(): ioapic的初始化](#515-kvm_ioapic_init-ioapic的初始化)
    - [5.1.6. kvm_setup_default_irq_routing(): 默认中断路由表的初始化](#516-kvm_setup_default_irq_routing-默认中断路由表的初始化)
...
普通情况下，OS Time Tick由**PIT**(i8254)或**APIC Timer**设备提供产生中断源

- PIT(i8254)定期(**1ms in Linux**)产生一个**timer interrupt**，作为**global tick**, 
- **APIC Timer**产生一个**local tick**。

两者电子线路连接不同，对于i8254设备来说首先连接到i8259中断控制器，i8259中断控制器再连接到ioapic设备中，送到lapic,最后注入到vcpu中。对于APIC Timer设备实际就是lapic的一个功能，意思就是通过编程可以触发lapic设备周期产生中断，然后注入到vcpu。通过上面了解知道两者区别了APIC Timer的是每个cpu内核都有一个定时器，而PIT是共用的一个。
...
### 5.1.4. kvm_pic_init(): 虚拟pic(8259A)的初始化
...
#### 5.1.4.1. kvm_io_bus_register_dev(): 在相应的io_bus总线上注册设备
...
### 5.1.5. kvm_ioapic_init(): ioapic的初始化
...
### 5.1.6. kvm_setup_default_irq_routing(): 默认中断路由表的初始化
