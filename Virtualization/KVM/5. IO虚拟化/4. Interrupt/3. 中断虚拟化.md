...
kvm_vm_ioctl()  // vm ioctl的入口
 ...
 |   |   ├─ it_state->timer.function = pit_timer_fn; // hrtimer的处理函数
 ...
 |   |   ├─ kvm_io_bus_register_dev(kvm, KVM_PIO_BUS, KVM_PIT_BASE_ADDRESS, KVM_PIT_MEM_LENGTH, &pit->dev);// 注册时钟到bus总线上
...
初始化hrtimer，hrtimer中断处理函数为`pit_timer_fn`，hrtimer的启动在`kvm_pit_reset`里，**每个1ms产生一次中断**
