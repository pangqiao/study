...
- [5. 中断注入的触发](#5-中断注入的触发)
  - [5.1. 整体流程](#51-整体流程)
  - [5.2. 入口函数: kvm_set_irq()](#52-入口函数-kvm_set_irq)
  - [5.3. PIC中断触发](#53-pic中断触发)
    - [5.3.1. 整体流程](#531-整体流程)
    - [kvm_set_pic_irq(): pic的触发方法](#kvm_set_pic_irq-pic的触发方法)
  - [5.4. IOAPIC中断触发](#54-ioapic中断触发)
    - [5.4.1. 整体流程](#541-整体流程)
    - [kvm_set_ioapic_irq(): ioapic的触发方法](#kvm_set_ioapic_irq-ioapic的触发方法)
      - [ioapic_service(): 根据RTE构建中断消息发往lapic](#ioapic_service-根据rte构建中断消息发往lapic)
  - [5.5. MSI中断触发函数](#55-msi中断触发函数)
    - [5.5.1. 整体流程](#551-整体流程)
    - [kvm_set_msi(): msi的触发方法](#kvm_set_msi-msi的触发方法)
  - [5.6. LAPIC接收中断](#56-lapic接收中断)
...
### kvm_set_ioapic_irq(): ioapic的触发方法
...
#### ioapic_service(): 根据RTE构建中断消息发往lapic
