...
- [6. 初始化虚拟PIT](#6-初始化虚拟pit)
  ...
  - [6.2. 整体流程](#62-整体流程)
  - [入口代码](#入口代码)
  - [kvm_create_pit(): 创建pit(8254)](#kvm_create_pit-创建pit8254)
- [7. 参考](#7-参考)
...
kvm_vm_ioctl()  // vm ioctl的入口
 ├─  kvm_arch_vm_ioctl()     
 |   ├─ kvm->arch.vpit = kvm_create_pit(kvm, u.pit_config.flags);  // 如果irqchip在kvm中实现, 则直接返回, 不执行任何动作
 |   |   ├─ kzalloc(sizeof(struct kvm_pit), GFP_KERNEL_ACCOUNT);  // 给pit分配空间
 |   |   ├─ pit->irq_source_id = kvm_request_irq_source_id(kvm); // 注册master的IO端口读写函数
 |   |   ├─ pid = get_pid(task_tgid(current)); // 注册slave的IO端口读写函数
 |   |   ├─ pit->worker = kthread_create_worker(0, "kvm-pit/%d", pid_nr); // 注册eclr的IO端口读写函数
 |   |   ├─ kthread_init_work(&pit->expired, pit_do_work); // 注册了PIO型的bus访问形式；另一种IO形式为MMIO；
 |   |   ├─ hrtimer_init(&pit_state->timer, CLOCK_MONOTONIC, HRTIMER_MODE_ABS); // 注册了PIO型的bus访问形式；另一种IO形式为MMIO；
 |   |   ├─ it_state->timer.function = pit_timer_fn; // 设置回调函数
 |   |   ├─ pit_state->irq_ack_notifier.irq_acked = kvm_pit_ack_irq; // 设置回调函数
 |   |   ├─ pit->mask_notifier.func = pit_mask_notifer; // 设置回调函数
 |   |   ├─ kvm_pit_reset(pit); // 设置回调函数
 |   |   ├─ kvm_pit_set_reinject(pit, true); // 设置回调函数
 |   |   ├─ kvm_iodevice_init(&pit->dev, &pit_dev_ops); // 设置回调函数
 |   |   ├─ kvm_io_bus_register_dev(kvm, KVM_PIO_BUS, KVM_PIT_BASE_ADDRESS, KVM_PIT_MEM_LENGTH, &pit->dev);// 设置回调函数
 |   |   ├─ kvm_iodevice_init(&pit->speaker_dev, &speaker_dev_ops); // 设置回调函数
