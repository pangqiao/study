...
- [4. 初始化中断控制器](#4-初始化中断控制器)
  ...
  - [4.2. PIC和IOAPIC的创建](#42-pic和ioapic的创建)
    ...
    - [用户态qemu中断控制器与虚拟PC的关联](#用户态qemu中断控制器与虚拟pc的关联)
    - [4.2.2. 整体流程](#422-整体流程)
    - [4.2.3. 入口代码](#423-入口代码)
...
void kvm_irqchip_add_irq_route(KVMState *s, int irq, intirqchip, int pin)

{

    structkvm_irq_routing_entry e;
...
kvm_add_routing_entry ==》 kvm_vm_ioctl(s, KVM_SET_GSI_ROUTING, s->irq_routes);
