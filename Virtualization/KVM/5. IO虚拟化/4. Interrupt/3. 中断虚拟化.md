...
- [5. 中断注入的触发: kvm_set_irq()](#5-中断注入的触发-kvm_set_irq)
  ...
  - [5.4. irq_set[i].set(): 调用路由项的触发函数](#54-irq_setiset-调用路由项的触发函数)
    ...
    - [5.4.2. kvm_set_ioapic_irq(): IOAPIC中断触发](#542-kvm_set_ioapic_irq-ioapic中断触发)
    ...
    - [5.4.3. kvm_set_msi(): MSI中断触发函数](#543-kvm_set_msi-msi中断触发函数)
...
中断控制器目前主要有**APIC**，这种架构下**设备控制器**通过**某种触发方式**通知**IO APIC**，**IO APIC**根据**自身维护**的**重定向表pci irq routing table**格式化出**一条中断消息**，把中断消息发送给**local APIC**.
...
`kvm_vcpu_kick` 产生**处理器中断ipi** , **重新调度**，为中断注入做准备。
