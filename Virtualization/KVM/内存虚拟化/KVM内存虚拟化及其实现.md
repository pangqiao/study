...
显然通过这种映射方式，**客户机**的**每次内存访问**都需要 **KVM 介入！！！**，并由**软件进行多次地址转换**，其**效率是非常低**的。因此，为了**提高 GVA 到 HPA 转换的效率**，KVM 提供了**两种实现方式**来进行客户机虚拟地址到宿主机物理地址之间的直接转换。其一是基于**纯软件**的实现方式，也即通过**影子页表 (Shadow Page Table**) 来实现**客户虚拟地址**到**宿主机物理地址**之间的直接转换。其二是基于**硬件对虚拟化**的支持，来实现两者之间的转换。下面就详细阐述两种方法在 KVM 上的具体实现。
