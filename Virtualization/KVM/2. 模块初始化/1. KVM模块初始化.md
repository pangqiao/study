...
- [1. 原理介绍](#1-原理介绍)
- [整体流程](#整体流程)
- [2. kvm_init: 初始化 kvm 框架](#2-kvm_init-初始化-kvm-框架)
  - [2.1. 架构初始化: kvm_arch_init](#21-架构初始化-kvm_arch_init)
    - [2.1.1. CPU特性支持检查](#211-cpu特性支持检查)
    - [2.1.2. kmem_cache分配(x86_fpu)](#212-kmem_cache分配x86_fpu)
    - [2.1.3. percpu kvm_shared_msrs](#213-percpu-kvm_shared_msrs)
    - [2.1.4. mmu 模块相关处理](#214-mmu-模块相关处理)
    - [2.1.5. 设置全局的 kvm_x86_ops](#215-设置全局的-kvm_x86_ops)
    - [2.1.6. 设置 MMU 的shadow PTE masks](#216-设置-mmu-的shadow-pte-masks)
    - [2.1.7. timer初始化](#217-timer初始化)
    - [2.1.8. lapic初始化](#218-lapic初始化)
  - [2.2. workqueue?](#22-workqueue)
  - [2.3. 架构相关的硬件设置](#23-架构相关的硬件设置)
    - [2.3.1. vmx/svm 的硬件配置 hardware_setup()](#231-vmxsvm-的硬件配置-hardware_setup)
        - [2.3.1.1. 设置全局 vmcs_config 和 vmx_capability](#2311-设置全局-vmcs_config-和-vmx_capability)
        - [2.3.1.2. 一堆检查](#2312-一堆检查)
        - [2.3.1.3. posted 中断的 wakeup 处理函数设置](#2313-posted-中断的-wakeup-处理函数设置)
        - [2.3.1.4. percpu的VMXON region](#2314-percpu的vmxon-region)
    - [2.3.2. msr保存到全局变量msrs_to_save[]数组](#232-msr保存到全局变量msrs_to_save数组)
  - [2.4. 检查每个处理器对vmx的兼容性](#24-检查每个处理器对vmx的兼容性)
  - [2.5. 注册CPU状态变化的通知函数](#25-注册cpu状态变化的通知函数)
  - [2.6. 注册reboot时候的通知函数](#26-注册reboot时候的通知函数)
  - [2.7. 给kvm_vcpu分配cache](#27-给kvm_vcpu分配cache)
  - [2.8. 赋值file_operations的模块名](#28-赋值file_operations的模块名)
  - [2.9. 注册设备文件/dev/kvm](#29-注册设备文件devkvm)
  - [2.10. 动作注册](#210-动作注册)
  - [2.11. debugfs初始化](#211-debugfs初始化)
  - [2.12. vfio操作初始化](#212-vfio操作初始化)
- [3. 参考](#3-参考)
...
### 2.1.6. 设置 MMU 的shadow PTE masks
...
### 2.3.1. vmx/svm 的硬件配置 hardware_setup()
...
##### 2.3.1.1. 设置全局 vmcs_config 和 vmx_capability
...
##### 2.3.1.3. posted 中断的 wakeup 处理函数设置

```cpp
kvm_set_posted_intr_wakeup_handler(wakeup_handler);
```

##### 2.3.1.4. percpu的VMXON region
...
### 2.3.2. msr保存到全局变量msrs_to_save[]数组
