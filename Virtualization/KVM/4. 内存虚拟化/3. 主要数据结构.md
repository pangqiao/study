...
struct kvm_mmu_page {
        ...
        gfn_t gfn;
        // 指向影子页表页地址，页中被分为多个spte。影子页表用的页面称为shadow page，一个页面中分为多个表项，每个表项称为spte，注意不论哪个级别，表项都称为spte
...
分配好`kvm_mmu_page`结构后, 会用其**管理**的**GUEST物理地址空间**的**起始GFN！！！** 为key计算一个**hash值**，并上到哈希表`arch.mmu_page_hash[]`上，以便可以**快速的根据gfn**找到**管理该物理地址空间的页表页**. 这个**起始GPN**在`__direct_map()`中计算, 使用下面公式, 即`base_gfn`;
...
link: 表明不同状态. 同时只要是激活的页表页，也会加入`vcpu->kvm->arch.active_mmu_page`链表中，以便在后面快速的释放内存. 同样在`kvm_mmu_alloc_page()`中调用`list_add(&sp->link, &vcpu->kvm->arch.active_mmu_pages)`设置
