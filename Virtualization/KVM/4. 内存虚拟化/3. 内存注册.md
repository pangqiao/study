...
`id_to_memslot`则是根据**qemu的内存槽号**得到kvm结构下的内存槽号，转换关系来自id_to_index数组，那映射关系怎么来的，映射关系是**一一对应**的，在kvm_create_vm虚拟机创建过程中，`kvm_init_memslots_id`初始化对应关系，即`slots->id_to_index[i] = slots->memslots[i].id = i`，当前映射是没有意义的，估计是为了后续扩展而存在的。
