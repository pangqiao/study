...
- [QEMU 的主要工作](#qemu-的主要工作)
- [QEMU 和 KVM 的工作分界](#qemu-和-kvm-的工作分界)
- [1. QEMU中物理内存的注册](#1-qemu中物理内存的注册)
- [2. 处理用户态虚拟地址](#2-处理用户态虚拟地址)
- [3. KVM同步脏页位图到Qemu](#3-kvm同步脏页位图到qemu)
- [4. EPT页表映射](#4-ept页表映射)
...
QEMU 和 KVM 之间是通过 KVM 提供的`ioctl()`接口进行交互的。在内核的`kvm_vm_ioctl()`中，设置虚拟机内存的系统调用为`KVM_SET_USER_MEMORY_REGION`：
