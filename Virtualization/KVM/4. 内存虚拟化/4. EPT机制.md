...
kvm_vm_ioctl()  // 虚拟机vm的ioctl入口
 ...
 |   |   |   ├─ alloc_mmu_pages(vcpu, &vcpu->arch.guest_mmu);  // 创建mmu
 |   |   |   └─ alloc_mmu_pages(vcpu, &vcpu->arch.root_mmu);   // 创建mmu
...
所谓初始化就是填充 `vcpu->arch.mmu` 结构体，里面有很多回调函数都会用到，最终的是`tdp_page_fault`
