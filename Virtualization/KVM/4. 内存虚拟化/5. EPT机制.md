...
- [5. EPT构建过程](#5-ept构建过程)
  ...
  - [5.3. EPT异常处理流程](#53-ept异常处理流程)
    - [EPT异常处理入口](#ept异常处理入口)
    - [kvm_tdp_page_fault(): 建立页表项](#kvm_tdp_page_fault-建立页表项)
- [6. 参考](#6-参考)
...
在**2M大页**的情况下，**一个大页内**可以包含**512个小页**。

在KVM的代码中通过`KVM_PAGES_PER_HPAGE(level)`宏可以获得，**level**代表了**第几级页表**是**叶子节点页表**。
...
/*
 * level-1 级叶子页表所管理的页面 是 level 级叶子页表所管理的页面大小的 2^9倍。
...
vmx_handle_exit()  // vmxexit的入口
 ...
 |   |   |   |           ├─ gfn = gpa >> PAGE_SHIFT;   // 虚拟机物理地址右移12位得到虚拟机物理页框号
 |   |   |   |           ├─ mmu_topup_memory_caches(vcpu);   // 分配缓存池
 |   |   |   |           ├─ fast_page_fault();   // 快速page fault处理
 |   |   |   |           ├─ try_async_pf();   // 得到pfn, 主机物理页框号, HPA
 ...
 |   |   |   |           |        └─ return hva_to_pfn(); // 主机虚拟地址(HVA)转换成主机物理页框号(HPA)
...
假设这里**不能快速处理**，那么到后面就调用`try_async_pf`函数根据GFN获取对应的PFN，这个过程具体来说需要首先获取GFN对应的slot，转化成HVA，接着就是**正常的HOST地址翻译的过程**了，如果HVA对应的地址并不在内存中，还需要**HOST自己处理缺页中断**。
