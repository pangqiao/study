...
- [5. EPT构建过程](#5-ept构建过程)
  ...
  - [5.3. EPT异常处理流程](#53-ept异常处理流程)
    ...
    - [5.3.2. kvm_tdp_page_fault(): 建立页表项](#532-kvm_tdp_page_fault-建立页表项)
      - [5.3.2.1. kvm_mmu_page: 创建页](#5321-kvm_mmu_page-创建页)
      - [5.3.2.2. for_each_shadow_entry: 遍历EPT页表项](#5322-for_each_shadow_entry-遍历ept页表项)
- [6. 参考](#6-参考)
...
vmx_handle_exit()  // vmxexit的入口
 ...
 |   |   |   |               ├─ sp = kvm_mmu_get_page();   // 对于非叶子节点, 如果该entry指向的页表 分配当前entry的下一级(level-1, 指向的页表)页表的kvm_mmu_page(sp)
                        ...
                        bool prefault, bool account_disallowed_nx_lpage)
{
        ...
        // 设置pte，页表下一级的page地址就是pfn(HPA)写入到pte
...
#### 5.3.2.2. for_each_shadow_entry: 遍历EPT页表项
