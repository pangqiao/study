...
- [8. EPT构建过程](#8-ept构建过程)
  ...
  - [8.2. 虚拟机EPT和CR3初始化](#82-虚拟机ept和cr3初始化)
  - [8.3. EPT异常处理流程](#83-ept异常处理流程)
    - [8.3.1. EPT异常处理入口](#831-ept异常处理入口)
    - [8.3.2. kvm_tdp_page_fault(): 建立页表项](#832-kvm_tdp_page_fault-建立页表项)
      - [fast_page_fault(): 快速处理](#fast_page_fault-快速处理)
    - [try_async_pf(): GPA到HPA的转换](#try_async_pf-gpa到hpa的转换)
...
#### fast_page_fault(): 快速处理



### try_async_pf(): GPA到HPA的转换
...
- `gfn_to_hva`首先确定**gpa**对应的**gfn**映射到哪一个`kvm_memory_slot`，通过`kvm_memory_slot`做一个**地址映射**(实际就是做一个线性的地址偏移，偏移大小为`(gfn - slot->base_gfn) * PAGE_SIZE)`，这样就得到了由**gfn到hva的映射**，实际获得的是GPA的客户物理页号到宿主虚拟地址的映射。
