...
- [8. EPT构建过程](#8-ept构建过程)
  - [8.1. EPT页表的建立流程](#81-ept页表的建立流程)
  - [8.2. 虚拟机EPT根页表和CR3初始化](#82-虚拟机ept根页表和cr3初始化)
  - [8.3. EPT异常处理流程](#83-ept异常处理流程)
    - [8.3.1. EPT异常处理入口](#831-ept异常处理入口)
    - [8.3.2. kvm_tdp_page_fault(): 建立页表项](#832-kvm_tdp_page_fault-建立页表项)
      - [8.3.2.1. fast_page_fault(): 快速处理](#8321-fast_page_fault-快速处理)
      - [8.3.2.2. try_async_pf(): GPA到HPA的转换](#8322-try_async_pf-gpa到hpa的转换)
...
**每级页表**的**页表项**都会管理**一定宽度的物理地址空间**，比如**level 1级页表**的**每个页表项**就可以管理 `1<< 12` 大小的物理地址空间，这个宽度就是**页大小**

**level 2 级页表**的**一个页表项**管理地址空间的大小就是  `1 << (12 + 9)` ，即
...
`地址 & MASK`，得到的就是**该地址**的**level级页表**的**页表项**所管理地址**地址空间的起始地址**，该地址的大小是同level级页表的**页表项**管理地址**空间大小对齐**的
...
- `iterator.shadow_addr= vcpu->arch.mmu->root_hpa`, 影子页表level4页表页物理地址，EPT情况下，该值就是VMCS的`EPT_pointer`, 指向**EPT页表基地址**，下一个要查找的页表就是当前vCPU的根页表目录
...
vmx_handle_exit()  // vmxexit的入口
 ...
 |   |   |   |               ├─ link_shadow_page(vcpu, it.sptep, sp);   // 将分配的下一级页表链接到当前entry
 |   |   |   |           |   └─ mmu_set_spte();   // 设置spte
...
当**Guest第一次访问某页面**时，首先触发的是**Guest OS**的**page fault**，**不会vm-exit**, Guest OS会修复好**自己mmu的页结构**，并且**访问对应的GPA**，此时由于**对应的EPT结构**还**没有建立**，会触发EPT Violation。对于Intel EPT，EPT缺页的处理在函数tdp_page_fault中。
...
### 8.3.2. kvm_tdp_page_fault(): 建立页表项
...
#### 8.3.2.1. fast_page_fault(): 快速处理



#### 8.3.2.2. try_async_pf(): GPA到HPA的转换
                        ...
                        bool prefault, bool account_disallowed_nx_lpage)
{
        ...
        for_each_shadow_entry(vcpu, gpa, it) {
                ...
                // 得到虚拟机物理页帧号, 确保对应层级的偏移部分为0，如level=1，则baseaddr的低12位就清零
                // KVM_PAGES_PER_HPAGE(level)按照上面的分析，就是**大页**是**普通页面多少倍大小**，也就是对其关系；那么这里计算出来的`gfn`就是按照**大页倍数对齐后**的**起始gfn编号**
		 ...
		 */
	         ...
	         */
                ...
                if (!is_shadow_present_pte(*it.sptep)) {
                        ...
                        // 将当前的mmu page(sp)的地址写入到mmu page的pte中(it.sptep)
...
`for_each_shadow_entry`，用于根据GFN遍历EPT页表的对应项，这点后面会详细解释。循环中首先判断**entry的level**和**请求的level**是否相等，相等说明**该entry处引起的violation**，即该**entry对应**的**下级页**或者**页表**不在**内存**中，或者直接为NULL。

如果**level不相等**，就进入后面的if判断，这是判断**该entry**对应的**下一级页是否存在**，如果**不存在**需要**重新构建**，**存在**就**直接向后遍历**，即对比二级页表中的entry。

整个处理流程就是这样，根据**GPA**组**逐层查找EPT**，最终**level相等！！！** 的时候，就根据**最后一层的索引**定位一个**PTE**，**该PTE**应该指向的就是**GFN对应的PFN**，那么这时候**set spte**就可以了。**最好的情况**就是**最后一级页表**中的**entry指向的物理页**被换出外磁盘，这样只需要处理一次EPT violation，而如果在初始全部为空的状态下访问，**每一级的页表**都需要**重新构建**，则需要处理四次EPTviolation，发生4次`VM-exit`。

对**非叶子节点** (**level相等之前**), 发现需要的某一级的页表项为NULL, 就调用 `kvm_mmu_get_page` 获得**一页新的mmu page**，之后调用 `link_shadow_page` 调用 `mmu_set_spte`将其加入到EPT paging structure中(即设置页表项指向这个page)。

**一直到叶子节点**, 对于**叶子节点**，则**直接调用** `mmu_set_spte`。 `mmu_set_spte`调用 `set_spte` 来**设置spte**。`set_spte`是**设置页表结构的最终调用函数**。

#### 8.3.2.2. kvm_mmu_page: 创建页表页
...
下面`for_each_valid_sp`是一个遍历链表的宏定义，KVM为了根据**GFN**查找对应的`kvm_mmu_page`，用一个**HASH数组**记录**所有**的`kvm_mmu_page`，**每一个表项**都是一个**链表头**，即根据GFN获取到的HASH值相同的，位于一个链表中。这也是HASH表处理冲突常见方法。

如果在**对应链表**中找到一个**合适的页**，就直接利用该页，否则需要调用`kvm_mmu_alloc_page`函数**重新申请一个页**，主要是申请一个`kvm_mmu_page`结构和**一个存放表项**的**page**，这就用到了之前我们说过的**三种缓存**，这里只用到了两个，分别是`mmu_page_header_cache`和`mmu_page_cache`。这样分配好后，把对应的`kvm_mmu_page`作为一个节点加入到**全局的HASH链表**中，最后返回sp.
