...
vmx_handle_exit()  // vmxexit的入口
 ...
 |   |   |   |               ├─ level = kvm_mmu_hugepage_adjust(vcpu, gfn, max_level, &pfn);   // 调整level, gfn, pfn
 |   |   |   |               ├─ for_each_shadow_entry(vcpu, gpa, it);   // 遍历EPT页表 
 |   |   |   |               ├─ it.level == level: break;   //  如果页表的level等于该entry引起的violation
 |   |   |   |               ├─ sp = kvm_mmu_get_page();   // 遍历EPT页表, 分配 kvm_mmu_page(sp)
 |   |   |   |               ├─ link_shadow_page(vcpu, it.sptep, sp);   // 将
 |   |   |   |           |   └─ mmu_set_spte();   // 设置spte
                        ...
                        bool prefault, bool account_disallowed_nx_lpage)
{
        ...
        if (WARN_ON(!VALID_PAGE(vcpu->arch.mmu->root_hpa)))
                return RET_PF_RETRY;
        // 对level和gfn、pfn做出调整
        // level是gfn转换成hva, 然后得到这个hva映射的level
        ...
        for_each_shadow_entry(vcpu, gpa, it) {
                ...
                if (!is_shadow_present_pte(*it.sptep)) {
                        ...
                        // 将当前的mmu page(sp)的地址写入到mmu page的pte中
