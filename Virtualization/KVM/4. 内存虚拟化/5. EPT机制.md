...
循环中首先判断**entry的level**和**请求的level**是否相等，相等说明**该entry处引起的violation**，即该**entry对应**的**下级页**或者**页表**不在**内存**中，或者直接为NULL。

如果**level不相等**(即不到叶子节点)，就进入后面的if判断，这是判断**该entry**对应的**下一级页是否存在**，如果**不存在**需要**重新构建**，**存在**就**直接向后遍历**，即对比二级页表中的entry。

对**非叶子节点** (**level相等之前**), 发现需要的**某一级的页表项为NULL**, 就调用 `kvm_mmu_get_page` 获得**一页新的mmu page**，之后调用 `link_shadow_page` 调用 `mmu_set_spte`将其加入到EPT paging structure中(即设置当前页表项spte指向这个page)。
