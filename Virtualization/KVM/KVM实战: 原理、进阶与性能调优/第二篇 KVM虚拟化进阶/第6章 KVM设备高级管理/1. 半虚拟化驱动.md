...
而QEMU/KVM通过提供一个**半虚拟化的时钟**￼，即**kvm\_clock**，为客户机提供精确的System time和Wall time，从而避免客户机中时间不准确的问题。kvm_clock使用较新的硬件（如Intel SandyBridge平台）提供的支持，如不变的时钟计数器（Constant Time Stamp Counter）。constant TSC的计数频率，即使当前CPU核心改变频率（如使用了一些省电策略），也能保持恒定不变。CPU有一个不变的constant TSC频率是将TSC作为KVM客户机时钟的必要条件。
