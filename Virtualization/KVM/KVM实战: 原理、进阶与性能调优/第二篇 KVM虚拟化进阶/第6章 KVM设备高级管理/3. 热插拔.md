...
但如我们前面几章提到的那样，**device\_add**（或者\-**device参数**）是指定**前端设备！！！**，也就是QEMU模拟出来的**客户机看到的设备**，它的**实体**是要靠一个**对应的后端设备**来实现的。
...
在客户机里我们查看一下**内存设备**，128MB×64=8GB，与我们的启动设置相符。
...
在**QEMU monitor**中，我们**添加后端设备**（**memory\-backend\-ram**对象，id=mem1），以及**前端设备**（**pc\-dimm**设备，id=dimm1）。
