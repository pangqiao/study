...
**dimm设备**的**热插拔**与**PCI设备一样**，通过“**device\_add**”来完成。但如我们前面几章提到的那样，device_add（或者-device参数）是指定前端设备，也就是QEMU模拟出来的客户机看到的设备，它的实体是要靠一个对应的后端设备来实现的。所以，在device_add之前，我们先要通过object_add来定义这个后端设备，它的名字叫“memory-backend-ram”。
