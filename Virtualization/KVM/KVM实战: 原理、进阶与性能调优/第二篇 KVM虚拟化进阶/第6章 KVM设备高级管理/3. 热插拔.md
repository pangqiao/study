...
- **kernelcore**指定系统boot起来时，分配**多少内存作为kernel page**，剩下的都作为movable page。
- movablecore就是反过来，指定多少作为movable page￼，剩下的都是kernel page。
- movable\_node是指定是否需要这样一个memory node专门放movable zone。
