...
读者可以发现，第2种方法不够灵活，重新加载驱动会作用于所有适用此驱动的设备。而第1种方法可以不用重新加载驱动，并且可以只作用于某一个PF。
