...
如果要一次性分配**多个设备**给客户机，只需在qemu-system-x86_64命令行中重复多次“-device pci-assign，host=$BDF”这样的选项即可。由于设备直接分配是客户机独占该设备，因此一旦将一个设备分配给客户机使用，就不能再将其分配给另外的客户机使用了，否则在通过命令行启动另一个客户机再分配这个设备时，会遇到如下的错误提示：
