...
- \-H\<时间间隔\>，它设置（**override）透明大页**（见7.2节）的**扫描间隔时间**。**默认地**，numad会将/sys/kernel/mm/tranparent\_hugepage/khugepaged/**scan\_sleep\_millisecs**值从默认的**10000毫秒**缩短为**1000毫秒**，因为更激进的透明大页合并更有利于numad将页面在NUMA节点之间迁移。
