...
**ksm**服务的类型是**一次性**的（**one shot**￼），而**ksmtuned服务**（**forking类型**）一直保持**循环执行**（**每间隔若干时间**，见下面配置），以**调节ksm**（/sys/kernel/mm/ksm/下各个参数），其配置文件在/etc/ksmtuned.conf中。配置文件的默认内容如下：
...
这次我们就看到KSM起作用了，它大约节省了22GB内存，从free \-m \-h和/sys/kernel/mm/ksm/pages\_sharing都获得了印证。
