...
* [2 KSM操作实践](#2-ksm操作实践)
	* [2.1 确认和配置守护进程ksmd](#21-确认和配置守护进程ksmd)
...
- **pages\_shared**：记录正在使用中的共享内存页的数量。
- pages\_sharing：记录有多少数量的内存页正在使用被合并的共享页，不包括合并的内存页本身。这就是实际节省的内存页数量。
- pages\_unshared：记录了守护进程去检查并试图合并，却发现了因没有重复内容而不能被合并的内存页数量。
- pages\_volatile：记录了因为其内容很容易变化而不被合并的内存页。
- pages\_to\_scan：在ksmd进程休眠之前扫描的内存页数量。
- sleep\_millisecs：ksmd进程休眠的时间（单位：毫秒），ksmd的两次运行之间的间隔。
