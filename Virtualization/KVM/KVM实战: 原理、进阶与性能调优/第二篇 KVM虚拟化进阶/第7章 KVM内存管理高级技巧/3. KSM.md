...
在RHEL 7.3上，如果**不运行ksm服务程序**，则KSM**默认只会共享2000个内存页**，这样一般很难起到较好的效果。而在**启动ksm服务**后，KSM能够共享最多达到系统**物理内存一半的内存页**。

**ksm**服务的类型是**一次性**的（one shot￼），而ksmtuned服务（forking类型）一直保持循环执行（每间隔若干时间，见下面配置），以调节ksm（/sys/kernel/mm/ksm/下各个参数），其配置文件在/etc/ksmtuned.conf中。配置文件的默认内容如下：
