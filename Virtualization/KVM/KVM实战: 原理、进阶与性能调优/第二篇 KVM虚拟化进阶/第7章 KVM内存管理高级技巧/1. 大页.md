...
- hugetlb\_shm\_group：SysV共享内存段（shared memory segment）的id，该内存段将使用大页。我们常用的POSIX共享内存的方法用不到这个。
- nr\_hugepages\_mempolicy：通常与NUMA mempolicy（内存策略）相关。后面详细讲到。
- nr\_hugepages：就是大页的数目。
- nr\_overcommit\_hugepages：在nr\_hugepages数目以外，在nr\_hugepages数目不够的时候，还可以动态补充多少大页。
