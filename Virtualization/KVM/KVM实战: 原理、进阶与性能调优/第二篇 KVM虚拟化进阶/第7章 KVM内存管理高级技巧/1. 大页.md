...
系统起来时候已经通过Kernel boot param（hugepages=，hugepagesz=）指定了，那么此时应该已经有了。本例因为启动时候没有指定大页，所以此时才创建大页池。
...
我们还可以同时创建**2MB大页池**，创建客户机，并故意给客户机分配内存大于池内persistent huge page数量而出现over commit的现象。相关命令行总结如下：
