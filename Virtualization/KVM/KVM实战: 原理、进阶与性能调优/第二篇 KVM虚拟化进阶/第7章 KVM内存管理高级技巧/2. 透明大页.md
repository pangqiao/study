...
5）如果预留了大页内存但没实际使用，就会造成**物理内存的浪费**。
...
而当系统内存较为充裕、有**很多的大页可用**时，**常规的页分配的物理内存**可以通过**khugepaged内核线程自动迁往透明大页内存**。内核线程khugepaged的作用是，**扫描正在运行的进程**，然后**试图将使用的常规内存页转换到使用大页**。
...
这表示**默认对所有应用程序的内存分配！！！** 都**尽可能地使用透明大页**。当然，还可以在**系统启动**时修改Linux内核**启动参数**“**transparent\_hugepage**”来调整这个默认值。其取值为如下3个值之一：
...
[root@kvm-host ~]# echo 搉ever?> \ /sys/kernel/mm/transparent_hugepage/defrag￼
