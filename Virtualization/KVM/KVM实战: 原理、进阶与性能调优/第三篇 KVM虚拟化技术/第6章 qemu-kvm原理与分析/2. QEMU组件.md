...
在Linux中，为了便于编写virtio驱动程序，提供了针对virt\-queue的helper函数：virtqueue\_get_buf和virtqueue_add_buf。驱动程序可以通过virtqueue_get_buf从 virtqueue中取得来自 Hypervisor的响应消息。如果 virtio驱动需要发起一个I/O请求，则应执行如下操作。
