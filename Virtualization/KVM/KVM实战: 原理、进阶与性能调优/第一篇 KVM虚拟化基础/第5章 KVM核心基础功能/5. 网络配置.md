...
由于QEMU在客户机关闭时会解除TAP设备的bridge绑定，也会自动删除已不再使用的TAP设备，所以qemu-ifdown这个脚本不是必需的，最好设置为“downscript=no”。如下列出一个qemu-ifdown脚本的示例，是为了说明清理bridge模式网络环境的步骤，在QEMU没有自动处理时可以使用。

```
#!/bin/bash￼ #This is a qemu-ifdown script for bridging.￼ #You can use it when starting a KVM guest with bridge mode network.￼ #Don't use this script in most cases; QEMU will handle it automatically.￼ ￼ switch=$(brctl show| sed -n 2p |awk '{print $1}')￼ if [ -n "$1" ]; then￼         # Delete the specified interfacename￼         tunctl -d $1￼         #release TAP interface from bridge￼         brctl delif ${switch} $1￼         #shutdown the TAP interface￼         ip link set $1 down￼         exit 0￼ else￼         echo "Error: no interface specified"￼         exit 1￼ fi
```

6）用qemu命令行启动bridge模式的网络。
