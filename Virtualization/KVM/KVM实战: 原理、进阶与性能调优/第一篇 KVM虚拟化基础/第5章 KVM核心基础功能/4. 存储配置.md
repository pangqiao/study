...
- lazy\_refcounts，用于**延迟引用计数（refcount）的更新**，可以减少metadata的I/O操作，以达到提高performance的效果。适用于**cache=writethrough**这类不会自己组合metadata操作的情况。它的缺点是一旦客户机意外崩溃，下次启动时会隐含一次qemu\-img check \-r all的操作，需要额外花费点时间。它是当**compact=1.1**时才有的选项。
