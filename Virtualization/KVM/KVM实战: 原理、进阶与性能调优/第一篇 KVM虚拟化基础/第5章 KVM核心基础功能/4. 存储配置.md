...
iSCSI是一套**基于IP协议**的网络存储标准，真正的**物理存储**放在**目标端（target**），而**使用iSCSI磁盘**的是**初始端（initiator**），它们之间实现了**SCSI标准的命令**，让**目标端使用**起来就与**使用本地的SCSI硬盘**一样，只是数据是在网络上进行读写操作的。**光纤通道（Fibre Channel**）也可以实现与iSCSI类似的存储区域网络（storage area network，SAN），不过它需要光纤作为特殊的网络媒介。而GFS2是由Redhat公司主导开发的主要给Linux计算机集群使用的共享磁盘文件系统，一般在Redhat的RHEL系列系统中有较多使用，它也可被用作QEMU/KVM的磁盘存储系统。
