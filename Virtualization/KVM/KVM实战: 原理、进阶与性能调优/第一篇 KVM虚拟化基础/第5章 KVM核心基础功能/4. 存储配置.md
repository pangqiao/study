...
“splash=splashfile”和“splash\-time=sp\-time”选项都是在“**menu=on**”时才有效，将名为**splashfile的图片**作为**logo**传递给BIOS来显示；而sp-time是BIOS显示splash图片的时间，其单位是毫秒（ms）。图5-5展示了在使用“-boot order=dc，menu=on”设置后，在客户机启动窗口中按F12键进入的启动菜单。
4.存储配置的示例
