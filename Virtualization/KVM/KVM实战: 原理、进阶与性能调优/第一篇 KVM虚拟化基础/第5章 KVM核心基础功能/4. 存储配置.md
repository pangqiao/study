...
qemu-img是QEMU的磁盘管理工具，在完成QEMU源码编译后就会默认编译好qemu-img这个二进制文件。qemu-img也是QEMU/KVM使用过程中一个比较重要的工具，本节对其用法进行介绍。
...
对磁盘镜像文件进行一致性检查，查找镜像文件中的错误，目前仅支持对“qcow2”“qed”“vdi”格式文件的检查。其中，qcow2是目前使用最广泛的格式。qed￼（QEMU enhanced disk）是从QEMU 0.14版开始加入的增强磁盘文件格式，它可以在不支持空洞（hole）的文件系统和存储媒介上压缩image，避免了qcow2格式的一些缺点，也提高了性能。而vdi（Virtual Disk Image）是Oracle的VirtualBox虚拟机中的存储格式。参数-f fmt是指定文件的格式，如果不指定格式，qemu-img会自动检测。filename是磁盘镜像文件的名称（包括路径）。
如下命令行演示了qemu-img的check命令的使用方法。

[root@kvm-host ~]# qemu-img check rhel7.3.qcow2￼ No errors were found on the image.￼ Image end offset: 262144
