...
* [2 SMP的支持](#2-smp的支持)
	...
	* [2.3 QEMU的支持](#23-qemu的支持)
		...
		* [2.3.4 \-smp 8,socket=2,cores=2,threads=2](#234-smp-8socket2cores2threads2)
...
**不加smp参数**，使用其**默认值1！！！**，在客户机中查看CPU情况，如下：
...
### 2.3.4 \-smp 8,socket=2,cores=2,threads=2
...
**CPU的过载使用**是让**一个或多个客户机**使用**vCPU的总数量**超过实际拥有的**物理CPU数量**。QEMU会启动更多的线程来为客户机提供服务，这些线程也被Linux内核调度运行在物理CPU硬件上。
