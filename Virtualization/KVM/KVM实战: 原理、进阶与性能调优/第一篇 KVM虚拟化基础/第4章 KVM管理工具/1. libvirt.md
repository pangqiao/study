...
要让TCP、TLS等连接生效，需要在启动libvirtd时加上\-\-listen参数（简写为\-l）。而默认的systemctl start libvirtd命令在启动libvirtd服务时并没带--listen参数。所以如果要使用TCP等连接方式，可以使用libvirtd--listen-d命令来启动libvirtd。
