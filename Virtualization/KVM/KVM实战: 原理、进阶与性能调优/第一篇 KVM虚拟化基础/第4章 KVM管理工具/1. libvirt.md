...
libvirt首先需要创建**virNetworkPtr对象**，然后才能**查询或控制虚拟网络**。查询网络相关信息的函数有，virNetworkGetName函数可以获取**网络的名称**，virNetworkGetBridgeName函数可以获取该网络中**网桥的名称**，virNetworkGetUUID函数可以获取**网络的UUID标识**，virNetworkGetXMLDesc函数可以获取网络的以XML格式的描述信息，virNetworkIsActive函数可以查询网络是否正在使用中。
...
5）**存储卷管理**的API：以virStorageVol开头的一系列函数。
...
6）**存储池管理**的API：以virStoragePool开头的一系列函数。
