...
>
>注意:
>要让**TCP、TLS等连接生效**，需要在**启动libvirtd**时加上\-\-**listen参数**（简写为\-l）。而**默认的systemctl start libvirtd**命令在启动libvirtd服务时并没带--listen参数。所以如果要使用TCP等连接方式，可以使用libvirtd--listen-d命令来启动libvirtd。
以上配置选项实现将UNIX socket放到/var/run/libvirt目录下，启动libvirtd并检验配置是否生效。命令行操作如下：

[root@kvm-host ~]# libvirtd --listen -d￼ ￼ [root@kvm-host ~]# virsh -c qemu+tcp://localhost:16666/system￼ Welcome to virsh, the virtualization interactive terminal.￼ ￼ Type:  'help' for help with commands￼        'quit' to quit￼ ￼ virsh # quit￼ ￼ [root@kvm-host ~]# ls /var/run/libvirt/libvirt-sock*￼ /var/run/libvirt/libvirt-sock  /var/run/libvirt/libvirt-sock-ro
