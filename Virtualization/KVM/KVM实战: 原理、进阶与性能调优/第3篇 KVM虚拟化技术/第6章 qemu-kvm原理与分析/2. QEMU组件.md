...
在Linux中，为了便于编写virtio驱动程序，提供了针对virt\-queue的helper函数：virtqueue\_get\_buf和virtqueue\_add\_buf。驱动程序可以通过virtqueue\_get\_buf从 virtqueue中取得来自 Hypervisor的响应消息。如果 **virtio驱动**需要**发起一个I/O请求**，则应**执行如下操作**。
