...
基于**virtio的驱动程序架构**如图6\-4所示，virtio通过提供一个叫做**vqueue**的组件，将 **Guest OS** 的**各种 I/O 请求**保存在一个**缓冲区**中，随后交给 QEMU以进行调度执行。除了在 Guest OS 和 Hypervisor 中分别有 Frond\-end Drivers和 Back\-end Drivers 之外，还定义了另外两个层次用于支持 Guest OS 到Hypervisor（KVM）的通信。在顶级的是virt-queue虚拟队列接口，负责缓冲前端驱动程序的请求和后端驱动程序的反馈。
￼
图6-4 virtio驱动程序架构
