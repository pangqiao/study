...
当**Guest OS**需要访问到**物理地址**中时，**vCPU**中会产生**缺页异常**，也就是page fault int，此时会跳出**KVM的执行流程**。KVM的异常执行程序会处理这个异常，并且根据Guest OS的page table填充shadow page。此时，shadow page中就保存了Guest OS中的虚拟地址到Host OS中的物理地址的转换信息。
￼
