...
**kvm\_dev\_ioctl\_create\_vm**函数通过调用**kvm\_create**函数对**KVM结构体**进行创建。**KVM 结构体**如前文所述，保存了**虚拟机运行的上下文**及**其他相关状态**，在**使用之前**，需要进行**一定的初始化**工作。
...
该操作主要是尝试将**所有的CPU**切换入vitualize模式，并且设置好时钟等信息，这个过程通过 kvm\_arch\_hardware\_enable 函数完成。该函数代码(arch/x86/kvm/x86.c)如下，主要执行了两个工作：整理CPU的时钟信息；调用kvm\_x86\_ops的硬件相关的函数进行具体操作。
