...
打开/dev/kvm 文件并且获得文件描述符 fd 后，通过 ioctl 指令写入KVM\_CREATE\_KVM，即可创建一个 VM 虚拟。KVM 的**该部分代码**实现在**kvm\_dev** 的 **file\_operation 结构体**中，对应的代码在 **kvm\_main.c** 中调用 **kvm\_dev\_ioctl\_creat\_vm**函数实现，其代码如下：
