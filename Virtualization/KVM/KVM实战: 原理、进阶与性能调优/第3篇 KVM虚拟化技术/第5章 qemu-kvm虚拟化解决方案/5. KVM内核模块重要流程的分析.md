...
- [2 虚拟机的创建](#2-虚拟机的创建)
  - [2.1 虚拟机创建过程kvm\_dev\_ioctl\_create\_vm](#21-虚拟机创建过程kvm_dev_ioctl_create_vm)
    ...
    - [2.1. 返回kvm\_vm的fd文件描述符](#21-返回kvm_vm的fd文件描述符)
- [3 vCPU的创建](#3-vcpu的创建)
...
每个vCPU的创建先alloc一个struct **vcpu\_vmx**里面包含了msr, vmcs, vcpu等, 这里不是主要对kvm\_vcpu做初始化
