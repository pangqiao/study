...
- [1. 摘要](#1-摘要)
- [2. Virtio Spec 协议简介](#2-virtio-spec-协议简介)
  - [2.1. virtio 设备的四要素](#21-virtio-设备的四要素)
    - [2.1.1. 设备状态域](#211-设备状态域)
    - [2.1.2. feature bits](#212-feature-bits)
    - [2.1.3. virtqueue](#213-virtqueue)
  - [2.2. 设备 IO 工作机制](#22-设备-io-工作机制)
  - [2.3. 三种呈现模式](#23-三种呈现模式)
- [3. virtio-pci 呈现](#3-virtio-pci-呈现)
  - [3.1. virtio legacy](#31-virtio-legacy)
  - [3.2. virtio modern](#32-virtio-modern)
- [4. 前后端数据共享](#4-前后端数据共享)
  - [4.1. Desriptor Table](#41-desriptor-table)
  - [4.2. Available Ring](#42-available-ring)
...
* `cfg_type` 表示 Cap 的**类型**,

* `bar` 表示这个**配置结构**被映射到的 **BAR 空间号**. 这样**每个配置结构**都可以**通过 BAR 空间直接访问**, 或者**通过 PCI 配置空间**的 `VIRTIO_PCI_CAP_PCI_CFG` **域**进行访问.
...
**Desriptor Table** 中存放的是**一个一个**的 **virtq_desc 元素**, **每个** `virq_desc` 元素占用 **16 个字节**(128 位).
