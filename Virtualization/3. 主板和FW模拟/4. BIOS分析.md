...
相比于实际的物理 BIOS, SeaBIOS 的功能简单很多, 因为虚拟化环境中并不存在实际的物理硬件, 很多硬件配置可以省略, SeaBIOS 只需要关注一些功能接口即可. SeaBIOS 是QEMU运行虚拟机的第一部分代码, 理解SeaBIOS的工作原理, 对于理解QEMU和KVM虚拟化方案以及排查一些疑难问题都非常有帮助. 本节后面会首先介绍QEMU对SeaBIOS的加载, 之后会对SeaBIOS的代码流程进行简单讲解.
