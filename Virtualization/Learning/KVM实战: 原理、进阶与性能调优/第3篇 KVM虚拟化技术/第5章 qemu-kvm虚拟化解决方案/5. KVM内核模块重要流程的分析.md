...
kvm_vm_ioctl() // kvm ioctl vm指令入口
    kvm_vm_ioctl_create_vcpu() // 为虚拟机创建VCPU的ioctl调用的入口函数
        ...
        kvm_arch_vcpu_create() // 初始化架构相关，对于intel x86来说，最终调用vmx_create_vcpu
            kvm_mmu_create() // vcpu->arch中mmu相关初始化
                alloc_mmu_pages() // 给vcpu->arch的gust mmu和root mmu分配页面
            kvm_create_lapic() //
            alloc_page(); // 给vcpu->arch的pio分配内存页
            vcpu->arch.mce_banks = kzalloc();
