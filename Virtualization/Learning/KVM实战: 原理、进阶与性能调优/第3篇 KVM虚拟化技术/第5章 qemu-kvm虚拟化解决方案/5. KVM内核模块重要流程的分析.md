...
- [4. 虚拟机的创建](#4-虚拟机的创建)
  - [4.1. 虚拟机创建过程kvm_dev_ioctl_create_vm](#41-虚拟机创建过程kvm_dev_ioctl_create_vm)
    - [4.1.1. kvm_create_vm流程](#411-kvm_create_vm流程)
    - [4.1.2. 返回kvm_vm的fd文件描述符](#412-返回kvm_vm的fd文件描述符)
- [5. 虚拟机fd的ioctl接口kvm_vm_ioctl](#5-虚拟机fd的ioctl接口kvm_vm_ioctl)
- [6. vCPU的创建](#6-vcpu的创建)
  - [6.1. vCPU的创建过程kvm_vm_ioctl_create_vcpu](#61-vcpu的创建过程kvm_vm_ioctl_create_vcpu)
    - [6.1.1. kvm_arch_vcpu_create创建kvm_vcpu](#611-kvm_arch_vcpu_create创建kvm_vcpu)
    - [6.1.2. kvm_arch_vcpu_setup初始化kvm_vcpu](#612-kvm_arch_vcpu_setup初始化kvm_vcpu)
    - [6.1.3. vCPU检测](#613-vcpu检测)
    - [6.1.4. 创建vcpu_fd](#614-创建vcpu_fd)
    - [6.1.5. 释放内核锁和返回vcpu_fd](#615-释放内核锁和返回vcpu_fd)
- [7. vCPU的运行](#7-vcpu的运行)
...
**打开/dev/kvm** 文件并且获得**文件描述符 fd** 后，通过 `kvm_dev_ioctl` 指令写入KVM\_CREATE\_VM，即可创建一个 VM 虚拟机。
...
## 4.1. 虚拟机创建过程kvm_dev_ioctl_create_vm
...
## 6.1. vCPU的创建过程kvm_vm_ioctl_create_vcpu
...
### 6.1.1. kvm_arch_vcpu_create创建kvm_vcpu
...
### 6.1.2. kvm_arch_vcpu_setup初始化kvm_vcpu
