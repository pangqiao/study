...
- [QEMU与KVM交互](#qemu与kvm交互)
- [2. /dev/kvm的ioctl接口kvm_dev_ioctl](#2-devkvm的ioctl接口kvm_dev_ioctl)
- [3. 虚拟机的创建](#3-虚拟机的创建)
  - [3.1. 虚拟机创建过程kvm_dev_ioctl_create_vm](#31-虚拟机创建过程kvm_dev_ioctl_create_vm)
    - [3.1.1. kvm_create_vm流程](#311-kvm_create_vm流程)
    - [3.1.2. 返回kvm_vm的fd文件描述符](#312-返回kvm_vm的fd文件描述符)
- [4. 虚拟机fd的ioctl接口kvm_vm_ioctl](#4-虚拟机fd的ioctl接口kvm_vm_ioctl)
- [5. vCPU的创建](#5-vcpu的创建)
  - [5.1. vCPU的创建过程kvm_vm_ioctl_create_vcpu](#51-vcpu的创建过程kvm_vm_ioctl_create_vcpu)
    - [5.1.1. kvm_arch_vcpu_create创建kvm_vcpu](#511-kvm_arch_vcpu_create创建kvm_vcpu)
    - [5.1.2. kvm_arch_vcpu_setup初始化kvm_vcpu](#512-kvm_arch_vcpu_setup初始化kvm_vcpu)
    - [5.1.3. vCPU检测](#513-vcpu检测)
    - [5.1.4. 创建vcpu_fd](#514-创建vcpu_fd)
    - [5.1.5. 释放内核锁和返回vcpu_fd](#515-释放内核锁和返回vcpu_fd)
- [6. vCPU的运行](#6-vcpu的运行)
...
# 2. /dev/kvm的ioctl接口kvm_dev_ioctl
...
## 3.1. 虚拟机创建过程kvm_dev_ioctl_create_vm

**打开/dev/kvm** 文件并且获得**文件描述符 fd** 后，通过 ioctl 指令写入KVM\_CREATE\_VM，即可创建一个 VM 虚拟机。
...
## 5.1. vCPU的创建过程kvm_vm_ioctl_create_vcpu
...
### 5.1.1. kvm_arch_vcpu_create创建kvm_vcpu
...
### 5.1.2. kvm_arch_vcpu_setup初始化kvm_vcpu
