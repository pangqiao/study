...
kvm\_vcpu中描述符包含的内容有很多，通常会包含各个**平台通用的内容**和**平台相关的内容**。

在**物理CPU上电**之后，需要进一步初始化才可以使用。在这个过程中，硬件会自动将CPU初始化成特定的状态。
...
- 初始化 kvm\_vcpu 的**状态信息**，标识该VCPU当前所处的状态(睡眠、运行等)，主要供调度器使用。
