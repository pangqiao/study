...
“QEMU通过IOCTL函数使用`/dev/kvm`设备来调用KVM内核模块提供的API，从而与KVM内核模块进行交互。提供的API中包括：`创建客户机（KVM_CREATE_VM）`、`为客户机创建vCPU（KVM_CREATE_VCPU）`、运行vCPU（KVM_RUN）、获取KVM的版本信息（KVM_GET_API_VERSION）、查询KVM的特性支持（KVM_CHECK_EXTENSION）等。在QEMU中将这一系列IOCTL函数调用分为4个类别：kvm_ioctl()、kvm_vm_ioctl()、kvm_vcpu_ioctl()和kvm_device_ioctl()。这4个函数都定义在kvm-all.c文件中，分别用于表示对KVM内核模块本身、对虚拟客户机、对客户机的vCPU和对设备进行交互。
