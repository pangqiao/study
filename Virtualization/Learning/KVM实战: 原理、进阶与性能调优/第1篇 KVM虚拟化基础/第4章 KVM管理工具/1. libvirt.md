...
- [4. libvirt域的XML配置文件](#4-libvirt域的xml配置文件)
  - [4.1. 客户机的XML配置文件格式的示例](#41-客户机的xml配置文件格式的示例)
  - [4.2. CPU、内存、启动顺序等基本配置](#42-cpu-内存-启动顺序等基本配置)
    - [4.2.1. CPU的配置](#421-cpu的配置)
    - [4.2.2. 内存的配置](#422-内存的配置)
    - [4.2.3. 客户机系统类型和启动顺序: os标签](#423-客户机系统类型和启动顺序-os标签)
  - [4.3. 网络的配置](#43-网络的配置)
    - [4.3.1. 桥接方式的网络配置](#431-桥接方式的网络配置)
    - [4.3.2. NAT方式的虚拟网络配置](#432-nat方式的虚拟网络配置)
      - [4.3.2.1. 默认配置](#4321-默认配置)
    - [4.3.3. 用户模式网络的配置](#433-用户模式网络的配置)
    - [4.3.4. 网卡设备直接分配（VT\-d）](#434-网卡设备直接分配vt-d)
      - [4.3.4.1. interface方式: 仅支持SRIOV的VF透传](#4341-interface方式-仅支持sriov的vf透传)
      - [4.3.4.2. hostdev方式: VF透传和直接透传](#4342-hostdev方式-vf透传和直接透传)
  - [4.4. 存储的配置](#44-存储的配置)
  - [4.5. 其他配置简介](#45-其他配置简介)
    - [4.5.1. 域的配置](#451-域的配置)
    - [4.5.2. 域的元数据配置](#452-域的元数据配置)
    - [4.5.3. QEMU模拟器的配置](#453-qemu模拟器的配置)
    - [4.5.4. 图形显示方式](#454-图形显示方式)
    - [4.5.5. 客户机声卡和显卡的配置](#455-客户机声卡和显卡的配置)
    - [4.5.6. 串口和控制台](#456-串口和控制台)
    - [4.5.7. 输入设备](#457-输入设备)
    - [4.5.8. PCI控制器](#458-pci控制器)
- [5. libvirt API简介](#5-libvirt-api简介)
  - [5.1. 连接Hypervisor相关的API](#51-连接hypervisor相关的api)
  - [5.2. 域管理的API](#52-域管理的api)
  - [5.3. 节点管理的API](#53-节点管理的api)
  - [5.4. 网络管理的API](#54-网络管理的api)
  - [5.5. 存储卷管理的API](#55-存储卷管理的api)
  - [5.6. 存储池管理的API](#56-存储池管理的api)
  - [5.7. 事件管理的API](#57-事件管理的api)
  - [5.8. 数据流管理的API](#58-数据流管理的api)
- [6. 建立到Hypervisor的连接](#6-建立到hypervisor的连接)
  - [6.1. 本地URI](#61-本地uri)
  - [6.2. 远程URI](#62-远程uri)
  - [6.3. 使用URI建立到Hypervisor的连接](#63-使用uri建立到hypervisor的连接)
- [7. libvirt API使用示例](#7-libvirt-api使用示例)
  - [7.1. libvirt的C API的使用](#71-libvirt的c-api的使用)
    - [7.1.1. 启动libvirtd守护进程](#711-启动libvirtd守护进程)
    - [7.1.2. libvirt\-devel软件包](#712-libvirt-devel软件包)
    - [7.1.3. C程序查询](#713-c程序查询)
    - [7.1.4. virsh命令查询](#714-virsh命令查询)
  - [7.2. libvirt的Python API的使用](#72-libvirt的python-api的使用)

<!-- /code_chunk_output -->

# 1. libvirt简介

libvirt是目前使用最为广泛的对KVM虚拟机进行管理的工具和应用程序接口，而且一些常用的虚拟机管理工具（如**virsh**、**virt\-install**、**virt\-manager**等）和云计算框架平台（如OpenStack、ZStack、OpenNebula、Eucalyptus等）都在**底层使用libvirt的应用程序接口**。
...
#### 4.3.4.1. interface方式: 仅支持SRIOV的VF透传
...
## 7.1. libvirt的C API的使用

### 7.1.1. 启动libvirtd守护进程

在使用libvirt API之前，**必须**要在**远程或本地的节点**上**启动libvirtd守护进程！！！**。

### 7.1.2. libvirt\-devel软件包
