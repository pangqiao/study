...
当向 2h 端口写人的值为 h 时，则认为写入 ICW 字，那么 guest 正处于 8259 的

初始化当中。8259 期望后续向 21h 端口分别写入 ICW2、ICW3 及 CW4 字（这几个初始化字使用同一个端口）

VMM 必须根据这个原理来监控 guest 初始化 8259。下面的伪代码是当 guest 尝试执

行 O 指令被拦截后，VMM 做出的粗略处理
