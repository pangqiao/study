...
小结: 当Guest在一个异常向量A的delivery期间发生了另外一个异常B(一定是异常), A和B符合下面表描述的, 同时异常B导致了`VM-exit`(exception bitmap相应位为1时). 当同时满足, 
* VMM会看到`IDT-vectoring information` 字段的`bit 31` (valid位) 为1时
* VMM看到`IDT-vectoring information`和`IDT-vectoring error code`记录异常A信息
* VMM看到`VM-exit interruption information`   同时符合下面说的两种类型情况, 则VMM需要反射一个`DF`异常
