...
小结: 当Guest在一个异常向量delivery期间发生了另外一个错误A(不一定是异常), 而这个错误A导致了`VM-exit`. 这时, VMM会看到`IDT-vectoring information` 字段的`bit 31` (valid位) 为1
...
注: 虚拟机原始异常delivery期间遇到错误(不一定是异常), 而这个错误会导致VM-exit. 见3.10说明. 这个时候能确定上面两个字段, 只有delivery期间发生的是外部中断、NMI、硬件异常以及软件异常, 才会有`VM-exit interruption information`和`VM-exit interruption error code`信息.

## 

小结: 当Guest在一个异常向量A的delivery期间发生了另外一个异常B(一定是异常), 同时异常B直接导致了`VM-exit`(exception bitmap相应位为1时). 当同时满足, 
* VMM会看到`IDT-vectoring information` 字段的`bit 31` (valid位) 为1时
* VMM看到`IDT-vectoring information`和`IDT-vectoring error code`记录异常A信息, 间接向量事件类信息字段, 因为异常A不直接导致VM-exit.
* VMM看到`VM-exit interruption information`和`VM-exit interruption error code`记录异常B信息, 直接向量事件类信息字段, 因为异常B直接导致了`VM-exit`   同时符合下面说的两种类型情况, 则VMM需要反射一个`DF`异常
