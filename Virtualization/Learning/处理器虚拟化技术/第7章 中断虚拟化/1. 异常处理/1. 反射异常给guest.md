...
1. `IDT-vectoring information` 字段的`bit 31` (valid位) 为1表明是这个字段异常向量delivery期间发生了另外一个错误(不一定是异常), 而这个异常导致了`VM-exit`
...
2. 
注: 即这两个异常属于上面的两种情况, 但是嵌套的异常对应的exception bitmap相应位不为1, 所以不会`VM-exit`, 但是会转化成 `#DF` ？？？为什么？？？也就是说只要虚拟机属于上面异常嵌套的两种情况, 都会转换成`DF`, 只是说
