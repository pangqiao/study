...
取决于 "`use TPR shadow`"、“`APIC-register virtualization`" 以及“`virtual-interrupt delivery`" 位的设置，guest 访问 `APC-access page` 页面可以**产生 VM-exit**，成功访问 `virtual-APIC page` 内的虚拟寄存器或者执行某些虚拟化操作。具体见 `7.2.5` 和 `7.2.6`.
