...
在 `lib\Vmx\VmxInit.asm` 文件的 int vm execution control fields 函数初始化期间，默认的 APC- access page 页面物理地址设置为 OFEEO 这里保持了一致，将 guest 尝试写入 1A32 APIC BASE 寄存器的基址映射到 OFEEO00 地址上。

1. 设置 guest I 默认 A32APIC_BASE

由于 host 可能会更改 1A32 APIC BASE 寄存器的值，为了保持 guest 环境在 REST 后的状态，需要在 Init guest a 和 init guest b 函数里，将 guest 默认的 A32APIC_BASE 值设置为 OFEEO09OH