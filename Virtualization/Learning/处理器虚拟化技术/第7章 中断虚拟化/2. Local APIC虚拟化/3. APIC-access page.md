...
通过在 VM-exit MSR-load 列表里增加 1A32 APIC BASE 寄存器数据来恢复 host 原

监控了 guest 尝试更改 A32APIC_BASE 寄存器的操作。实际

上，1A32APIC_BASE 寄存器并没有被修改过。在这种情况下，恢复 host 的

A32APIC_BASE 寄存器操作不是必需的！

7.2.4 拟化 x2 APIC MSR 组
