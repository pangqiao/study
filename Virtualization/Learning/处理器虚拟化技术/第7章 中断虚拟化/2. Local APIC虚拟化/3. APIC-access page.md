...
通过在 VM-exit MSR-load 列表里增加 `IA32_APIC_BASE` 寄存器数据来恢复 host 原值. 然而, 由于 VMM 监控了 guest 尝试更改 `IA32_APIC_BASE` 寄存器的操作。实际上，1A32APIC_BASE 寄存器并没有被修改过。在这种情况下，恢复 host  IA32_APIC_BASE 寄存器操作不是必需的！
