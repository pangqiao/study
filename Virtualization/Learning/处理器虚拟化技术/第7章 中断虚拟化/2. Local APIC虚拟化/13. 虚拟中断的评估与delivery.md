...
在 host 端里，VMM 除了可以**注人虚拟中断**外，也可以利用 `posted-Interrupt processing` 机制来实现**虚拟中断的 delivery**。VMM 需要通过 posted- interrupt notification vector 字段来发送一个通知 IPI 中断给 guest, guest 收到通知 IPI 后接着处理 VMM 设置的 posted-  interrupt（通告中断）
