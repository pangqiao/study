...
由此可见，当 local APIC 收到外部中断请求时，要么产生 `VM-exit`，要么就执行**虚拟中断的评估与 delivery 操作**。“`acknowledge interrupt on exit`”位为 1, 表明**处理器**收到**外部中断请求**后，将**响应**中断控制器（local APIC）从而**取得中断向量信息**。
...
VMM需要在`posted-interrupt notification vector`字段里提供一个**通知向量号**(见3.5.13). 那么, local APIC接收到的这个外部中断可被成为"通知中断", 其作用是**通知guest进行posted-interrupt的处理**.
