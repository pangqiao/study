...
发布中断处理是一种功能，通过该功能处理器可以通过将虚拟中断记录为在虚拟APIC页面上挂起来处理虚拟中断。

通过设置“进程发布中断” VM执行控件来启用发布中断处理。 响应于具有中断后通知向量的中断的到达而执行处理。 响应于这样的中断，处理器处理记录在称为张贴中断描述符的数据结构中的虚拟中断。 中断后通知向量和中断后描述符的地址是VMCS中的字段； 参见第24.6.8节。
如果“进程发布的中断” VM执行控制为1，则逻辑处理器将使用位于发布的中断描述符地址的64字节的发布的中断描述符。 发布的中断描述符具有以下格式：
...
`posted-interrupt`(直译为"通告中断")是指**VMM**张贴出来给guest进行处理的**虚拟中断**.
