...
`DoWritemsrforapicbase` 函数实现在 `lib\Vmx\Vmxmsr.asm` 文件里，它根据 guest 设置 local APIC 的模式（XAPIC 还是 x2APIC），以及 secondary processor- based VM- execution control 字段的设置进行虚拟化配置。