...
上面代码来自`DoEptViolaton`例程, 它读取引发 EPT violation 故障的GPA地址值, 然后检查这个GPA是否需要额外处理. 使用 `GetGpaHte` 函数来查找GPA地址值对应的HTE(Handler Table Entry)表项, 如果找到则调用 GPA 对应的处理例程来处理. 处理完毕后检查返回值来确定
