...
- [1. 两种监控模式](#1-两种监控模式)
  - [1.1. 监控基于内存映射的local APIC访问: APIC-page](#11-监控基于内存映射的local-apic访问-apic-page)
  - [1.2. 监控基于MSR的localAPIC访问: MSR空间](#12-监控基于msr的localapic访问-msr空间)
...
在**不引入任何lapic 虚拟化机制**情况下(`7.2.2`节), VMM通过**EPT映射机制**或者`MSR-bitmap`来限制guest访问local APIC, 其原理是监控guest访问local APIC的行为并做出相应处理。
