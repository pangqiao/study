...
Appendgpahte 函数的定义看起来像上面的 C 代码，实现在 libvmxvmx Page.asm 文件里。在 GPA HTE (Handler Table Entry）区域建立 GPA 与 EPT violation 处理例程的对应关系。这个 GPA HTE 区域在 init vmcs buffer 函数初始化时在 kernel pool 里分配。GPA HTE 结构定义在 inclvmcs.inc 文件里。
