...
VsbBase 指向 VM 对应的 VSB (VM Store Block）结构，而 Domain Base 指向 VM 所使用的内存 domain 基址（参见 6.3.1 节）。GuestEntry 和 HostEntry 分别指向 guest 和 host 的入口地址。每个虚拟机的 Guestentry 值一般情况下是不同的，Hostentry 值可以样，表示 VMM 管理多个虚拟机。
