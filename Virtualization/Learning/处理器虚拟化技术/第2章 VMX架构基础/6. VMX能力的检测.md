...
* bit 48指示可用于VMXON区域，每个VMCS以及VMCS中的指针所引用的数据结构的物理地址的宽度（I / O位图，virtual-APIC页面，VMX Transi-
2 tions。）。如果0为0，则这些地址被限制为处理器的物理地址宽度。 如果是
这些地址限制为32位。 对于支持Intel 64架构的处理器，该位始终为0。
•如果将位49读为1，则逻辑处理器支持对系统管理中断和系统管理模式进行双监视器处理。 有关此处理的详细信息，请参见第34.15节。
