...
VMX体系结构被设计为**可扩展**的，以便将来VMX操作中的处理器可以支持VMX体系结构的第一代实现中不存在的其他功能。 使用一组**VMX功能MSR**将可扩展的VMX功能的**可用性报告**给软件。

`CPUID.1:ECX.VMX[bit 5] = 1`指示处理器支持VMX操作和相关指令的能力。

通过从一组功能MSR中读取值来确定对第26章和其他VMX各章中详述的特定功能的支持。 从MSR地址480H开始对这些MSR进行索引。 VMX功能MSR是只读的。 尝试使用WRMSR写入它们会产生一般保护异常（#GP（0））。 它们在不支持VMX操作的处理器上不存在。 尝试在此类处理器上读取它们（使用RDMSR）会产生一般保护异常（#GP（0））。
