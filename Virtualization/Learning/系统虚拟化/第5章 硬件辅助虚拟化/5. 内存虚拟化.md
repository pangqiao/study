...
- [1 概述](#1-概述)
  - [1.1 影子页表](#11-影子页表)
  - [1.2 VT\-x支持](#12-vt-x支持)
- [2 Intel EPT](#2-intel-ept)
  - [2.1 EPT原理](#21-ept原理)
    - [2.1.1 地址转换过程](#211-地址转换过程)
  - [2.2 EPT的硬件支持](#22-ept的硬件支持)
    - [2.2.1 VM-Execution控制域的Enable EPT字段](#221-vm-execution控制域的enable-ept字段)
    - [2.2.2 VM-Execution控制域的Extended page table pointer字段](#222-vm-execution控制域的extended-page-table-pointer字段)
    - [2.2.3 TLB、INVEPT指令和VPID](#223-tlb-invept指令和vpid)
    - [2.2.4 VM\-Exit信息域的EPT Violation相关字段](#224-vm-exit信息域的ept-violation相关字段)
  - [2.3 EPT的软件使用](#23-ept的软件使用)
- [3 VPID](#3-vpid)
  - [3.1 传统TLB](#31-传统tlb)
  - [3.2 TLB项的VPID标识不同虚拟处理器的不同TLB](#32-tlb项的vpid标识不同虚拟处理器的不同tlb)
  - [3.3 VPID的软件使用](#33-vpid的软件使用)
- [4 Linux系统检查](#4-linux系统检查)
  - [4.1 /proc/cpuinfo](#41-proccpuinfo)
  - [4.2 sysfs文件系统](#42-sysfs文件系统)
...
- **GPA→HPA**的转换是由**VMM决定**, VMM在将**物理内存分配给客户机时**就确定了**GPA→HPA**的转换, VMM通常会用内部数据结构来记录这个关系. VMM 为每个虚拟机动态地维护了一张客户机物理地址与宿主机物理地址映射表。

## 1.1 影子页表

传统的IA32架构只支持一次地址转换, 即通过**CR3指定的页表**实现"**虚拟地址**"→"**物理地址**"的转换. 

这和**内存虚拟化**所要求的**两次地址转换**产生矛盾. 
...
上图可以看出, CPU需要**5次查询EPT页表(CR3\<L4页表地址GPA>、L3页表地址、L2页表地址、L1页表地址和最终GPA查询EPT页表！！！**), **每次查询！！！**都需要**4次内存访问(！！！每级页表都是一次内存访问！！！**), 最坏共**20次内存访问**. EPT硬件通过增大EPT TLB来尽量减少内存访问.
