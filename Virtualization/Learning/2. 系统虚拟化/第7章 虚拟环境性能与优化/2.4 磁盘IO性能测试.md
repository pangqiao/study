...
图10-14所示为Bonnie++中文件读写操作的测试结果对比，其中的Seq-O-Per-Char￼表示顺序按字节写、Seq-O-Blk表示顺序按块写，Seq-O-Rewr就表示先读后写并lseek，Seq-I-Per-Char表示顺序按字节读，Seq-I-Blk表示顺序按块读。图10-15为文件创建、删除操作的测试结果对比，其中Rand、Seek表示随机改变文件读写指针偏移量（使用lseek()和random()函数），Seq-Create、Seq-Del等顾名思义，不赘述了。
