...
guest写入任务优先级寄存器（TPR），写入中断结束（EOI）寄存器和中断命令低位寄存器（ICRL）的特定使用情况会导致**特定的硬件操作**。 由于该寄存器的写入没有立即的硬件副作用，因此AVIC硬件允许guest写入中断命令寄存器高电平（ICRH）。 中航工业硬件维护并使用处理器优先级寄存器（PPR）中的值来控制向guest虚拟处理器的中断传递。 以下各节讨论guest对vAPIC支持页面中的这些寄存器的访问处理。