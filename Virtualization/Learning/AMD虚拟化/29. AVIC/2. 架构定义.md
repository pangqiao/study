...
### 门铃信号处理: Processing of Doorbell Signals

硬件可以识别传递给正在运行的guest的门铃信号，而不管它是否可以作为虚拟中断立即注入到guest中。 在下一个VMRUN上，虚拟中断传递机制评估guest的vAPIC支持页的IRR寄存器的状态，以找到优先级最高的未决中断，并在中断屏蔽和优先级允许的情况下将其注入。