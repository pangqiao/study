...
1. 如果命令中编码的 `destination-shorthand` 为**01b**（即**self**），请更新backing 页面中的IRR，**向自己**发出**门铃**(doorbell)信号，并跳过其余步骤。
2. 如果`destination-shorthand`**不为零**(非自定义目标)，或者目标字段(destination field)为**FFh**（即**广播**），请跳到步骤4。
...
5. 对于**每个有效的目的地**：
    ...
    - 如果目标IsRunning位为1，则使用**物理APIC ID表**中的Host Physical core号发送**门铃消息**。
6. 如果任何目的地被标识为当前未在物理核心上调度（即，未设置该虚拟处理器的IsRunning位），则导致#VMEXIT。
...
**每个core提供门铃机制**，**其他CPUcore**(用于**IPI**)和**IOMMU**(用于**设备中断**)使用该**门铃机制**向**目标物理core的VMM**发出信号，告知**需要处理虚拟中断**。 而这个对虚拟中断的处理还是会去读取IRR寄存器的内容, 详细的下面会讲到.

准确的机制是特定实现的，但是必须加以保护，以防止其他core上运行的非特权软件的访问以及外部设备的直接访问。
...
除了正常的VMRUN操作外，core在**进入guest时！！！** 还会在**vAPIC backing页面**中**重新评估APIC状态**，并根据需要**处理未决的中断！！！**。 这个就是额外的VMRUN处理, AVIC打开后的效果.
