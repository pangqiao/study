...
处理器实现可以在`VM_HSAVE_PA MSR`指向的存储区域中**仅**存储**主机状态**的**一部分**或**不存储任何主机状态**，并且可以将**某些**或**全部主机状态**存储在隐藏的**片上存储器**(on-chip memory)中。**不同的实现方式**可能会选择保存主机的**段寄存器**以及**选择器的隐藏部分**。由于这些原因，**软件**不得使用主机状态保存区的格式或内容，也不得尝试通过修改主机保存区的内容来更改主机状态。
