...
* 把上面的函数封装到一个函数中，并且这个函数添加了gcc的attribute：\_\_attribute\_\_((constructor)),其含义是在整个程序的main函数执行之前该函数就会被执行。

至此我们就可以看到，在main函数开始执行之前，init\_type\_list链表就已经初始化完成。因此上面的module\_call\_init(MODULE\_INIT\_QOM);就可以遍历所有的QOM设备并执行他们的init函数，以virtio-blk函数为例，init的执行内容如下。就是注册一下当前的设备类型，设备总线，以及其它一些相关的初始化函数。
