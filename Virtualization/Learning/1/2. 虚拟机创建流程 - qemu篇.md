...
至此我们就可以看到，在**main函数**开始**执行之前**，**init\_type\_list链表**就已经初始化完成。因此上面的module\_call\_init(MODULE\_INIT\_QOM);就可以遍历所有的QOM设备并执行他们的init函数，以virtio-blk函数为例，init的执行内容如下。就是注册一下当前的设备类型，设备总线，以及其它一些相关的初始化函数。
