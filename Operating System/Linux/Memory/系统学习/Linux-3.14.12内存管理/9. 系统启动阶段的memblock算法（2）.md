...
memblock\_add()主要封装了memblock\_add\_region()，特别需要留意它操作对象是memblock.memory（可用可分配的内存），可以推测其意图是将e820的内存信息往这里添加，接着往下看memblock\_add\_region()的实现：
...
加上标示MEMBLOCK\_ALLOC\_ACCESSIBLE表示申请内存可访问，封装调用memblock\_alloc\_base()：
...
继续\_\_memblock\_alloc\_base()（封装了memblock\_alloc\_base\_nid()，**新增NUMA\_NO\_NODE入参表示无NUMA的节点**，毕竟当前还没初始化到那一步）：
...
先看一下memblock\_find\_in\_range\_node()的实现：
...
该函数主要封装了\_\_memblock\_remove()用于对memblock.reserved的操作。
