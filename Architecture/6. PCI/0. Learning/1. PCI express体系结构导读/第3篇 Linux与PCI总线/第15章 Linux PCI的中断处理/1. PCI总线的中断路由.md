...
在 `acpi_prt_list` **链表**中存放 PCI 总线的**中断路由表**, 本章将在第 15.1.3 节进一步介绍该表.
...
这段程序在获得 **GSI** 号之后, 将调用 `acpi_register_gsi` 函数, 将 **GSI** 号**转换**为系统软件使用的 **irq** 号.
...
函数使用三个入口参数, 分别为 **GSI 号**, **中断触发方式**和**采用电平触发时的极性**. 其中 PCI 设备使用**低电平触发**方式.
...
为了深入理解 `acpi_register_gsi` 函数, 需要理解
