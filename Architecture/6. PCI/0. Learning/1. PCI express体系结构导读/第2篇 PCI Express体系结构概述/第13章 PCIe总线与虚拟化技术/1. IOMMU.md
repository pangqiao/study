...
其中每一个 IOMMU 都使用一个 DeviceTable. AMD 处理器使用 Device Table 存放图13-3中的结构, Device Table 最多由2"个Entry 组成, 其中每个 Entry的大小为256b, 因此Device Table最大将占用2MB内存空间, 与Intel使用的Root/ContextEntry结构相比AMD 使用的这种方法容易造成内存的浪费.其中每一个 IOMMU 都使用一个 Device Table. AMD 处理器使用 Device Table 存放图 13-3 中的结构, Device Table 最多由 2^n 个 Entry 组成, 其中每个 Entry 的大小为 256b, 因此 Device Table 最大将占用 2MB 内存空间, 与 Intel 使用的 Root/Context Entry 结构相比 AMD 使用的这种方法容易造成内存的浪费.
