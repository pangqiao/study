...
x86 处理器定义了两个 I/O 端口寄存器, 分别为 CONFIG_ADDRESS 和 CONFIG_DATA 寄存器, 其地址为 0xCF8 和 0xCFG. x86 处理器使用这两个 I/O 端口访问 PCI 设备的配置空间. PCI 总线规范也以这两个寄存器为例, 说明处理器如何访问 PCI 设备的配置空间. 其中 CONFIG_ADDRESS 寄存器存放 PCI 设备的 ID 号, 而 CONFIG_DATA 寄存器存放进行配置读写的数据.
