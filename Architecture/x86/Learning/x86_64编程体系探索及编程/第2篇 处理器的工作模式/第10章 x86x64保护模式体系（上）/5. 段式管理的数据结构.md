...
- [Segment Descriptor（段描述符）](#segment-descriptor段描述符)
  ...
  - [代码段描述符](#代码段描述符)
    ...
    - [conforming与non-conforming代码段](#conforming与non-conforming代码段)
    - [DPL属性](#dpl属性)
    - [S属性](#s属性)
    - [P属性](#p属性)
    - [D/B属性](#db属性)
    - [long mode下的D/B属性](#long-mode下的db属性)
    - [L属性](#l属性)
    - [G属性](#g属性)
  - [long mode下的Code段描述符](#long-mode下的code段描述符)
  - [代码段寄存器的加载](#代码段寄存器的加载)
    - [加载CS寄存器的常规检查](#加载cs寄存器的常规检查)
      - [selector的检查](#selector的检查)
      - [limit的检查](#limit的检查)
      - [Code segment类型的检查](#code-segment类型的检查)
    - [用far pointer加载CS寄存器](#用far-pointer加载cs寄存器)
    - [使用call gate加载CS寄存器](#使用call-gate加载cs寄存器)
      - [offset域](#offset域)
      - [selector域](#selector域)
      - [cnt域](#cnt域)
      - [Call-gate的DPL值](#call-gate的dpl值)
      - [gate selector和gate descriptor的常规检查](#gate-selector和gate-descriptor的常规检查)
      - [使用call指令调用call-gate时的权限检查](#使用call指令调用call-gate时的权限检查)
      - [加载selector及code segment descriptor到CS寄存器](#加载selector及code-segment-descriptor到cs寄存器)
      - [权限的切换](#权限的切换)
      - [当使用jmp指令调用call-gate时](#当使用jmp指令调用call-gate时)
      - [stack的切换](#stack的切换)
      - [long-mode下的Call-gate](#long-mode下的call-gate)
      - [64位Call-gate的offset值](#64位call-gate的offset值)
      - [64位Call-gate中的selector](#64位call-gate中的selector)
      - [64位Call-gate的调用](#64位call-gate的调用)
      - [在compatibility模式下对64位Call-gate的调用](#在compatibility模式下对64位call-gate的调用)
    - [使用TSS selector调用加载CS寄存器](#使用tss-selector调用加载cs寄存器)
      - [TSS descriptor](#tss-descriptor)
      - [TSS类型](#tss类型)
      - [提供TSS selector进行call或jmp调用](#提供tss-selector进行call或jmp调用)
      - [TSS selector及TSS描述符的常规检查](#tss-selector及tss描述符的常规检查)
      - [权限检查](#权限检查)
      - [对原TSS descriptor进行处理](#对原tss-descriptor进行处理)
      - [保存原处理器状态](#保存原处理器状态)
      - [在新的TSS段写入原TSS selector](#在新的tss段写入原tss-selector)
      - [置Eflags.NT标志位](#置eflagsnt标志位)
      - [CR0.TS标志位置位](#cr0ts标志位置位)
      - [新task的TSS descriptor Busy被置位](#新task的tss-descriptor-busy被置位)
      - [加载TR](#加载tr)
      - [加载CS寄存器及其他寄存器](#加载cs寄存器及其他寄存器)
    - [使用iret指令进行任务切换](#使用iret指令进行任务切换)
    - [使用Task-gate加载CS寄存器](#使用task-gate加载cs寄存器)
      - [Selector域](#selector域-1)
      - [Task-gate的类型](#task-gate的类型)
      - [selector和Task-gate描述符的常规检查](#selector和task-gate描述符的常规检查)
      - [访问Task-gate的权限检查](#访问task-gate的权限检查)
    - [在long-mode下TSS及任务切换](#在long-mode下tss及任务切换)
      - [long-mode下的TSS段](#long-mode下的tss段)
      - [64位模式下的TSS描述符](#64位模式下的tss描述符)
      - [long-mode下的TR加载](#long-mode下的tr加载)
    - [使用int指令加载CS寄存器](#使用int指令加载cs寄存器)
      - [IDTR中断描述符表寄存器](#idtr中断描述符表寄存器)
      - [Interrupt/Trap gate descriptor](#interrupttrap-gate-descriptor)
      - [访问gate descriptor](#访问gate-descriptor)
      - [常规检查](#常规检查)
      - [权限的检查](#权限的检查)
      - [Interrupt/Trap-gate与Call-gate的异同](#interrupttrap-gate与call-gate的异同)
      - [long-mode下的Interrupt/Trap-gate描述符](#long-mode下的interrupttrap-gate描述符)
      - [gate的类型](#gate的类型)
      - [IST指针域](#ist指针域)
    - [使用int3、into，以及bound指令加载CS寄存器](#使用int3-into以及bound指令加载cs寄存器)
      - [#OF异常](#of异常)
      - [#BP异常](#bp异常)
      - [#BR异常](#br异常)
    - [使用RETF指令加载CS与SS寄存器](#使用retf指令加载cs与ss寄存器)
      - [权限的处理](#权限的处理)
      - [Selector与所使用的Descriptor的权限检查](#selector与所使用的descriptor的权限检查)
      - [Selector及Descriptor的类型检查](#selector及descriptor的类型检查)
      - [CS与SS寄存器的加载](#cs与ss寄存器的加载)
      - [隐式的Null selector加载](#隐式的null-selector加载)
    - [在long-mode下使用RETF指令](#在long-mode下使用retf指令)
      - [从64位模式返回到64位模式](#从64位模式返回到64位模式)
      - [从64位模式返回到compatibility模式](#从64位模式返回到compatibility模式)
      - [从compatibility模式返回到64位模式](#从compatibility模式返回到64位模式)
      - [Jmp指令在64位操作数下的变通](#jmp指令在64位操作数下的变通)
      - [Jmp指令使用32位操作数](#jmp指令使用32位操作数)
      - [使用retf指令来切换](#使用retf指令来切换)
    - [使用IRET指令加载CS和SS寄存器](#使用iret指令加载cs和ss寄存器)
      - [使用IRET指令返回](#使用iret指令返回)
    - [在long-mode下使用IRETQ指令](#在long-mode下使用iretq指令)
      - [使用IRETQ指令从64位模式返回到64位模式](#使用iretq指令从64位模式返回到64位模式)
      - [使用IRETQ指令从64位模式返回到compatibility模式](#使用iretq指令从64位模式返回到compatibility模式)
      - [使用IRET指令从compatibility模式返回到64位模式](#使用iret指令从compatibility模式返回到64位模式)
    - [使用SYSENTER/SYSEXIT指令加载CS与SS寄存器](#使用sysentersysexit指令加载cs与ss寄存器)
      - [使用SYSENTER指令进入0级权限代码](#使用sysenter指令进入0级权限代码)
      - [使用SYSEXIT指令退回到3级权限代码](#使用sysexit指令退回到3级权限代码)
      - [非对称地使用sysenter/sysexit指令](#非对称地使用sysentersysexit指令)
      - [设置一个stub函数](#设置一个stub函数)
      - [在3级权限里使用sysenter调用](#在3级权限里使用sysenter调用)
    - [在IA-32e模式下使用SYSENTER/SYSEXIT指令](#在ia-32e模式下使用sysentersysexit指令)
      - [设置IA-32e模式里的sysenter/sysexit使用环境](#设置ia-32e模式里的sysentersysexit使用环境)
      - [使用SYSENTER指令进入0级64位代码](#使用sysenter指令进入0级64位代码)
      - [从64位模式进入0级64位模式](#从64位模式进入0级64位模式)
      - [从compatibility模式进入0级64位模式](#从compatibility模式进入0级64位模式)
      - [使用SYSEXIT指令返回](#使用sysexit指令返回)
      - [统一使用SYSEXIT指令返回到64位模式](#统一使用sysexit指令返回到64位模式)
    - [使用SYSCALL/SYSRET指令来加载CS与SS寄存器](#使用syscallsysret指令来加载cs与ss寄存器)
      - [设置SYSCALL指令的使用环境](#设置syscall指令的使用环境)
      - [为SYSCALL指令所准备的stub函数](#为syscall指令所准备的stub函数)
      - [SYSCALL版本的系统服务例程](#syscall版本的系统服务例程)
      - [非对称地使用syscall/sysret指令](#非对称地使用syscallsysret指令)
  - [Stack（栈）结构及Stack的切换](#stack栈结构及stack的切换)
    - [Legacy模式下的Stack](#legacy模式下的stack)
      - [Expand-up类型的stack段（或Data段）](#expand-up类型的stack段或data段)
      - [段Limit值的计算](#段limit值的计算)
      - [Expand-up段的有效范围](#expand-up段的有效范围)
      - [Expand-down类型的stack段（或Data段）](#expand-down类型的stack段或data段)
      - [段Limit值的计算](#段limit值的计算-1)
      - [Expand-down段的有效范围](#expand-down段的有效范围)
    - [在64位模式下的Stack](#在64位模式下的stack)
    - [Data segment descriptor](#data-segment-descriptor)
      - [D/B与G标志位](#db与g标志位)
    - [long-mode下的Data segment descriptor](#long-mode下的data-segment-descriptor)
      - [W标志位](#w标志位)
      - [DPL标志位](#dpl标志位)
      - [Base域](#base域)
      - [FS段和GS段的基地址](#fs段和gs段的基地址)
    - [Stack的使用](#stack的使用)
      - [显式使用SS段](#显式使用ss段)
    - [SS寄存器显式加载](#ss寄存器显式加载)
      - [使用LSS指令加载](#使用lss指令加载)
      - [selector检查](#selector检查)
      - [权限权查](#权限权查)
      - [limit检查](#limit检查)
      - [Data段描述符类型的检查](#data段描述符类型的检查)
    - [TR的显式加载](#tr的显式加载)
      - [置Busy标志位](#置busy标志位)
      - [使用Call-gate调用下的Stack切换](#使用call-gate调用下的stack切换)
      - [TSS段里的栈指针](#tss段里的栈指针)
    - [在long-mode下使用Call-gate调用的Stack切换](#在long-mode下使用call-gate调用的stack切换)
      - [long-mode下的TSS段里的栈指针](#long-mode下的tss段里的栈指针)
      - [加载Null selector到SS寄存器](#加载null-selector到ss寄存器)
    - [使用RETF指令返回时的Stack切换](#使用retf指令返回时的stack切换)
    - [long-mode下使用RETF指令返回时的Stack切换](#long-mode下使用retf指令返回时的stack切换)
      - [返回到compatibility模式时](#返回到compatibility模式时)
      - [返回到64位模式时](#返回到64位模式时)
      - [从伪造的Call-gate服务例程RETF返回时](#从伪造的call-gate服务例程retf返回时)
    - [调用中断或中断/异常发生时的Stack切换](#调用中断或中断异常发生时的stack切换)
    - [long-mode下的中断Stack切换](#long-mode下的中断stack切换)
      - [IST（Interrupt Stack Table）](#istinterrupt-stack-table)
      - [从compatibility模式进入64位模式](#从compatibility模式进入64位模式)
      - [从64位模式进入64位模式](#从64位模式进入64位模式)
    - [使用IRET指令返回时的Stack切换](#使用iret指令返回时的stack切换)
      - [long-mode下的IRETQ返回Stack切换](#long-mode下的iretq返回stack切换)
      - [返回到compatibility模式时](#返回到compatibility模式时-1)
      - [从伪造的中断handler环境中返回](#从伪造的中断handler环境中返回)
  - [Data段](#data段)
    - [段的访问类型限制](#段的访问类型限制)
    - [加载Data段寄存器](#加载data段寄存器)
      - [selector的检查](#selector的检查-1)
      - [权限检查](#权限检查-1)
      - [limit的检查](#limit的检查-1)
      - [Data段描述符的检查](#data段描述符的检查)
    - [加载Code段描述符到Data寄存器](#加载code段描述符到data寄存器)
...
注意：**Null selector不允许加载到CS及SS寄存器**，会产生\#GP异常。允许被加载到**ES、DS、FS，以及GS寄存器**中，但是这些寄存器**使用Null selector进行访问**时会产生\#GP异常。
...
#### gate selector和gate descriptor的常规检查
...
#### 加载selector及code segment descriptor到CS寄存器
...
#### long-mode下的Interrupt/Trap-gate描述符
...
#### 使用IRETQ指令从64位模式返回到compatibility模式
...
#### 使用IRET指令从compatibility模式返回到64位模式
...
### 使用SYSENTER/SYSEXIT指令加载CS与SS寄存器
...
### 在IA-32e模式下使用SYSENTER/SYSEXIT指令
...
#### 设置IA-32e模式里的sysenter/sysexit使用环境
...
### long-mode下的Data segment descriptor
...
### 在long-mode下使用Call-gate调用的Stack切换
