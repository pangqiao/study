...
STAR寄存器的低32位对于Intel64机器来说是无效的，在调用时，syscall指令从`STAR[47：32]`获得CS selector，从`STAR[47：32]+8`获得SS selector。

在返回64位模式时，sysret指令从`STAR[63：48]+16`得到CS selector，从`STAR[63：48]+8`得到SS selector。

返回到compatibility模式时，sysret指令从`STAR[63：48]得到CS selector，从`STAR[63：48]+8`得到SS selector。
