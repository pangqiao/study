...
- [Breakpoint调试模式](#breakpoint调试模式)
- [Memory和I/O地址调试模式](#memory和io地址调试模式)
  - [处理器的支持](#处理器的支持)
    - [DR4和DR5寄存器](#dr4和dr5寄存器)
  - [断点寄存器DR0～DR3](#断点寄存器dr0~dr3)
  - [状态寄存器DR6](#状态寄存器dr6)
...
在上面的代码里，通过使用popf指令对eflags.TF进行置位，处理器在执行完`mov eax，1`指令后进入\#DB handler。
