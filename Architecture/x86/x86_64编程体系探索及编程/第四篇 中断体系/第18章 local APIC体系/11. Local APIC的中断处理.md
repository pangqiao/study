...
对于除NMI，SMI，INIT，ExtINT，start\-up或INIT-Deassert传送模式之外的所有中断，**中断处理程序**必须包括对中断结束（EOI）寄存器的写入。此写操作必须在**处理程序例程的末尾**，在**IRET指令之前**的某个时间发生。此操作表示当前中断的服务已完成，本地APIC可以从ISR发出下一个中断。

收到EOI后，Local APIC**清除ISR中的最高优先级位**，并将下一个最高优先级中断发送给处理器。如果终止的中断是电平触发的中断，则L**ocal APIC**还向**所有I/O APIC**发送**中断结束消息(end-of-interrupt message**)。

系统软件可能更倾向于将EOI引导到特定的I/O APIC，而不是让本地APIC向所有I / O APIC发送中断结束消息。

软件可以通过设置寄生中断向量寄存器的第12位来禁止广播EOI消息（见第10.9节）。如果该位置1，即使相关的TMR位指示当前中断是电平触发，也不会在EOI周期产生广播EOI。该位的默认值为0，表示执行了EOI广播。
如果处理器不支持抑制EOI广播，则伪寄生中断向量寄存器的第12位保留为0。本地APIC版本寄存器的第24位报告了对EOI广播抑制的支持（参见第10.4.8节）;如果该位设置为1，则支持该功能。如果支持，该功能在xAPIC模式和x2APIC模式下均可用。
希望为电平触发中断执行定向EOI的系统软件应设置杂散中断向量寄存器的第12位，并将每个EOI跟随本地xAPIC进行电平触发中断，并通过定向EOI向I / O APIC产生中断（这是通过写入I / O APIC的EOI寄存器来完成的。执行定向EOI的系统软件必须保留将级别触发的中断与系统中的I / O APIC相关联的映射。
