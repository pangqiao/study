...
当**IRQ线(连接的外设线！！！**)上有中断请求发生时，**I/O APIC**在**redirection table**里找到对应的**RTE（redirection table entry或者说redirection table寄存器**），读取**RTE**内的**中断消息内容**发送到**system bus(！！！**)，那么后续的中断处理器流程就和18.11节所述的“local APIC的中断处理”是一致的。
