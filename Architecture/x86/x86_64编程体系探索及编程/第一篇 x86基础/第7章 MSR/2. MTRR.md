...
这些值的作用是在MTRR里为range定义相关的memory类型。

>memory中可以划分哪些区域？

可以划分为3种区域：fixed（固定的），variable（可变的）以及default（默认的），当memory中不划分fixed和variable区域时，这时候memory就是default的区域：

![config](./images/2.png)

如上所示，在整个物理memory中可以划分3种区域形式，MAXPHYADDR的值提供了最高的物理memory数量，它由CPUID.EAX=80000008H.EAX[7：0]中获得，这个值最高是52位，一般要么是36要么是40位。
