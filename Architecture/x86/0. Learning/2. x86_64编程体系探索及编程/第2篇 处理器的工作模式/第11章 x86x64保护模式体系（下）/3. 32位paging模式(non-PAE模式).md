...
- [4. 使用32位paging](#4-使用32位paging)
  ...
  - [4.4. 测试一: 在用户代码里写0x400000地址](#44-测试一-在用户代码里写0x400000地址)
  - [4.5. 测试二: 在0级代码里写0x400000地址](#45-测试二-在0级代码里写0x400000地址)
  - [4.6. 测试三: 在开启WP功能下, 0级代码里写0x400000地址](#46-测试三-在开启wp功能下-0级代码里写0x400000地址)
...
这个标志位指示和决定物理page的访问权限:
* 当为**0**时**只有Supervisor(0级、1级或者2级权限**)才能访问;
* 当为**1**时User(3级权限)和Supervisor可以访问.

关于U/S标志位如何决定物理page的访问权限, 在第11.8节有详细描述.
