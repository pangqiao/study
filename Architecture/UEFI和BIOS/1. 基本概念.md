...
`GUID Partition Table`, 是 EFI 标准的一部分, 用于**替代 MBR 分区表**, 相较起来有**分区更大**、**数量更多**(没有4个主分区的限制)等优势GPT格式的硬盘结构如下可以看到首部MBR的位置有个保护MBR(用于防止不识别GPT的硬盘工具错误识别并破坏硬盘中的数据)这个MBR中只有一个类型为0xEE的分区. GPT结构如下:
