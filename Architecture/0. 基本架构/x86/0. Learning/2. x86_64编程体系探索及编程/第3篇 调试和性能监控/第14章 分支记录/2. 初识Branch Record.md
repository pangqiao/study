...
① **LBR stack**: 即**Last Branch Record 栈**, 这个 stack**由几对 MSR 寄存器！！！组成**.

② **System bus**: 处理器可以将**Branch Trace Message(BTM**)发送到**system bus 上**, 由监控**system bus**的**debugger device**来读取 system bus 上的**BTM 记录**.

③ **BTS(Branch Trace Store**): 处理器可以将**BTM 记录**存储在**内存 Debug Store(DS)区域**中其中一部分的 BTS 区域.
...
① 在**LBR stack**中使用**处理器**提供的**数对 from/to 寄存器**: `MSR\_LASTBRANCH\_n\_FROM\_IP`和`MSR\_LASTBRANCH\_n\_TO\_IP`, 这些寄存器是**MSR 寄存器中的一部分**.

② 以**BTM(Branch Trace Message**)形式: 实际上和上面的 from/to 寄存器是一样的, 区别是可以**存储在内存中(！！！**)或者**发送到 system bus**上.
