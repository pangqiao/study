...
remote IRR: 此位用于电平 (level) 触发中断. 对于边沿触发中断, 其含义未定义. 对于电平 (level) 触发中断, 当 local APIC 接受 IOAPIC 发送的电平中断时, 此位设置为 1. 当从 local APIC 接收到具有匹配中断向量的 EOI 消息时, remote IRR 位设置为 0.
