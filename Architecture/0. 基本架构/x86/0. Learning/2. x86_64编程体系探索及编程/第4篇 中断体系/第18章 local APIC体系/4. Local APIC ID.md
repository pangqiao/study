...
① 在处理器支持**CPUID 0B leaf** 的情况下, 使用`CPUID.0B: EDX[31: 0]`得到**32 位**的 APIC ID 值.

② **不支持 CPUID 0B leaf** 时, 使用`CPUID.01: EBX[31: 24]`得到 **8 位**的 APIC ID 值.
...
2. **判断当前**执行的处理器是**BSP 处理器还是 AP 处理器**: 通过**IA32\_APIC\_BASE[8]位**判断, BSP 与 AP 处理器执行不同的代码.
