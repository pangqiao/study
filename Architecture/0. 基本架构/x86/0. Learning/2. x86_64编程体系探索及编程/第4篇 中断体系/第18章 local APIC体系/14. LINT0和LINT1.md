...
**Trigger Mode(触发模式**)的设置只能使用在 Fixed delivery 模式下: 置 0 时使用 edge 触发, 置 1 时使用 level 触发.
...
在代码里, 我们使用了 Fxied 交付模式及 level 触发, 并将整个 LINT0 的 vector 设置为 IRQ1(keyboard 中断)的中断请求 vector 值. 因此在这种情况下, 所有的 8259 中断控制器传过的中断请求使用**同一个中断处理程序**进行处理(例如: IRQ0 的 timer 中断和 IRQ1 的 keyboard 中断).
