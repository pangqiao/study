...
5) remote IRR flag(bit 14): 这个位**只用于 LVT LINT0 和 LVT LINT1 寄存器**, 使用在 **delivery 模式为 Fixed**, **level 触发模式**中.

* 为 1 时, local APIC **已经接收并处理由 INTR 和 NMI 交付的中断**;
