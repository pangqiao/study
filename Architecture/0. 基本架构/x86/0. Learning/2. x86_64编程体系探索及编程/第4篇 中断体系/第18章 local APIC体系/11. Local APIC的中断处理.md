...
2) 如果 local APIC 确定它是中断的指定目的地，并且中断请求是 NMI、SMI、INIT、ExtINT 或 INIT-deassert 中断，或者是 MP 协议 IPI 消息之一（BIPI、FIPI 和 SIPI），则中断将直接发送到处理器内核进行处理。
