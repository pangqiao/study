...
3) I/O APIC 产生中断消息并转发

* I/O APIC 会读取相应 RTE 信息

* 设置 RTE 中的 delivery status 为 pending, 表明中断已经被触发, 但还没有 delivery 完成.
...
* 最后 I/O APIC 格式化一条中断消息, 发送中断消息从 PCI 桥通过 system bus 到指定的处理器(或一组处理器), 由相应处理器的 local APIC 进行处理.
...
1) 首先它先确认是否是指定的目的地（参见下图）。如果它是指定的目标，则接受消息; 如果不是，则**丢弃消息**。
