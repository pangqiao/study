...
initial\-count与current\-count寄存器是32位寄存器，在**开始计数时**，**软件**需要在**Initial\-count（初始计数）寄存器**提供一个**非0值**的初始计数值。

一旦设置初始计数值，local APIC会将**初始计数值**复制到**只读**的**current\-count（当前计数）寄存器**里。current\-count寄存器的值开始**减1，直至为0值**。current\-count减为0时产生**timer中断**。
...
注意**前两种模式**下，为**Initial Count写入0**即可**停止Timer**运作，在第三种模式下则是`为IA32_TSC_DEADLINE_MSR`写入0，此外**修改模式**也会**停止Timer运行**。当然，也可以通过`LVT Timer Register`中的**Mask屏蔽**Timer中断实现同样的效果。
