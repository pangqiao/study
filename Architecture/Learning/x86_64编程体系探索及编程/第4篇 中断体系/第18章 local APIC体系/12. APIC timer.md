...
使用**periodic**计数模式，当**current\-count**减为**0**时，产生**timer中断**。local APIC将在**current\-count寄存器**里装入**initial\-count**值**重新开始计数**，从而**周期性**地产生timer中断，除非写入0值到initial\-count寄存器里。
