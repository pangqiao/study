...
- [IPI消息对象](#ipi消息对象)
  - [使用No shorthand目标类型](#使用no-shorthand目标类型)
- [使用physical目标模式](#使用physical目标模式)
...
**ICR**的**destination shorthand域**设置了总的**目标类型**，**只有No Shorthand目标类型**才需要**提供具体的IPI消息发送对象**。在Self、All including self，以及All excluding self目标类型中使用固定的目标对象。
