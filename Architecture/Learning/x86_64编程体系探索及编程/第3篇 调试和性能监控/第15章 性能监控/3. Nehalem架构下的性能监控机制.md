...
- [3. counter（计数器）](#3-counter计数器)
  - [3.1. IA32_PMCx与IA32_FIXED_CTRx寄存器](#31-ia32_pmcx与ia32_fixed_ctrx寄存器)
  - [3.2. 触发PMI: 通用和固定达到最大数溢出](#32-触发pmi-通用和固定达到最大数溢出)
...
- [10. 使用Performance monitoring的例子](#10-使用performance-monitoring的例子)
  - [10.1. 开启IA32_PMC0计数器](#101-开启ia32_pmc0计数器)
  - [10.2. 收集信息](#102-收集信息)
  - [10.3. 设置PMI handler](#103-设置pmi-handler)
  - [10.4. 编写PMI handler](#104-编写pmi-handler)
  - [10.5. 清counter溢出标志位](#105-清counter溢出标志位)
  - [10.6. 设置counter溢出条件](#106-设置counter溢出条件)
  - [10.7. 设置的事件](#107-设置的事件)
...
在**Nehalem架构**下，处理器支持**版本3**的**性能监控**，从`CPUID.0AH：EAX[7：0]`返回的**ID值是4**，表明支持**4个PMC寄存器（即general\-purpose counter**）。
