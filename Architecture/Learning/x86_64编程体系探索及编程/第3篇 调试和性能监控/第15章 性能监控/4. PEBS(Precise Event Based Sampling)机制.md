...
- [概述](#概述)
  - [检测PEBS机制是否可用](#检测pebs机制是否可用)
- [1 PEBS buffer](#1-pebs-buffer)
  - [1.1 64位的PEBS记录格式](#11-64位的pebs记录格式)
  - [1.2 增强的PEBS记录格式](#12-增强的pebs记录格式)
  - [1.3 IA32\_PERF\_CAPABILITIES寄存器](#13-ia32_perf_capabilities寄存器)
  - [1.4 PEBS buffer区域](#14-pebs-buffer区域)
  - [1.5 PEBS buffer的maximum与threshold值](#15-pebs-buffer的maximum与threshold值)
  - [1.6 PEBS reset counter值](#16-pebs-reset-counter值)
- [2 PEBS中断](#2-pebs中断)
  - [2.1 开启PEBS计数器](#21-开启pebs计数器)
  - [2.3 PEBS中断的overflow标志位](#23-pebs中断的overflow标志位)
  - [2.4 PEBS中断的抑制](#24-pebs中断的抑制)
  - [2.5 PEBS buffer的溢出](#25-pebs-buffer的溢出)
- [3 PEBS事件](#3-pebs事件)
- [4 PEBS的触发](#4-pebs的触发)
- [5 PEBS记录的报告](#5-pebs记录的报告)
  - [5.1 值得注意](#51-值得注意)
  - [5.2 64位的PEBS记录](#52-64位的pebs记录)
- [6 PEBS buffer满时中断](#6-pebs-buffer满时中断)
  - [6.1 检测中断触发条件](#61-检测中断触发条件)
    - [6.1.1 检测PEBS中断发生](#611-检测pebs中断发生)
    - [6.1.2 检测PEBS buffer满时中断发生](#612-检测pebs-buffer满时中断发生)
  - [6.2 PEBS buffer满时中断优先级](#62-pebs-buffer满时中断优先级)
- [7 多个PMI触发](#7-多个pmi触发)
  - [7.1 counter的优先级](#71-counter的优先级)
  - [7.2 PMI与PEBS中断](#72-pmi与pebs中断)
    - [7.2.1 perfmon中断handler的处理](#721-perfmon中断handler的处理)
  - [7.3 PEBS中断，PEBS buffer与BTS buffer溢出中断](#73-pebs中断pebs-buffer与bts-buffer溢出中断)
    - [7.3.1 检测触发条件的if（）-else if（）逻辑](#731-检测触发条件的if-else-if逻辑)
    - [7.3.2 推荐所有中断触发条件都检测处理](#732-推荐所有中断触发条件都检测处理)
- [8 Load latency监控机制](#8-load-latency监控机制)
  - [8.1 Load latency监控事件](#81-load-latency监控事件)
  - [8.2 Threshold值](#82-threshold值)
  - [8.3 开启load latency监控许可](#83-开启load-latency监控许可)
  - [8.4 Load latency的data source信息](#84-load-latency的data-source信息)
...
由于**PEBS机制**是建立在DS（Debug Store）存储区域之上的，在检测PEBS是否可用时还需要**先检测处理器是否支持DS机制**。
...
Intel特别说明，对于**使用PEBS事件**，在IA32\_PERFEVTSELx寄存器的设置里，ANY位（bit 20）、INV位（bit 23）、CMASK域（bit31到bit24），以及Edge位（bit 18）都必须为0值，否则将是无效的事件。关于**IA32\_PERFEVTSELx寄存器的结构**，请参考15.3.5节。
...
4个通用counter优先于3个固定用途counter，并且它们的优先级以寄存器的序号排序。IA32\_PMC0优先于IA32\_PMC1，以此类推，那么**IA32\_PMC0**是**优先级最高的counter**，而优先级最低的是IA32\_FIXED\_CTR2计数器。
...
## 7.3 PEBS中断，PEBS buffer与BTS buffer溢出中断
...
## 8.4 Load latency的data source信息
