...
- [3. PEBS中断](#3-pebs中断)
  ...
  - [3.4. PEBS buffer的溢出](#34-pebs-buffer的溢出)
- [4. PEBS事件](#4-pebs事件)
- [5. PEBS的触发](#5-pebs的触发)
...
- [7. PEBS buffer满时中断](#7-pebs-buffer满时中断)
  - [7.1. 检测中断触发条件](#71-检测中断触发条件)
    - [7.1.1. 检测PEBS中断发生](#711-检测pebs中断发生)
    - [7.1.2. 检测PEBS buffer满时中断发生](#712-检测pebs-buffer满时中断发生)
  - [7.2. PEBS buffer满时中断优先级](#72-pebs-buffer满时中断优先级)
- [8. 多个PMI触发](#8-多个pmi触发)
  - [8.1. counter的优先级](#81-counter的优先级)
  - [8.2. PMI与PEBS中断](#82-pmi与pebs中断)
    - [8.2.1. perfmon中断handler的处理](#821-perfmon中断handler的处理)
  - [8.3. PEBS中断，PEBS buffer与BTS buffer溢出中断](#83-pebs中断pebs-buffer与bts-buffer溢出中断)
    - [8.3.1. 检测触发条件的if（）-else if（）逻辑](#831-检测触发条件的if-else-if逻辑)
    - [8.3.2. 推荐所有中断触发条件都检测处理](#832-推荐所有中断触发条件都检测处理)
...
## 8.3. PEBS中断，PEBS buffer与BTS buffer溢出中断
