...
- [IA32_DEBUGCTL控制寄存器(Branch记录功能)](#ia32_debugctl控制寄存器branch记录功能)
  - [LBR（Last Branch Record）位](#lbrlast-branch-record位)
  - [BTF（single\-step on branch）位](#btfsingle-step-on-branch位)
  - [TR（trace record message）位](#trtrace-record-message位)
  - [BTS（branch trace store）位](#btsbranch-trace-store位)
  - [BTINT（branch trace interrupt）位](#btintbranch-trace-interrupt位)
- [配置Branch trace record的存放](#配置branch-trace-record的存放)
...
## BTINT（branch trace interrupt）位
