...
这个数量从前面介绍的 IA32_MTRRCAP 寄存器的VCNT域获得, 当VCNT的值设置为10(1010B)时, 目前可使用在variable-range定义的寄存器有10对.

>每个 Variable-range 区域对应着一对 PHYSBASE/PHYSMASK 寄存器.

这10对寄存器是:

① IA32\_MTRR\_PHYSBASE0 到 IA32\_MTRR\_PHYSBASE9.

② IA32\_MTRR\_PHYSMASK0 到 IA32\_MTRR\_PHYSMASK9.

每对 PHYSBASE/PHYSMASK 寄存器配合描述一个variable\-range区域.
