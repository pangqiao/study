...
- [基本原理](#基本原理)
  ...
  - [页表的大小](#页表的大小)
- [在Intel64下的CR3与PDPTE寄存器](#在intel64下的cr3与pdpte寄存器)
  - [CR3](#cr3)
  - [PDPTE寄存器](#pdpte寄存器)
    - [PDPTE寄存器的加载](#pdpte寄存器的加载)
    - [PDPTE寄存器的重新加载](#pdpte寄存器的重新加载)
    - [Intel64下的新PAE paging模式](#intel64下的新pae-paging模式)
...
- [8 使用和测试Execution Disable功能](#8-使用和测试execution-disable功能)
  - [8.1 测试一: 尝试在XD页里执行代码](#81-测试一-尝试在xd页里执行代码)
  - [8.2 测试二: 在PTE.XD=0的页里执行代码](#82-测试二-在ptexd0的页里执行代码)
  - [8.3 测试三: 在\#PF handler里修复错误](#83-测试三-在pf-handler里修复错误)
