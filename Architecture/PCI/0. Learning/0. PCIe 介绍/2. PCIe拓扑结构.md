...
Root Port: A PCI Express Port on a Root Complex that maps a portion of the Hierarchy through an associated virtual PCI-PCI Bridge. 一个位于 Root Complex 上通过相关联的虚拟 PCI-PCI Bridge 映射一个层次结构整体部分的的 PCIE Port.
...
1. Host Bridge 从 CPU 出来, 是整个系统的第一个设备. RC 是 Bus 0; Host Bridge 是 `00:00.0`;

![2021-11-18-21-16-10.png](./images/2021-11-18-21-16-10.png)

2. Host Bridge 往往不会产生真正的 pci 总线, 而是 UPI/QPI/FSB 总线, 所以 Bus 0 其实并不具备 pci 电气, 因而连接真正的 PCI/PCIe 设备需要先有 Host-PCI Bridge(即上面的 PCI-PCI Bridge), 而 Bridge 就会产生新的 Bus number. 


3. Bus 0 上的 PCI Bridge 分为两种: 一种是下面连接 Endpoint 的; 一种则是属于 Switch 的 Upstream Port(即 Root Port)
