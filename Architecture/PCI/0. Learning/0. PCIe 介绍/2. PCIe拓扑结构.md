...
Host Bridge 并不会生成真正的 pci 总线, 而是 UPI/QPI/FSB 总线; 所以 Bus 0 其实并不是 pci 电气, 所以真正的 PCI/PCIe 设备需要先有 Host-PCI Bridge(也就是图中的 PCI-PCI Bridge); 但是 UPI 总线可以直接连接 memory/Display controller, 从而连接 内存/图形.
